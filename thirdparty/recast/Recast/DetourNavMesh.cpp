...
{
	void* mem = dtAlloc(sizeof(dtNavMeshData), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem)dtNavMeshData;
}
...
dtStatus dtNavMesh::init(const dtNavMeshParams* params, dtNavMeshData* oData)
{
	m_data = oData;
	if (m_data)
	{
		memcpy(&m_data->params, params, sizeof(dtNavMeshParams));
	}
