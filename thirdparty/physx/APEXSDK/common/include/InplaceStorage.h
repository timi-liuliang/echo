...
class InplaceStorage
{
	...
	public:
		...
		{
			size_t offset = size_t(reinterpret_cast<physx::PxU8*>(&handle) - _blockPtr);
...
public:
	...
	template <typename T>
	PX_INLINE bool fetch(InplaceHandleBase handle, T& out, physx::PxU32 index = 0) const
	{
	...
	template <typename T>
	PX_INLINE bool update(InplaceHandleBase handle, const T& in, physx::PxU32 index = 0)
	{
	...
	template <typename T>
	PX_INLINE bool updateRange(InplaceHandleBase handle, const T* in, physx::PxU32 count, physx::PxU32 start = 0)
	{
		T* ptr = resolveType<T>(handle);
		if (ptr != 0)
		{
	...
	template <typename T>
	PX_INLINE bool alloc(InplaceHandleBase& handle, physx::PxU32 count = 1)
	{
		PX_ASSERT(count > 0);
	...
	template <typename T>
	PX_INLINE bool alloc(InplaceHandle<T>& handle, physx::PxU32 count = 1)
	{
	...
	template <typename T>
	bool realloc(InplaceHandleBase& handle, physx::PxU32 oldCount, physx::PxU32 newCount)
	{
		if (handle._value != NULL_INDEX)
		{
	...
	physx::PxU32 getAllocatedSize() const
	{
		return _allocatedSize;
	}
...
private:
	...
	PX_INLINE physx::PxI32 getMoveDelta(physx::PxU32 blockIndex, physx::PxU32 moveOffset) const
	{
		...
		physx::PxI32 moveDelta = (physx::PxI32)moveOffset - (physx::PxI32)currOffset;
		//align moveDelta
		if (moveDelta >= 0)
		{
	...
	PX_INLINE bool growBuffer(physx::PxU32 newAllocatedSize)
	{
	...
	{
		physx::PxU8* newBufferPtr = storageResizeBuffer(_allocatedSize);
