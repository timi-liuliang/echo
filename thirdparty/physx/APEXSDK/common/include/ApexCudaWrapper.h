...
struct DimBlock
{
	...
	DimBlock() {}
	DimBlock(PxU32 x, PxU32 y = 1, PxU32 z = 1)
	{
		this->x = x;
		this->y = y;
		this->z = z;
...
{
	PxU32 fixedSharedMemDWords;
	PxU32 sharedMemDWordsPerWarp;
	DimBlock blockDim;
	PxU32 minWarpsPerBlock;
	PxU32 maxGridSize;
	...
	ApexKernelConfig(PxU32 fixedSharedMemDWords, PxU32 sharedMemDWordsPerWarp, int fixedWarpsPerBlock = 0, PxU32 minWarpsPerBlock = 1, PxU32 maxGridSize = MAX_BOUND_BLOCKS)
	{
	...
	ApexKernelConfig(PxU32 fixedSharedMemDWords, PxU32 sharedMemDWordsPerWarp, const DimBlock& blockDim)
	{
		this->fixedSharedMemDWords = fixedSharedMemDWords;
		this->sharedMemDWordsPerWarp = sharedMemDWordsPerWarp;
...
struct ApexCudaMemRefBase
{
	...
	ApexCudaMemRefBase(const void* ptr, size_t byteSize, PxI32 offset, Intent intent)
...
template <class T>
inline ApexCudaMemRef<T> createApexCudaMemRef(const ApexMirroredArray<T>& ma, size_t size, PxI32 offset, ApexCudaMemRefBase::Intent intent = ApexCudaMemFlags::IN_OUT)
{
...
{
	PxU32 mMaxSharedMemPerBlock;
	PxU32 mMaxSharedMemPerSM;
	PxU32 mMaxRegistersPerSM;
	PxU32 mMaxThreadsPerSM;

	PxU32 mBlocksPerSM;
	PxU32 mBlocksPerSM_2D;
	PxU32 mBlocksPerSM_3D;
	PxU32 mMaxBlocksPerGrid;
};
...
public:
	...
	void init(NxModule*	nxModule, ApexCudaTestManager* cudaTestManager, physx::PxGpuDispatcher*	gpuDispatcher)
	{
		mNxModule = nxModule;
		...
		mDeviceTraits.mMaxBlocksPerGrid = PxU32(smCount) * mDeviceTraits.mBlocksPerSM;
#endif
	...
	PX_INLINE NxModule* getNxModule() const
	{
		return mNxModule;
	}
	...
	PX_INLINE physx::PxGpuDispatcher* getGpuDispatcher() const
	{
...
public:
	...
	void bindTo(const void* ptr, size_t bytes, size_t* retByteOffset = 0)
	{
		...
		CUT_SAFE_CALL(cuTexRefSetFlags(mTexRef, (physx::PxU32)mFlags));
...
public:
	...
	void init(ApexCudaObjManager* manager, ApexCudaModule* cudaModule, CUdeviceptr devPtr, size_t size, physx::PxCudaContextManager* ctx)
	{
		ApexCudaObj::init(manager, cudaModule);
...
protected:
	virtual void init(ApexCudaObjManager* , physx::PxCudaContextManager*) = 0;
...
public:
	...
	virtual void init(ApexCudaObjManager* manager, physx::PxCudaContextManager* ctx)
	{
	...
	bool copyToDevice(physx::PxCudaContextManager* ctx, CUstream stream)
	{
		if (mStoragePtr == 0)
		{
			return false;
		...
		if (storage->isChanged())
		{
			...
			physx::PxU8* hostPtr = reinterpret_cast<physx::PxU8*>(mHostBuffer->getPtr());
...
protected:
	bool reallocHostBuffer(physx::PxCudaContextManager* ctx, size_t size)
	{
		if (mHostBuffer == 0)
		{
	...
	virtual physx::PxU8* storageResizeBuffer(physx::PxU32 newSize)
	{
		...
		const physx::PxU32 PageSize = 4096;
		size_t allocSize = mStoreInTexture ? (newSize + (PageSize - 1)) & ~(PageSize - 1) : ApexCudaVar::getSize();

		if (allocSize > mStorageSize)
		{
			physx::PxU8* allocStoragePtr = static_cast<physx::PxU8*>(getAllocator().allocate(allocSize, "ApexCudaConstStorage", __FILE__, __LINE__));
...
private:
	...
	physx::PxCudaBuffer*    mDeviceBuffer;
...
public:
	PX_INLINE bool testNameMatch(const char* name) const
	{
		if (const char* name$ = strrchr(name, '$'))
		{
			...
			{
				return (strncmp(name, mName, (physx::PxU32)(name_ - name)) == 0);
			}
		}
		return (strcmp(name, mName) == 0);
	}

	void init(ApexCudaObjManager* manager, const char* name, CUfunction cuFunc, ApexCudaModule* cudaModule, PxU16 kernelId)
	{
		int funcInstIndex = 0;
		if (const char* name$ = strrchr(name, '$'))
		{
		...
		physx::PxCudaContextManager* ctx = mManager->mGpuDispatcher->getCudaContextManager();
		{
			...
			fid.mName = name;
			fid.mCuFunc = cuFunc;
			fid.mKernelId = kernelId;
			fid.mMaxThreadsPerBlock = (physx::PxU32)funcMaxThreadsPerBlock;

			fid.mNumRegsPerThread = (physx::PxU32)funcNumRegsPerThread;
			fid.mStaticSharedSize = (physx::PxU32)funcSharedMemSize;
		...
		mFuncInstCount = physx::PxMax(mFuncInstCount, PxU32(funcInstIndex) + 1);
	}
	...
	PX_INLINE PxU32 getProfileId() const
	{
		return mProfileId;
	}
...
protected:
	...
	struct FuncInstData
	{
		...
		PxU32		mMaxThreadsPerBlock;
		PxU32		mNumRegsPerThread;
		PxU32		mStaticSharedSize;

		PxU32		mWarpsPerBlock;
		PxU32		mDynamicShared;
	};

	PxU32			mFuncInstCount;
	...
	virtual void init(physx::PxCudaContextManager* , int /*funcInstIndex*/) {}
	...
	void setParam(ApexCudaFuncParams& params, T* ptr)
	{
		...
		physx::PxMemCopy(params.mParams + params.mOffset, &ptr, sizeof(ptr));
	...
	void setParam(ApexCudaFuncParams& params, const T& val)
	{
		...
		physx::PxMemCopy(params.mParams + params.mOffset, (void*)&val, sizeof(val));
	...
	template <>
	void copyParam(const char* name, const ApexCudaMemRef<float>& memRef)
	{
		copyParam(name, memRef, 4);
	}

	template <>
	void copyParam(const char* name, const ApexCudaMemRef<float2>& memRef)
	{
		copyParam(name, memRef, 4);
	}

	template <>
	void copyParam(const char* name, const ApexCudaMemRef<float3>& memRef)
	{
		copyParam(name, memRef, 4);
	}

	template <>
	void copyParam(const char* name, const ApexCudaMemRef<float4>& memRef)
	{
		copyParam(name, memRef, 4);
	}

	template <>
	void copyParam(const char* name, const ApexCudaMemRef<double>& memRef)
	{
...
private:
	template <typename T>
	void copyParam(const char* name, const ApexCudaMemRef<T>& memRef, PxU32 fpType)
	{
	...
	void setParam(ApexCudaFuncParams& params, unsigned align, unsigned size, void* ptr)
	{
		...
		physx::PxMemCopy(params.mParams + params.mOffset, ptr, (physx::PxU32)size);
		params.mOffset += size;
	}	
	friend class ApexCudaTestKernelContextReader;
};
