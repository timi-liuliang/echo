...
public:
	...
	PX_CUDA_CALLABLE static PxVec4 eval4D(float x, float y, float z, float w, int seed)
	{
		// The skewing and unskewing factors are hairy again for the 4D case
		const float F4 = (PxSqrt(5.0f) - 1.0f) / 4.0f;
		const float G4 = (5.0f - PxSqrt(5.0f)) / 20.0f;
