...
// RAII wrapper around the NxApexScene::startWrite() method, note that this
...
private:
	...
	#define NX_WRITE_ZONE() WriteCheck __writeCheck(static_cast<ApexRWLockable*>(this), __FUNCTION__);

	// Creates a scoped write check object that disallows re-entrant writes, this is used by
	// the NxApexScene::simulate method to detect when callbacks make write calls to the API
	#define NX_WRITE_ZONE_NOREENTRY() WriteCheck __writeCheck(static_cast<ApexRWLockable*>(this), __FUNCTION__, false);
#else
	#define NX_WRITE_ZONE()
	#define NX_WRITE_ZONE_NOREENTRY()
#endif
