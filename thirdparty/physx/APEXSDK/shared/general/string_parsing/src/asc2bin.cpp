...
static inline PxU8 GetHEX1(const char *foo,const char **endptr)
{
	...
	ret = physx::PxU32((GetHex(PxU8(foo[0]))<<4) | GetHex(PxU8(foo[1])));
...
static inline unsigned short GetHEX2(const char *foo,const char **endptr)
{
	...
	ret = PxU32((GetHex(PxU8(foo[0]))<<12) | (GetHex(PxU8(foo[1]))<<8) | (GetHex(PxU8(foo[2]))<<4) | GetHex(PxU8(foo[3])));
...
static inline PxU32 GetHEX4(const char *foo,const char **endptr)
{
	...
	for (PxI32 i=0; i<8; i++)
	{
		ret = physx::PxU32((ret<<4) | GetHex(PxU8(foo[i])));
	}
