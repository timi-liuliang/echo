...
PxU8 GetHEX1(const char *foo,const char **endptr)
{
	...
	ret = (PxU32)GetHex(PxU8(foo[0]<<4)) | GetHex(PxU8(foo[1]));
...
PxU16 GetHEX2(const char *foo,const char **endptr)
{
	...
	ret = (GetHex(PxU8(foo[0]))<<12) | (GetHex(PxU8(foo[1]))<<8) | (GetHex(PxU8(foo[2]))<<4) | GetHex(PxU8(foo[3]));
...
bool CharToWide(const char *source,wchar_t *dest,PxI32 maxlen)
{
	...
	ret = true;
	mbstowcs(dest, source, (physx::PxU32)maxlen );
...
bool WideToChar(const wchar_t *source,char *dest,PxI32 maxlen)
{
	...
	ret = true;
	wcstombs(dest, source, (physx::PxU32)maxlen );
...
#if defined (LINUX_GENERIC) || defined(LINUX) || defined(__CELLOS_LV2__) || defined(__APPLE__) || defined(ANDROID) || defined(PX_PS4) || defined(PX_LINUX)
