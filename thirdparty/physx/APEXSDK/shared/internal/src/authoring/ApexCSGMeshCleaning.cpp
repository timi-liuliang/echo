...
{
	return (physx::PxU32)((point|n0) + d0 >= (Real)0) | ((physx::PxU32)((point|n1) + d1 >= (Real)0)) << 1 | ((physx::PxU32)((point|n2) + d2 >= (Real)0)) << 2;
}

PX_INLINE bool
triangulate(LinkedEdge2D* loop, physx::Array<Vec2Real>& planeVertices, physx::Pool<LinkedEdge2D>& linkedEdgePool)
{
	// Trivial cases
...
static bool
mergeTriangles2D(physx::Array<Vec2Real>& planeVertices, physx::Pool<LinkedEdge2D>& linkedEdgePool, Real distanceTol)
{
...
static void
mergeTriangles(physx::Array<physx::NxExplicitRenderTriangle>& cleanedMesh, const Triangle* triangles, const Interpolator* frames, ClippedTriangleInfo* info, physx::PxU32 triangleCount,
               const physx::Array<Triangle>& originalTriangles, const Plane& plane, physx::Pool<LinkedEdge2D>& linkedEdgePool, Real distanceTol, const Mat4Real& BSPToMeshTM)
{
	if (triangleCount == 0)
	{
		return;
	...
	physx::createIndexStartLookup(originalTriangleGroupStarts, (physx::PxI32)originalTriangleIndexStart, originalTriangleIndexCount,
	...
	physx::NxIndexBank<physx::PxU32> frameIndices;
...
//		const Plane plane(seedOriginalTri.normal, (Real)0.333333333333333333333 * (seedOriginalTri.vertices[0] + seedOriginalTri.vertices[1] + seedOriginalTri.vertices[2]));
		...
		const physx::PxU32 maxDir = physx::PxAbs(zAxis[0]) > physx::PxAbs(zAxis[1]) ?
		                            (physx::PxAbs(zAxis[0]) > physx::PxAbs(zAxis[2]) ? 0u : 2u) :
			                        (physx::PxAbs(zAxis[1]) > physx::PxAbs(zAxis[2]) ? 1u : 2u);
		Dir xAxis((Real)0);
		xAxis[(physx::PxI32)(maxDir + 1) % 3] = (Real)1;
		...
		for (physx::PxU32 i = originalTriangleGroupStarts[seedFrameIndex]; i < originalTriangleGroupStarts[seedFrameIndex + 1]; ++i)
		{
			...
			const physx::PxU32 ccw = (physx::PxU32)((triangle.normal | zAxis) > 0);
		...
		if (success && planeVertices.size() < oldPlaneVertexCount)
		{
			...
			const physx::PxU32 ccw = (physx::PxU32)(signedArea >= 0);
			const Real sign = ccw ? (Real)1 : (Real) - 1;
			const physx::PxU32 vMap[3] = { 0, 2 - ccw, 1 + ccw };
			const Pos planeOffset = Pos((Real)0) + (-plane.d()) * zAxis;
			for (physx::PxU32 i = 0; i < planeVertices.size(); i += 3)
			{
				physx::NxExplicitRenderTriangle& cleanedTri = cleanedMesh.insert();
		...
		else
		{
			...
			for (physx::PxU32 i = 0; i < mergedFrameIndices.size(); ++i)
			{
				for (physx::PxU32 j = originalTriangleGroupStarts[mergedFrameIndices[i]]; j < originalTriangleGroupStarts[mergedFrameIndices[i] + 1]; ++j)
				{
					...
					tri.transform(BSPToMeshTM);
					physx::NxExplicitRenderTriangle& cleanedMeshTri = cleanedMesh.insert();
					VertexData vertexData[3];
					for (int v = 0; v < 3; ++v)
					{
...
cleanMesh(physx::Array<physx::NxExplicitRenderTriangle>& cleanedMesh, const physx::Array<Triangle>& mesh, physx::Array<ClippedTriangleInfo>& triangleInfo, const physx::Array<Plane>& planes, const physx::Array<Triangle>& originalTriangles, const physx::Array<Interpolator>& frames, Real distanceTol, const Mat4Real& BSPToMeshTM)
{
	...
	physx::createIndexStartLookup(planeGroupStarts, 0, planes.size(), (physx::PxI32*)&triangleInfo.begin()->planeIndex, triangleInfo.size(), sizeof(ClippedTriangleInfo));

	physx::Pool<LinkedEdge2D> linkedEdgePool;
