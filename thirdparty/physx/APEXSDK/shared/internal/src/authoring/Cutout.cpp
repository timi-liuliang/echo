...
PX_INLINE physx::PxU32 mod(physx::PxI32 n, physx::PxU32 modulus)
{
	...
	const physx::PxI32 m = n - d*(physx::PxI32)modulus;
	return m >= 0 ? (physx::PxU32)m : (physx::PxU32)m + modulus;
}
...
public:
	...
	{
		x = (physx::PxI32)mod(x+(physx::PxI32)mOriginX, mWidth);
		y = (physx::PxI32)mod(y+(physx::PxI32)mOriginY, mHeight);
		return mMem[(physx::PxU32)(x + y * (physx::PxI32)mWidth)];
	}
...
PX_INLINE physx::PxF32 compareTraceSegmentToLineSegment(const physx::Array<POINT2D>& trace, int _start, int delta, physx::PxF32 distThreshold, physx::PxU32 width, physx::PxU32 height, bool hasBorder)
{
	if (delta < 2)
	{
		return 0.0f;
	...
	const physx::PxU32 size = trace.size();

	physx::PxU32 start = (physx::PxU32)_start, end = (physx::PxU32)(_start + delta) % size;
...
static void createCutout(physx::Cutout& cutout, const physx::Array<POINT2D>& trace, physx::PxF32 snapThreshold, physx::PxU32 width, physx::PxU32 height, bool hasBorder)
{
	...
	for (physx::PxU32 iStart = 0; iStart < size; ++iStart)
	{
		...
		for (; iDelta > 1; --iDelta)
		{
			physx::PxF32 fit = compareTraceSegmentToLineSegment(trace, (physx::PxI32)iStart, (physx::PxI32)iDelta, CUTOUT_DISTANCE_THRESHOLD, width, height, hasBorder);
	...
	while ((size -= delta) > 0)
	{
		...
		{
			physx::PxF32 fit = compareTraceSegmentToLineSegment(trace, (physx::PxI32)start, (physx::PxI32)delta, CUTOUT_DISTANCE_THRESHOLD, width, height, hasBorder);
			if (fit < errorThreshold)
			{
				break;
			}
...
static void splitTJunctions(physx::CutoutSet& cutoutSet, physx::PxF32 threshold)
{
	...
	for (physx::PxU32 i = 0; i < cutoutSet.cutouts.size(); ++i)
	{
		...
		for (physx::PxU32 j = 0; j < cutoutSize; ++j)
		{
			...
			cutoutMap[edgeCount].set((physx::PxI32)i, (physx::PxI32)j);
			++edgeCount;
		}
	}
	...
	for (physx::PxU32 overlapIndex = 0; overlapIndex < overlaps.size(); ++overlapIndex)
	{
		...
		const CutoutVert& seg0Map = cutoutMap[(physx::PxU32)mapPair.i0];
		const CutoutVert& seg1Map = cutoutMap[(physx::PxU32)mapPair.i1];
		...
		const physx::Cutout& cutout0 = cutoutSet.cutouts[(physx::PxU32)seg0Map.cutoutIndex];
		const physx::PxU32 cutoutSize0 = cutout0.vertices.size();
		const physx::Cutout& cutout1 = cutoutSet.cutouts[(physx::PxU32)seg1Map.cutoutIndex];
		const physx::PxU32 cutoutSize1 = cutout1.vertices.size();

		if (projectOntoSegmentXY(newVertex.edgeProj, dist2, cutout0.vertices[(physx::PxU32)seg0Map.vertIndex], cutout1.vertices[(physx::PxU32)seg1Map.vertIndex], 
			cutout1.vertices[(physx::PxU32)(seg1Map.vertIndex + 1) % cutoutSize1], 0.25f))
		{
		...
		if (projectOntoSegmentXY(newVertex.edgeProj, dist2, cutout1.vertices[(physx::PxU32)seg1Map.vertIndex], cutout0.vertices[(physx::PxU32)seg0Map.vertIndex], 
			cutout0.vertices[(physx::PxU32)(seg0Map.vertIndex + 1) % cutoutSize0], 0.25f))
		{
			if (dist2 <= distThreshold2)
			{
	...
	if (newVertices.size())
	{
		...
		for (physx::PxU32 newVertexIndex = newVertices.size(); newVertexIndex--;)
		{
			...
			{
				lastCutoutIndex = (physx::PxU32)newVertex.vertex.cutoutIndex;
				lastVertexIndex = 0xFFFFFFFF;
			}
			if (newVertex.vertex.vertIndex != (physx::PxI32)lastVertexIndex)
			{
				lastVertexIndex = (physx::PxU32)newVertex.vertex.vertIndex;
				lastProj = 1.0f;
			}
			physx::Cutout& cutout = cutoutSet.cutouts[(physx::PxU32)newVertex.vertex.cutoutIndex];
			const physx::PxF32 proj = lastProj > 0.0f ? newVertex.edgeProj / lastProj : 0.0f;
			const physx::PxVec3 pos = (1.0f - proj) * cutout.vertices[(physx::PxU32)newVertex.vertex.vertIndex] 
				+ proj * cutout.vertices[(physx::PxU32)(newVertex.vertex.vertIndex + 1) % cutout.vertices.size()];
			...
			cutout.vertices[(physx::PxU32)newVertex.vertex.vertIndex + 1] = pos;
...
static void mergeVertices(physx::CutoutSet& cutoutSet, physx::PxF32 threshold, physx::PxU32 width, physx::PxU32 height)
{
	...
	for (physx::PxU32 i = 0; i < cutoutSet.cutouts.size(); ++i)
	{
		...
		for (physx::PxU32 j = 0; j < cutout.vertices.size(); ++j)
		{
			...
			cutoutMap[vertexCount].set((physx::PxI32)i, (physx::PxI32)j);
			++vertexCount;
		}
	}
	...
	for (physx::PxU32 i = 0; i < vertexCount; ++i)
	{
		...
		const CutoutVert& cutoutVert0 = cutoutMap[(physx::PxU32)overlaps[start].i0];
		const physx::PxVec3& vert0 = cutoutSet.cutouts[(physx::PxU32)cutoutVert0.cutoutIndex].vertices[(physx::PxU32)cutoutVert0.vertIndex];
		const bool isOnBorder0 = !cutoutSet.periodic && isOnBorder(vert0, width, height);
		for (physx::PxU32 j = start; j < stop; ++j)
		{
			const CutoutVert& cutoutVert1 = cutoutMap[(physx::PxU32)overlaps[j].i1];
			if (cutoutVert0.cutoutIndex == cutoutVert1.cutoutIndex)
			{
				// No pairs from the same cutout
				continue;
			}
			const physx::PxVec3& vert1 = cutoutSet.cutouts[(physx::PxU32)cutoutVert1.cutoutIndex].vertices[(physx::PxU32)cutoutVert1.vertIndex];
	...
	for (physx::PxU32 i = 0; i < vertexCount; ++i)
	{
		...
		const CutoutVert& cutoutVert0 = cutoutMap[(physx::PxU32)pairs[start].i0];
		const physx::PxVec3& vert0 = cutoutSet.cutouts[(physx::PxU32)cutoutVert0.cutoutIndex].vertices[(physx::PxU32)cutoutVert0.vertIndex];
		physx::PxU32 groupStart = start;
		while (groupStart < stop)
		{
			physx::PxU32 next = groupStart;
			const CutoutVert& cutoutVert1 = cutoutMap[(physx::PxU32)pairs[next].i1];
			physx::PxI32 currentOtherCutoutIndex = cutoutVert1.cutoutIndex;
			const physx::PxVec3& vert1 = cutoutSet.cutouts[(physx::PxU32)currentOtherCutoutIndex].vertices[(physx::PxU32)cutoutVert1.vertIndex];
			physx::PxU32 keep = groupStart;
			physx::PxF32 minDist2 = (vert0 - vert1).magnitudeSquared();
			while (++next < stop)
			{
				const CutoutVert& cutoutVertNext = cutoutMap[(physx::PxU32)pairs[next].i1];
				if (currentOtherCutoutIndex != cutoutVertNext.cutoutIndex)
				{
					break;
				}
				const physx::PxVec3& vertNext = cutoutSet.cutouts[(physx::PxU32)cutoutVertNext.cutoutIndex].vertices[(physx::PxU32)cutoutVertNext.vertIndex];
	...
	{
		const physx::PxU32 i0 = (physx::PxU32)pairs[i].i0;
		...
		physx::PxVec3& vert0 = cutoutSet.cutouts[(physx::PxU32)cutoutVert0.cutoutIndex].vertices[(physx::PxU32)cutoutVert0.vertIndex];
		const physx::PxU32 i1 = (physx::PxU32)pairs[i].i1;
		bool& pinned1 = pinned[i1];
		const CutoutVert& cutoutVert1 = cutoutMap[i1];
		physx::PxVec3& vert1 = cutoutSet.cutouts[(physx::PxU32)cutoutVert1.cutoutIndex].vertices[(physx::PxU32)cutoutVert1.vertIndex];
...
static void createCutoutSet(physx::CutoutSet& cutoutSet, const physx::PxU8* pixelBuffer, physx::PxU32 bufferWidth, physx::PxU32 bufferHeight, physx::PxF32 snapThreshold, bool periodic)
{
	...
	map.setOrigin((physx::PxU32)originCoord, (physx::PxU32)originCoord);
	...
	for (physx::PxU32 y = 0; y < bufferHeight; ++y)
	{
		for (physx::PxU32 x = 0; x < bufferWidth; ++x)
		{
			...
			if ((pix >> 28) != 0)
			{
				map.set((physx::PxI32)x, (physx::PxI32)y);
			}
		}
	}
	...
	if (!periodic)
	{
		...
		{
			map.set(x, -1);
			map.set(x, (physx::PxI32)bufferHeight);
		}
		for (physx::PxI32 y = -1; y <= (physx::PxI32)bufferHeight; ++y)
		{
			map.set(-1, y);
			map.set((physx::PxI32)bufferWidth, y);
		}
	}
	...
	regions.setOrigin((physx::PxU32)originCoord, (physx::PxU32)originCoord);

	// Create a path counting map
	Map2d<physx::PxU8> pathCounts(bufferWidth + borderPad, bufferHeight + borderPad, 0);
	pathCounts.setOrigin((physx::PxU32)originCoord, (physx::PxU32)originCoord);
	...
	if (!periodic)
	{
		...
		{
			pathCounts(x, -1) = 1;
			pathCounts(x, (physx::PxI32)bufferHeight) = 1;
		}
		for (physx::PxI32 y = -1; y <= (physx::PxI32)bufferHeight; ++y)
		{
			pathCounts(-1, y) = 1;
			pathCounts((physx::PxI32)bufferWidth, y) = 1;
		}
	}
...
static bool calculateUVMapping(physx::NxExplicitHierarchicalMesh& theHMesh, const physx::PxVec3& theDir, physx::PxMat33& theResultMapping)
{	
