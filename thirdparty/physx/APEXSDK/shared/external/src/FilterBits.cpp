...
public:
...
#ifdef PX_X64
		ew64.matchBits64 = (physx::PxU64)this;
#else
		ew32.word2 = (physx::PxU32)this;
		ew32.word3 = 0;
#endif
	}

	float computeWeight(physx::PxU64 mask,float weightIn) const
	{
		float w = weightIn;
		physx::PxU32 index=0;
		while ( mask )
	...
	bool allWeightOne(void) const // return true if the weights of all mask bits are one..
	{
		...
		physx::PxU32 index=0;
		physx::PxU64 mask = ew64.matchBits64;
		while ( mask )
		{
			if ( mask &1 )
			{
...
public:
	...
	{
		gFilterBits = NULL;
		for (physx::PxU32 i=0; i<mFilterDataSpecCount; i++)
		{
			FilterDataSpec *spec = mFilterDataTypes[i];
			delete spec;
		}
		for (physx::PxU32 i=0; i<mTypesCount; i++)
		{
			char *str = mTypes[i];
			free(str);
		}
	...
	virtual const char *getFilterString(physx::PxU64 bits) const
	{
		mTemp[0] = 0;
	...
	virtual const char *getTypeString(physx::PxU8 type) const
	{
		const char *ret = NULL;
	...
	virtual physx::PxU8 getTypeCount(void) const
	{
		return (physx::PxU8) mTypesCount;
	}
	...
	virtual physx::PxU64 getTypeBit(const char *str,physx::PxU32 &index,bool &applyDefault) 
	{
		physx::PxU64 ret = 0;
	...
	virtual bool getWeightedFilter(const EncodedFilterData &o1,const EncodedFilterData &o2,float &weight) 
	{
		...
		physx::PxU64 mask1 = d1.ew64.typeBits64 & d2.ew64.matchBits64;
		physx::PxU64 mask2 = d2.ew64.typeBits64 & d1.ew64.matchBits64;
	...
	physx::PxU32				mTypesCount;
	char					*mTypes[MAX_TYPES];
	physx::PxU32				mFilterDataSpecCount;
