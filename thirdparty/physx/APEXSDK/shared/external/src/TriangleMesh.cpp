                                 ...
                                 bool dummyMaterial, const char* materialPrefix, const char* materialSuffix, bool onlyVisibleMaterials)
{
	...
	const unsigned int maxBoneIndexMesh = physx::PxU32(mParent != NULL ? mParent->mMaxBoneIndexInternal : mMaxBoneIndexInternal);
...
void TriangleMesh::copyFromSubMesh(const TriangleMesh& mesh, int subMeshNr, bool filterVertices)
{
	...
	if (subMeshNr >= 0)
	{
		const TriangleSubMesh& sm = mesh.mSubMeshes[(physx::PxU32)subMeshNr];
		firstIndex = (physx::PxI32)sm.firstIndex;
		numIndices = (physx::PxI32)sm.numIndices;
	}
	...
	for (int i = firstIndex; i < firstIndex + numIndices; i += 3)
	{
		...
		if (filter)
		{
			for (int j = 0; j < 3; j++)
			{
				skipTri |= (physChannel[mesh.mIndices[physx::PxU32(i + j)]].paintValueU32 != 0);
			}
		}
		if (skipTri)
		{
		...
		for (int j = 0; j < 3; j++)
		{
			physx::PxU32 idx = mesh.mIndices[physx::PxU32(i + j)];
			if (oldToNew[idx] >= 0)
			{
				mIndices.push_back((physx::PxU32)oldToNew[idx]);
			}
			else
			{
				mIndices.push_back((physx::PxU32)nextIndex);
	...
	mVertices.resize((physx::PxU32)numNewVerts);
	for (size_t i = 0; i < numVerts; i++)
	{
		if (oldToNew[i] >= 0)
		{
			mVertices[(physx::PxU32)oldToNew[i]] = mesh.mVertices[i];
		}
	}

	if (mesh.mNormals.size() == numVerts)
	{
		mNormals.resize((physx::PxU32)numNewVerts);
		for (size_t i = 0; i < numVerts; i++)
		{
			if (oldToNew[i] >= 0)
			{
				mNormals[(physx::PxU32)oldToNew[i]] = mesh.mNormals[i];
			}
		}
	}
	...
	{
		mTangents.resize((physx::PxU32)numNewVerts);
		for (size_t i = 0; i < numVerts; i++)
		{
			if (oldToNew[i] >= 0)
			{
				mTangents[(physx::PxU32)oldToNew[i]] = mesh.mTangents[i];
			}
		}
	}
	...
	{
		mBitangents.resize((physx::PxU32)numNewVerts);
		for (size_t i = 0; i < numVerts; i++)
		{
			if (oldToNew[i] >= 0)
			{
				mBitangents[(physx::PxU32)oldToNew[i]] = mesh.mBitangents[i];
			}
		}
	}
	...
	for (unsigned int t = 0; t < NUM_TEXCOORDS; t++)
	{
		...
		{
			mTexCoords[t].resize((physx::PxU32)numNewVerts);

			for (size_t j = 0; j < numVerts; j++)
			{
				if (oldToNew[j] >= 0)
				{
					mTexCoords[t][(physx::PxU32)oldToNew[j]] = mesh.mTexCoords[t][j];
				}
			}
		}
	}
	...
	for (int i = 0; i < PC_NUM_CHANNELS; i++)
	{
		...
		{
			mPaintChannels[i].resize((physx::PxU32)numNewVerts);
			for (size_t j = 0; j < numVerts; j++)
			{
				if (oldToNew[j] >= 0)
				{
					mPaintChannels[i][(physx::PxU32)oldToNew[j]] = mesh.mPaintChannels[i][j];
				}
			}
		}
	}
	...
	{
		mBoneIndicesExternal.resize((physx::PxU32)numNewVerts * 4);
		mBoneWeights.resize((physx::PxU32)numNewVerts);
		...
		for (size_t i = 0; i < numVerts; i++)
		{
			if (oldToNew[i] >= 0)
			{
				...
				for (int j = 0; j < 4; j++)
				{
					mBoneIndicesExternal[(physx::PxU32)newIndex * 4 + j] = mesh.mBoneIndicesExternal[i * 4 + j];
				}
				mBoneWeights[(physx::PxU32)newIndex] = mesh.mBoneWeights[i];
			}
		}
	}
	mSubMeshes[0].firstIndex = 0;
...
bool TriangleMesh::loadFromObjFile(const std::string& filename, bool useCustomChannels)
{
	...
	{
		const physx::PxVec3 p0 = vertices[(physx::PxU32)refs[i + 0].vert];
		const physx::PxVec3 p1 = vertices[(physx::PxU32)refs[i + 1].vert];
		const physx::PxVec3 p2 = vertices[(physx::PxU32)refs[i + 2].vert];
		...
		if (refs[i + 0].texCoord >= 0)
		{
			...
			const physx::NxVertexUV w0 = texCoords[(physx::PxU32)refs[i + 0].texCoord];
			const physx::NxVertexUV w1 = texCoords[(physx::PxU32)refs[i + 1].texCoord];
			const physx::NxVertexUV w2 = texCoords[(physx::PxU32)refs[i + 2].texCoord];
	...
	while (readRefs < refs.size())
	{
		...
		mVertices.push_back(vertices[(physx::PxU32)r.vert]);

		if (r.normal >= 0)
		{
			mNormals.push_back(normals[(physx::PxU32)r.normal]);
		}
		else
		{
		...
		{
			mTexCoords[0].push_back(texCoords[(physx::PxU32)r.texCoord]);
		}
		else
		{
		...
		mIndices[(physx::PxU32)r.indexNr] = (physx::PxU32)vertNr;
		readRefs++;
...
bool TriangleMesh::saveToObjFile(const std::string& filename) const
{
	...
	for (physx::PxU32 i = 0; i < (physx::PxU32)numTex; i++)
	{
...
//	coeffsOK = false; // no more coeffs
	...
	if (coeffsOK)
	{
		...
		for (physx::PxU32 i = 0; i < mVertices.size(); i++)
		{
			...
			for (physx::PxU32 j = 0; j < PC_NUM_CHANNELS; j++)
			{
	...
	if (mBoneWeights.size() > 0)
	{
		...
		for (physx::PxU32 i = 0; i < (physx::PxU32)mBoneWeights.size(); i++)
		{
...
bool TriangleMesh::loadFromMeshImport(mimp::MeshSystemContainer* msc, bool useCustomChannels)
{
	...
	for (unsigned int k = 0; k < ms->mMeshCount; k++)
	{
		...
		unsigned int base_index = (physx::PxU32)mVertices.size();
...
void TriangleMesh::initFrom(physx::apex::NxRenderMeshAssetAuthoring& mesh, bool initCustomChannels)
{
	...
	for (physx::PxU32 i = 0; i < mesh.getSubmeshCount(); i++)
	{
		...
		switch(format.getBufferFormat((physx::PxU32)boneIndexIndex))
		{
	...
	for (physx::PxU32 submeshIndex = 0; submeshIndex < mesh.getSubmeshCount(); submeshIndex++)
	{
...

		{
			...
			const physx::PxVec3* positions = (const physx::PxVec3*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
...

		{
			...
			const physx::PxVec3* normals = (const physx::PxVec3*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
		...
		if (numBonesPerVertex > 0)
		{
			...
			const float* boneWeights = (const float*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
			bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(physx::apex::NxRenderVertexSemantic::BONE_INDEX));
			const short* boneIndices = (const short*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
		...
		if (hasTexCoords)
		{
			...
			const physx::NxVertexUV* texCoords = (const physx::NxVertexUV*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
		...
		if (hasTangents)
		{
			...
			const physx::PxVec3* tangents = (const physx::PxVec3*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
			...
			const physx::PxVec3* bitangents = (const physx::PxVec3*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
		...
		if (bufferIndex != -1)
		{
			const float* maxDistances = (const float*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
		...
		if (bufferIndex != -1)
		{
			const unsigned char* usedForPhysics = (const unsigned char*)vb.getBufferAndFormat(format, (physx::PxU32)bufferIndex);
...
void TriangleMesh::drawVertices(size_t boneNr, float minWeight, float pointScaling, physx::apex::NxApexRenderDebug* batcher) const
{
	...
	for (size_t i = 0; i < numVertices; i++)
	{
		...
		if (found)
		{
			...
			batcher->setCurrentColor(physx::PxU32(gray << 16 | gray << 8 | gray));
...
void TriangleMesh::skin(const SkeletalAnim& anim, float scale /* = 1.0f */)
{
	...
	const physx::PxU32 numSkinningMatricesNeeded = physx::PxMin<physx::PxU32>((physx::PxU32)mMaxBoneIndexInternal + 1, (physx::PxU32)skinningMatrices.size());
	...
	{
		mSkinningMatrices[i] = skinningMatrices[(physx::PxU32)boneMappingInt2Ext[i]] * scale;
	}
...
void TriangleMesh::subdivideSubMesh(int subMeshNr, int subdivision, bool evenOutVertexDegrees)
{
	...
	TriangleSubMesh& sm = mSubMeshes[(physx::PxU32)subMeshNr];

	// move submesh to the end so it can be expanded
	std::vector<physx::PxU32> tempIndices;
	tempIndices.resize(sm.numIndices);
	for (physx::PxU32 i = 0; i < sm.numIndices; i++)
	{
		tempIndices[i] = mIndices[sm.firstIndex + i];
	}

	physx::PxU32 numShift = (physx::PxU32)mIndices.size() - (sm.firstIndex + sm.numIndices);
	for (physx::PxU32 i = 0; i < numShift; i++)
	{
		mIndices[sm.firstIndex + i] = mIndices[sm.firstIndex + sm.numIndices + i];
	}

	physx::PxU32 last = (physx::PxU32)mIndices.size() - sm.numIndices;
	for (physx::PxU32 i = 0; i < sm.numIndices; i++)
	{
	...
	physx::PxU32 numTris = sm.numIndices / 3;
	int id[6];
	...
	{
		id[0] = (physx::PxI32)mIndices[sm.firstIndex + 3 * i + 0];
		id[1] = (physx::PxI32)mIndices[sm.firstIndex + 3 * i + 1];
		id[2] = (physx::PxI32)mIndices[sm.firstIndex + 3 * i + 2];
		physx::PxVec3 p0 = mVertices[(physx::PxU32)id[0]];
		physx::PxVec3 p1 = mVertices[(physx::PxU32)id[1]];
		physx::PxVec3 p2 = mVertices[(physx::PxU32)id[2]];
		...
		mIndices[sm.firstIndex + 3 * i + 0] = (physx::PxU32)id[(physx::PxU32)*newId++];
		mIndices[sm.firstIndex + 3 * i + 1] = (physx::PxU32)id[(physx::PxU32)*newId++];
		mIndices[sm.firstIndex + 3 * i + 2] = (physx::PxU32)id[(physx::PxU32)*newId++];
		...
		while (true)
		{
			...
			if (j >= 0)
			{
				mIndices.push_back((physx::PxU32)id[(physx::PxU32)j]);
			}
			else
			{
				break;
...
void TriangleMesh::evenOutVertexDegree(int subMeshNr, int numIters)
{
	...
	physx::PxU32 firstIndex = 0;
	physx::PxU32 lastIndex = (physx::PxU32)mIndices.size();
	if (subMeshNr >= 0)
	{
		firstIndex = mSubMeshes[(physx::PxU32)subMeshNr].firstIndex;
		lastIndex = firstIndex + mSubMeshes[(physx::PxU32)subMeshNr].numIndices;
	}
	physx::PxU32* indices = &mIndices[firstIndex];
	physx::PxU32 numTris = (lastIndex - firstIndex) / 3;
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		vertDegs[i1]++;
		vertDegs[i2]++;
		edge.init((physx::PxI32)i0, (physx::PxI32)i1, 0, (physx::PxI32)i);
		edges.push_back(edge);
		edge.init((physx::PxI32)i1, (physx::PxI32)i2, 1, (physx::PxI32)i);
		edges.push_back(edge);
		edge.init((physx::PxI32)i2, (physx::PxI32)i0, 2, (physx::PxI32)i);
		edges.push_back(edge);
	}
	...
	while (edgeNr < numEdges)
	{
		...
		if (j < numEdges && j == edgeNr + 2)  	// manifold edge
		{
			...
			neighbors[physx::PxU32(e0.triNr * 3 + e0.edgeNr)] = e1.triNr;
			neighbors[physx::PxU32(e1.triNr * 3 + e1.edgeNr)] = e0.triNr;
		}
		edgeNr = j;
	}
	...
	for (physx::PxU32 i = 0; i < numTris - 1; i++)
	{
		...
		physx::PxU32 j = i + random % (numTris - i);
		int p = permutation[i];
	...
	// flip edges
	for (physx::PxU32 iters = 0; iters < (physx::PxU32)numIters; iters++)
	{
		for (physx::PxU32 i = 0; i < numTris; i++)
		{
			int t = permutation[i];
			for (physx::PxU32 j = 0; j < 3; j++)
			{
				...
				physx::PxU32& i0 = indices[3 * (physx::PxU32)t + j];
				physx::PxU32& i1 = indices[3 * (physx::PxU32)t + (j + 1) % 3];
				physx::PxU32& i2 = indices[3 * (physx::PxU32)t + (j + 2) % 3];

				int backNr = -1;
				if (neighbors[3 * (physx::PxU32)n] == t)
				{
					backNr = 0;
				}
				else if (neighbors[3 * (physx::PxU32)n + 1] == t)
				{
					backNr = 1;
				}
				else if (neighbors[3 * (physx::PxU32)n + 2] == t)
				{
					backNr = 2;
				}
				PX_ASSERT(backNr >= 0);
				//physx::PxU32& j0 = indices[3 * n + backNr];
				physx::PxU32& j1 = indices[3 * (physx::PxU32)n + ((physx::PxU32)backNr + 1) % 3];
				physx::PxU32& j2 = indices[3 * (physx::PxU32)n + ((physx::PxU32)backNr + 2) % 3];
				...
				// update neighbors
				int& ni0 = neighbors[3 * (physx::PxU32)t + j];
				int& ni1 = neighbors[3 * (physx::PxU32)t + (j + 1) % 3];

				int& nj0 = neighbors[3 * (physx::PxU32)n + (physx::PxU32)backNr];
				int& nj1 = neighbors[3 * (physx::PxU32)n + ((physx::PxU32)backNr + 1) % 3];
				...
				if (ni0 >= 0)
				{
					if (neighbors[3 * (physx::PxU32)ni0] == n)
					{
						neighbors[3 * (physx::PxU32)ni0] = t;
					}
					else if (neighbors[3 * (physx::PxU32)ni0 + 1] == n)
					{
						neighbors[3 * (physx::PxU32)ni0 + 1] = t;
					}
					else if (neighbors[3 * (physx::PxU32)ni0 + 2] == n)
					{
						neighbors[3 * (physx::PxU32)ni0 + 2] = t;
					}
					else
					{
						PX_ALWAYS_ASSERT();
				...
				if (nj0 >= 0)
				{
					if (neighbors[3 * (physx::PxU32)nj0] == t)
					{
						neighbors[3 * (physx::PxU32)nj0] = n;
					}
					else if (neighbors[3 * (physx::PxU32)nj0 + 1] == t)
					{
						neighbors[3 * (physx::PxU32)nj0 + 1] = n;
					}
					else if (neighbors[3 * (physx::PxU32)nj0 + 2] == t)
					{
						neighbors[3 * (physx::PxU32)nj0 + 2] = n;
					}
					else
					{
						PX_ALWAYS_ASSERT();
...
void TriangleMesh::setCullMode(physx::NxRenderCullMode::Enum cullMode, physx::PxI32 submeshIndex)
{
	if (submeshIndex < 0)
	{
		for (physx::PxU32 i = 0; i < mSubMeshes.size(); i++)
		{
			setCullMode(cullMode, (physx::PxI32)i);
		}
	}
	else if ((physx::PxU32)submeshIndex < mSubMeshes.size())
	{
		if (cullMode != mSubMeshes[(physx::PxU32)submeshIndex].cullMode)
		{
			mSubMeshes[(physx::PxU32)submeshIndex].cullMode = cullMode;
...
bool TriangleMesh::processElement(const char* elementName, int argc, const char** argv, const char* /*elementData*/, int /*lineno*/)
{
	...
	else if (strcmp(elementName, "sharedgeometry") == 0)
	{
		...
		mVertices.resize(0);
		mVertices.reserve((physx::PxU32)vertexCount);
	}
	...
	else if (strcmp(elementName, "faces") == 0)
	{
		...
		mSubMeshes.back().numIndices = (physx::PxU32)faceCount * 3;
	}
	else if (strcmp(elementName, "face") == 0)
	{
		...
		mIndices.push_back((physx::PxU32)atoi(argv[1]));
		mIndices.push_back((physx::PxU32)atoi(argv[3]));
		mIndices.push_back((physx::PxU32)atoi(argv[5]));
	}
	...
	else if (strcmp(elementName, "vertexboneassignment") == 0)
	{
		...
		float* weights = &mBoneWeights[(physx::PxU32)vertexNr].x;
		physx::PxU16* indices = &mBoneIndicesExternal[(physx::PxU32)vertexNr * 4];
...
void TriangleMesh::updateNormals(int subMeshNr)
{
	...
	else
	{
		...
		const physx::PxU32 start = subMeshNr < 0 ? 0 : mSubMeshes[(physx::PxU32)subMeshNr].firstIndex;
		const physx::PxU32 end = subMeshNr < 0 ? (physx::PxU32)mIndices.size() : mSubMeshes[(physx::PxU32)subMeshNr].firstIndex + mSubMeshes[(physx::PxU32)subMeshNr].numIndices;
		for (physx::PxU32 i = start; i < end; i += 3)
		{
	...
	else
	{
		if (mBoneWeights.size() == mVertices.size())
		{
			...
			for (unsigned int i = 0; i < mVertices.size(); i++)
			{
				...
				newValue = physx::PxU32((newValue & ~(0x3 << offset)) | maxWeight << offset);
	...
	if (mBoneWeights.size() == mVertices.size() && mVertices.size() > 0)
	{
		...
		for (size_t v = 0; v < mVertices.size(); v++)
		{
			for (unsigned int k = 0; k < 4; k++)
			{
				if (mBoneWeights[v][k] > 0.0f)
				{
					...
					{
						ext2int[boneIndex] = (physx::PxI32)maxBones++;
					}
				}
			}
		}
		...
		for (size_t i = 0; i < ext2int.size(); i++)
		{
			if (ext2int[i] >= 0)
			{
				...
				mBoneMappingInt2Ext[(physx::PxU32)ext2int[i]] = (int)i;
			}
		}
	}
...
void TriangleMesh::complete(bool useCustomChannels)
{
	...
	else
	{
		...
		for (physx::PxU32 i = (physx::PxU32)mSubMeshes.size() - 1; i > 0; i--)
		{
...
int TriangleMesh::addSplitVert(int vertNr0, int vertNr1)
{
	...
	int i = mVertexFirstSplit[(physx::PxU32)v0];
	while (i >= 0)
	{
		TriangleEdgeSplit& s = mVertexSplits[(physx::PxU32)i];
		if (s.adjVertNr == v1)
		{
			return s.newVertNr;
		}
	...
	sNew.adjVertNr = v1;
	sNew.newVertNr = (physx::PxI32)mVertices.size();

	// create interpolated vertex
	physx::PxVec3 v = (mVertices[(physx::PxU32)v0] + mVertices[(physx::PxU32)v1]) * 0.5f;
	mVertices.push_back(v);
	if (mSkinnedVertices.size() == mVertices.size() - 1)
	{
		physx::PxVec3 sv = (mSkinnedVertices[(physx::PxU32)v0] + mSkinnedVertices[(physx::PxU32)v1]) * 0.5f;
	...
	for (physx::PxU32 j = 0; j < PC_NUM_CHANNELS; j++)
	{
		...
		{
			u = mPaintChannels[j][(physx::PxU32)v0].paintValueU32 | mPaintChannels[j][(physx::PxU32)v1].paintValueU32;    // bitwise OR
		}
		else if (j == PC_COLLISION_DISTANCE && (mPaintChannels[PC_COLLISION_DISTANCE][(physx::PxU32)v0].paintValueF32 == -1.1f || mPaintChannels[PC_COLLISION_DISTANCE][(physx::PxU32)v1].paintValueF32 == -1.1f))
		{
			v = -1.1f;
		}
		else
		{
			v = (mPaintChannels[j][(physx::PxU32)v0].paintValueF32 + mPaintChannels[j][(physx::PxU32)v1].paintValueF32) * 0.5f;
		}
	...
	for (physx::PxU32 j = 0; j < NUM_TEXCOORDS; j++)
	{
		...
		newTc.u = (mTexCoords[j][(physx::PxU32)v0].u + mTexCoords[j][(physx::PxU32)v1].u) * 0.5f;
		newTc.v = (mTexCoords[j][(physx::PxU32)v0].v + mTexCoords[j][(physx::PxU32)v1].v) * 0.5f;
	...
	if (!mBoneIndicesExternal.empty())
	{
		...
		for (physx::PxU32 k = 0; k < 2; k++)
		{
			physx::PxU32 v = physx::PxU32((k == 0) ? v0 : v1);
			for (physx::PxU32 j = 0; j < 4; j++)
			{
				...
				int k = 0;
				while (k < 4 && newIndices[(physx::PxU32)k] != bi)
				{
					k++;
				}
				if (k < 4)
				{
					newWeights[k] = (newWeights[(physx::PxU32)k] + bw) * 0.5f;
					continue;
				}
				// else insert the pair at the right place
				k = 3;
				while (k >= 0 && newWeights[(physx::PxU32)k] < bw)
				{
					if (k < 3)
					{
						newWeights[(physx::PxU32)k + 1] = newWeights[(physx::PxU32)k];
						newIndices[(physx::PxU32)k + 1] = newIndices[(physx::PxU32)k];
					}
					k--;
				}
				k++;
				if (k < 4)
				{
					newWeights[(physx::PxU32)k] = bw;
					newIndices[(physx::PxU32)k] = bi;
				}
			}
		}
	...
	sNew.next = mVertexFirstSplit[(physx::PxU32)v0];
	mVertexFirstSplit[(physx::PxU32)v0] = (physx::PxI32)mVertexSplits.size();
