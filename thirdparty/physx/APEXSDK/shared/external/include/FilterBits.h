...
public:
	...
	union
	{
		struct 
		{
			physx::PxU64	typeBits64;		// The set of bits which describe what 'type' or 'group' this corresponds with
			physx::PxU64	matchBits64;	// The set of bits describing which types or groups this object interacts with.
		} w64;
		struct 
		{
			physx::PxU32	word0;
...
public:
	...
	virtual const char *getFilterString(physx::PxU64 bits) const = 0;

	// Returns the string for a single bit type 0-63
	virtual const char *getTypeString(physx::PxU8 type) const = 0; // single type 0-63
	...
	virtual physx::PxU64 getTypeBit(const char *str,physx::PxU32 &index,bool &applyDefault) = 0;

	// Returns how many types were defined.
	virtual physx::PxU8 getTypeCount(void) const = 0;
