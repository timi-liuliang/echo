...
namespace NxParameterized
{
	...
	static physx::PxU32 ReadVersion(const physx::FastXml::AttributePairs& attr)
	{
	...
	bool XmlDeserializer::verifyObject(Interface *obj, const physx::FastXml::AttributePairs& attr)
	{
...
#	endif
		...
		physx::PxI32 /*lineno*/)
	{
		...
		if( strcmp(elementName, "NxParameters")  == 0 )
		{
			...
			physx::PxU32 ver = ReadVersion(attr);
			if( ver != mVer )
			{
			...
			if (numObjects != NULL)
			{
				const physx::PxU32 num = (physx::PxU32)atoi(numObjects);
				if (num > MAX_ROOT_OBJ)
				{
		...
		else if ( strcmp(elementName, "value") == 0 )
		{
			if( !mObjIndex ) //Root object?
			{
				...
				physx::PxU32 version = ReadVersion(attr);
		...
		else if ( strcmp(elementName, "array") == 0 )
		{
			...
			if ( arraySize > 0 )
			{
				...
				if( !handle.parameterDefinition()->arraySizeIsFixed() )
					if( NxParameterized::ERROR_NONE != handle.resizeArray((physx::PxI32)arraySize) )
					{
					...
					if ( elementData )
					{
						...
						if ( paramDef->type() == TYPE_STRUCT )
						{
							...
							physx::PxI32* simpleStructRedirect = getSimpleStructRedirect((physx::PxU32)paramDef->numChildren());
