...
{	
	...
	template <uint32_t> __m256 fmadd_ps(__m256 a, __m256 b, __m256 c)
	{
		return _mm256_add_ps(_mm256_mul_ps(a, b), c);
	}
	template <uint32_t> __m256 fnmadd_ps(__m256 a, __m256 b, __m256 c)
	{
		...
		const float* __restrict rEnd, const uint16_t* __restrict iIt, const __m128& stiffnessRef)
	{
		...
		for(; rIt < rEnd; rIt += 8, iIt += 16)
		{
			...
			if(useMultiplier)
			{
				erij = fnmadd_ps<avx>(multiplier,
					_mm256_max_ps(compressionLimit, _mm256_min_ps(erij, stretchLimit)), erij);
			}
...
forEnd:
	...
	template void solveConstraints<false, 1>(float* __restrict, const float* __restrict, 
		const float* __restrict, const uint16_t* __restrict, const __m128&);

	template void solveConstraints<true, 1>(float* __restrict, const float* __restrict, 
		const float* __restrict, const uint16_t* __restrict, const __m128&);

	template void solveConstraints<false, 2>(float* __restrict, const float* __restrict, 
		const float* __restrict, const uint16_t* __restrict, const __m128&);

	template void solveConstraints<true, 2>(float* __restrict, const float* __restrict, 
		const float* __restrict, const uint16_t* __restrict, const __m128&);
