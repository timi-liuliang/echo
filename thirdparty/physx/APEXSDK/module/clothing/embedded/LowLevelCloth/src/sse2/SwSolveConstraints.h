...
void solveConstraints(float* __restrict posIt, const float* __restrict rIt, 
	const float* __restrict rEnd, const uint16_t* __restrict iIt, __m128 stiffness)
{
	...
	for(; rIt != rEnd; rIt += 4, iIt += 8)
	{
		...
		__m128 e2ij = _mm_add_ps(sEpsilon, _mm_add_ps(_mm_mul_ps(hxij, hxij), _mm_add_ps(_mm_mul_ps(hyij, hyij), _mm_mul_ps(hzij, hzij))));
		__m128 mask = _mm_cmpnle_ps(rij, sEpsilon);
		__m128 erij = _mm_and_ps(_mm_sub_ps(sOne, _mm_mul_ps(rij, _mm_rsqrt_ps(e2ij))), mask);

		if(useMultiplier)
		{
			erij = _mm_sub_ps(erij, _mm_mul_ps(multiplier,
				_mm_max_ps(compressionLimit, _mm_min_ps(erij, stretchLimit))));
		}
...
template <>
void solveConstraints<false>(float* __restrict posIt, const float* __restrict rIt, 
	const float* __restrict rEnd, const uint16_t* __restrict iIt, __m128 stiffness)
{
	__m128 sOne = _mm_set1_ps(1.0f);
	stiffness = _mm_shuffle_ps(stiffness, stiffness, 0x00);
...
template <>
void solveConstraints<true>(float* __restrict posIt, const float* __restrict rIt, 
	const float* __restrict rEnd, const uint16_t* __restrict iIt, __m128 stiffness)
{
	__m128 sOne = _mm_set1_ps(1.0f);
