...
class CuFabric;
class CuCloth;
template <typename> class ClothImpl;
...
public:
	...
	virtual Solver* createSolver(physx::PxProfileZone* profiler, physx::PxTaskManager* taskMgr);
	...
	virtual void extractMotionConstraints(const Cloth& cloth, 
		Range<PxVec4> destConstraints) const;

	virtual void extractSeparationConstraints(const Cloth& cloth, 
		Range<PxVec4> destConstraints) const;

	virtual void extractParticleAccelerations(const Cloth& cloth, 
		Range<PxVec4> destAccelerations) const;
	...
	virtual void copyToHost(const void* srcIt, const void* srcEnd, void* dstIt) const;
	virtual void copyFromHost(const void* srcIt, const void* srcEnd, void* dstIt) const;

	virtual void copyToHost(const void* srcIt, const void* srcEnd, void* dstIt, CUstream) const;
	virtual void copyFromHost(const void* srcIt, const void* srcEnd, void* dstIt, CUstream) const;
