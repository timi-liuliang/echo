...
namespace
{
	...
	void generateSimd4f(Simd4f* dIt, const SrcIterator& src, uint32_t count)
	{
...
: mClothData(clothData), mAllocator(alloc), mProfiler(profiler)
{
	...
	if(mClothData.mEnableContinuousCollision || mClothData.mFrictionScale > 0.0f)
	{
		...
		generateSimd4f(reinterpret_cast<Simd4f*>(mPrevData.mSpheres), 
			reinterpret_cast<const Simd4f*>(
			clothData.mStartCollisionSpheres), clothData.mNumSpheres);
...
void cloth::SwCollision<Simd4f>::operator()(const IterationState<Simd4f>& state)
{
	...
	if (!lastIteration)
	{
		...
		generateSimd4f(reinterpret_cast<Simd4f*>(
...
PX_INLINE void calculateFrictionImpulse(
	...
	const Simd4f& scale, const Simd4f& coefficient, const Simd4f& mask, Simd4f* impulse)
{
...
void cloth::SwCollision<Simd4f>::collideConvexes(const IterationState<Simd4f>& state)
{
	...
	if (state.mRemainingIterations != 1)
	{
		...
		generateSimd4f(planes, planeIter, mClothData.mNumPlanes);
	}
	else
	{
		// otherwise use the target planes directly
		generateSimd4f(planes, targetPlanes, mClothData.mNumPlanes);
	}
