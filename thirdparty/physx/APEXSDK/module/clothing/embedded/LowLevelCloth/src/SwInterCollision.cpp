...
namespace
{
	...
	void radixSort(const uint32_t* first, const uint32_t* last, uint32_t* out)
	{
		...
		uint32_t* __restrict histograms[] = { buffer, 
...
cloth::SwInterCollision<Simd4f>::SwInterCollision(const cloth::SwInterCollisionData* instances, uint32_t n,
	float colDist, float stiffness, uint32_t iterations, InterCollisionFilter filter,
	cloth::SwKernelAllocator& alloc, physx::PxProfileZone* zone) 
: mInstances(instances), mNumInstances(n), mClothIndices(NULL), mParticleIndices(NULL),
	mNumParticles(0), mTotalParticles(0), mFilter(filter), mAllocator(alloc), mProfiler(zone)
{
	PX_ASSERT(mFilter);
...
namespace
{
	...
	struct ClothSorter
	{
		...
		ClothSorter(BoundingBox* bounds, uint32_t n, uint32_t axis) :
			mBounds(bounds), mNumBounds(n), mAxis(axis) {}
	...
	uint32_t calculatePotentialColliders(const cloth::SwInterCollisionData* cBegin, const cloth::SwInterCollisionData* cEnd, 
		Simd4f colDist, uint16_t* clothIndices, uint32_t* particleIndices, cloth::BoundingBox<Simd4f>& bounds, 
		uint32_t* overlapMasks, cloth::InterCollisionFilter filter, cloth::SwKernelAllocator& allocator)
	{
		...
		for (uint32_t i=0; i < numCloths; ++i)
		{
			...
			BoundingBox cBounds = { 
				(Simd4f)simd4f(cWorld.minimum.x, cWorld.minimum.y, cWorld.minimum.z, 0.0f),
...
void cloth::SwInterCollision<Simd4f>::operator()()
{
	...
	{		
		...
		{ 
			...
			mNumParticles = calculatePotentialColliders(mInstances, mInstances+mNumInstances,
				mCollisionDistance, mClothIndices, mParticleIndices, bounds, mOverlapMasks, mFilter, mAllocator);
		}
		...
		if (mNumParticles)
		{
			...
			uint32_t* __restrict keys = PxMax(sortedKeys + mNumParticles, 
					sortedIndices + 2*mNumParticles + 1024);
...
template <typename Simd4f>
void cloth::SwInterCollision<Simd4f>::collideParticles(const uint32_t* keys, 
	uint32_t firstColumnSize, const uint32_t* indices,
