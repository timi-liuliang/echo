...
cloth::TripletScheduler::TripletScheduler(Range<const uint32_t[4]> triplets)
	: mTriplets(reinterpret_cast<const Vec4u*>(triplets.begin()), 
				reinterpret_cast<const Vec4u*>(triplets.end()))
{
}

// SSE version
void cloth::TripletScheduler::simd( 
	uint32_t numParticles, uint32_t simdWidth)
{
	if(mTriplets.empty())
		return;
	...
	for(TripletIter tIt = mTriplets.begin(),
		tEnd = mTriplets.end(); tIt != tEnd; ++setIndex)
	{
...
// CUDA version
void cloth::TripletScheduler::warp( 
	uint32_t numParticles, uint32_t warpWidth )
{
	...
	*/
	...
	uint32_t maxTripletCount = *maxElement(
		adjacentCount.begin(), adjacentCount.end());
	...
	for(tIt = mTriplets.begin(), tripletIndex = 0; tIt != tEnd; ++tIt, ++tripletIndex)
	{
		...
		for(uint32_t setIndex = 0; setIndex < numSets && minReplays; ++setIndex)
		{
			...
			for(uint32_t i=0; i<3; ++i)
				numReplays += set.mNumReplays[i] ==
					set.mNumConflicts[i][warpMask & (*tIt)[i]];
		...
		if(++mSetSizes[bestIndex] & warpMask)
			for(uint32_t i=0; i<3; ++i) 
				set.mNumReplays[i] = PxMax(set.mNumReplays[i],
					++set.mNumConflicts[i][warpMask & (*tIt)[i]]);
		else
			set = TripletSet();
