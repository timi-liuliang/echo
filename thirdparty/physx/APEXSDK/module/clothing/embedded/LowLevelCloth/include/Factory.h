...
public:
	 ...
	 */
	virtual Solver* createSolver(physx::PxProfileZone* profiler, physx::PxTaskManager* taskMgr) = 0;
	...
	virtual void extractFabricData(const Fabric& fabric, 
		...
		Range<uint32_t> anchors, Range<float> tetherLengths) const = 0;
	...
	/**		
		...
		@param planes pre-allocated memory range to write planes
	 */
	virtual void extractCollisionData(const Cloth& cloth, Range<PxVec4> spheres, 
		Range<uint32_t> capsules, Range<PxVec4> planes, Range<uint32_t> convexes,
		Range<PxVec3> triangles) const = 0;
	...
	*/
	virtual void extractRestPositions(const Cloth& cloth, Range<PxVec4> restRestPositions) const = 0;

	/**
		Copy cloth data (potentially stored in device memory) to host.
	 */
	virtual void copyToHost(const void* srcIt, const void* srcEnd, void* dstIt) const = 0;

	/**
		Copy host data to cloth (potentially stored in device memory).
	 */
	virtual void copyFromHost(const void* srcIt, const void* srcEnd, void* dstIt) const = 0;

	/**
		Asynchronously copy cloth data (potentially stored in device memory) to host.
		\note Destination must be non-pageable for the copy to be asynchronous.
	 */
	virtual void copyToHost(const void* srcIt, const void* srcEnd, void* dstIt, CUstream) const = 0;

	/**
		Asynchronously copy host data to cloth (potentially stored in device memory).
		\note Source must be non-pageable for the copy to be asynchronous.
	 */
	virtual void copyFromHost(const void* srcIt, const void* srcEnd, void* dstIt, CUstream) const = 0;
