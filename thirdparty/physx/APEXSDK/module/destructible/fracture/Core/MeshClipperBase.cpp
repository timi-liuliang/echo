...
int MeshClipper::getNumMeshes()
{
	if (mOutMeshes.size() == 1)		// no islands
		...
		return (physx::PxI32)mOutMeshes.size()-1;		
}
...
const MeshClipper::Mesh& MeshClipper::getMesh(int meshNr)
{
	if (mOutMeshes.size() == 1)		// no islands
		...
		return mOutMeshes[1 + (physx::PxU32)meshNr];
}
...
void MeshClipper::init(const Convex *meshConvex)
{
	...
	for (physx::PxU32 i = 0; i < mMeshVertices.size(); i++) 
	...
	physx::PxU32 numPolys = meshPolyStarts.size()-1;
	...
	for (physx::PxU32 i = 0; i < numPolys; i++) {
		...
		for (int k = first; k < last; k++) 
			mPolyBounds[i].include(mMeshVertices[(physx::PxU32)meshPolyIndices[(physx::PxU32)k]]);
	}
...
{
	const physx::PxU32 minGridPolys = 1000;

	physx::PxU32 numPolys = mPolyBounds.size();
...
//	int targetNumCells = numPolys;		
	...
	physx::PxU32 numCells = physx::PxU32(mGridNumX * mGridNumY * mGridNumZ);
	...
	mGridPolys.resize((physx::PxU32)first);
		...
		for (int xi = x0; xi <= x1; xi++) {
			for (int yi = y0; yi <= y1; yi++) {
				for (int zi = z0; zi <= z1; zi++) {
					int nr = (xi * mGridNumY + yi) * mGridNumZ + zi;
					mGridFirstPoly[(physx::PxU32)nr]--;
					mGridPolys[(physx::PxU32)mGridFirstPoly[(physx::PxU32)nr]] = (physx::PxI32)i;
				}
			}
		}
	}
...
void MeshClipper::computeClipFaceNeighbors()
{
	...
	for (physx::PxU32 i = 0; i < faces.size(); i++) {
		...
		for (int j = 0; j < f.numIndices; j++) {
			...
			mEdges[(physx::PxU32)edgeNr].init(indices[physx::PxU32(f.firstIndex + j)], indices[physx::PxU32(f.firstIndex + (j+1)%f.numIndices)], edgeNr, (physx::PxI32)i);
		}
	}
	...
	while (i < mEdges.size()) {
		...
		if (i < mEdges.size() && mEdges[i] == e0) {
			Edge &e1 = mEdges[i];
			mClipFaceNeighbors[(physx::PxU32)e0.edgeNr] = e1.faceNr;
			mClipFaceNeighbors[(physx::PxU32)e1.edgeNr] = e0.faceNr;
			i++;
		}
...
bool MeshClipper::getGridCandidates(const PxBounds3 &bounds)
{
	...
	for (int xi = x0; xi <= x1; xi++) {
		for (int yi = y0; yi <= y1; yi++) {
			for (int zi = z0; zi <= z1; zi++) {
				int nr = (xi * mGridNumY + yi) * mGridNumZ + zi;
				int first = mGridFirstPoly[(physx::PxU32)nr];
				int last = mGridFirstPoly[(physx::PxU32)nr+1];
				for (int i = first; i < last; i++) {
					int polyNr = mGridPolys[(physx::PxU32)i];
					if (mGridPolyMarks[(physx::PxU32)polyNr] != mGridCurrentMark) {
						mGridPolyMarks[(physx::PxU32)polyNr] = mGridCurrentMark;
...
void MeshClipper::generateCandidates(const PxTransform &pxTrans)
{
	...
	clipBounds.setEmpty();
	for (physx::PxU32 i = 0; i < clipConvexVertices.size(); i++)
	...
	if (!getGridCandidates(clipBounds)) {
		...
		for (physx::PxU32 i = 0; i < numPolys; i++)
			mPolyCandidates[i] = (physx::PxI32)i;			
	}

	// use bounding boxes for further culling
	PxBounds3 polyBounds;
	physx::PxU32 numCandidates = 0;
	for (physx::PxU32 i = 0; i < mPolyCandidates.size(); i++) {
		int polyNr = mPolyCandidates[i];
		PxBounds3 &polyBounds = mPolyBounds[(physx::PxU32)polyNr];
	...
	for (physx::PxU32 i = 0; i < clipPlanes.size(); i++) {
		PxPlane plane = (clipPlanes[i]);
		plane.n = invTrans.rotate(plane.n);
		plane.d = plane.d + invTrans.p.dot(plane.n);

		physx::PxU32 numCandidates = 0;
		for (physx::PxU32 j = 0; j < mPolyCandidates.size(); j++) {
			physx::PxU32 polyNr = (physx::PxU32)mPolyCandidates[j];
			int first = meshPolyStarts[polyNr];
			int last = meshPolyStarts[polyNr+1];
			bool outside = true;
			for (int k = first; k < last; k++) {
				PxVec3 &p = mMeshVertices[(physx::PxU32)meshPolyIndices[(physx::PxU32)k]];
			...
			if (!outside) {
				mPolyCandidates[numCandidates] = (physx::PxI32)polyNr;
				numCandidates++;
			}
		}
		mPolyCandidates.resize(numCandidates);
...
bool MeshClipper::createInternalPolygons(const PxTransform &pxTrans)
{
	...
	physx::PxU32 current = 0;
	Mesh &out = mOutMeshes[0];

	for (physx::PxU32 i = 0; i < mPolyCandidates.size(); i++) {
		physx::PxU32 polyNr = (physx::PxU32)mPolyCandidates[i];
		...
		mClipPolyVerts[current].resize((physx::PxU32)polySize);

		if (mPolyInside[polyNr]) {		// fast path for polygons completely inside convex

			mFirstOutPoly[polyNr] = (physx::PxI32)out.polyStarts.size()-1;
			mNumOutPolys[polyNr] = 1;	

			for (int j = 0; j < polySize; j++) {
				physx::PxU32 id = (physx::PxU32)meshPolyIndices[physx::PxU32(first + j)];
				if (mOutVertNr[id] < 0) {
					mOutVertNr[id] = (physx::PxI32)out.vertices.size();
				...
				out.polyNeighbors.pushBack(meshPolyNeighbors[physx::PxU32(first + j)]);
			}
			out.polyStarts.pushBack((physx::PxI32)out.polyIndices.size());
			continue;
		}

		for (int j = 0; j < polySize; j++) {
			PolyVert &pv = mClipPolyVerts[current][(physx::PxU32)j];
			pv.init();
			pv.id = meshPolyIndices[physx::PxU32(first + j)];
			pv.pos = trans.transform(mMeshVertices[(physx::PxU32)pv.id]);
			pv.normal = trans.rotate((meshNormals[(physx::PxU32)pv.id]));
			pv.tangent = trans.rotate((meshTangents[(physx::PxU32)pv.id]));
			pv.u = meshTexCoords[2 * (physx::PxU32)pv.id];
			pv.v = meshTexCoords[2 * (physx::PxU32)pv.id + 1];
			pv.edgeNeighbor = meshPolyNeighbors[physx::PxU32(first + j)];
			pv.edgeOnFace = false;
		}
		...
		for (int j = 1; j < polySize-1; j++) {
			PxVec3 &p1 = mClipPolyVerts[current][(physx::PxU32)j].pos;
			PxVec3 &p2 = mClipPolyVerts[current][(physx::PxU32)j+1].pos;
			faceNormal += (p1-p0).cross(p2-p0);
		}
		faceNormal.normalize();

		// clip working poly against all planes
		for (physx::PxU32 j = 0; j < clipPlanes.size(); j++) {
			...
			physx::PxU32 numPrevPolys = mClipPolyStarts[prev].size()-1;
			if (numPrevPolys == 0)
				break;

			for (physx::PxU32 k = 0; k < numPrevPolys; k++) {
				...
				for (int l = 0; l < prevNum; l++) {
					PolyVert &pv0 = mClipPolyVerts[prev][physx::PxU32(prevFirst + l)];
					PolyVert &pv1 = mClipPolyVerts[prev][physx::PxU32(prevFirst + (l+1)%prevNum)];
					pv0.nextVert = -1;
					...
					if (sign1) {
						cut.edgeOnFace = true;
						cut.edgeNeighbor = (physx::PxI32)j;		// planeNr
					...
					pv0.nextVert = (physx::PxI32)mCuts.size();
					mCuts.pushBack(cut);
				}
				if (!anyIn)
					continue;
				...
				if (mCuts.empty()) {		
					for (int l = 0; l < prevNum; l++) {
						PolyVert &pv = mClipPolyVerts[prev][physx::PxU32(prevFirst + l)];
						mClipPolyVerts[current].pushBack(pv);
					}
					mClipPolyStarts[current].pushBack((physx::PxI32)mClipPolyVerts[current].size());
					continue;
				}

				// construct new polygon(s)
				for (physx::PxU32 l = 0; l < mCuts.size(); l++) 
				{
					...
					while ( true )
					{
						...
						mClipPolyVerts[current].pushBack(mClipPolyVerts[prev][(physx::PxU32)nr]);
						int cutNr = mClipPolyVerts[prev][(physx::PxU32)nr].nextVert;
						if (cutNr < 0) {
							nr = (nr+1) % prevNum;
							continue;
						}
						// we reached the next cut
						mClipPolyVerts[current].pushBack(mCuts[(physx::PxU32)cutNr]);

						// find the closest cut to the left
						PxF32 s0 = mCuts[(physx::PxU32)cutNr].pos.dot(cutNormal);
						PxF32 sMax = -FLT_MAX;
						PxF32 sMin = FLT_MAX;
						int minNr = -1;
						int maxNr = -1;
						for (int m = 0; m < (int)mCuts.size(); m++) {
							PxF32 s = mCuts[(physx::PxU32)m].pos.dot(cutNormal);
							if (s > s0 && s < sMin) {
								sMin = s;
								minNr = m;
							}
						...
						physx::PxU32 nextCutNr = physx::PxU32((minNr >= 0) ? minNr : maxNr);
		...
		mFirstOutPoly[polyNr] = (physx::PxI32)out.polyStarts.size()-1;
		mNumOutPolys[polyNr] = (physx::PxI32)mClipPolyStarts[current].size()-1;	
		...
		for (physx::PxU32 j = 0; j < mClipPolyStarts[current].size()-1; j++) {
			int first = mClipPolyStarts[current][j];
			int last  = mClipPolyStarts[current][j+1];
			int num = last - first;
			for (int k = 0; k < num; k++) {
				PolyVert &pv0 = mClipPolyVerts[current][physx::PxU32(first + k)];
				PolyVert &pv1 = mClipPolyVerts[current][physx::PxU32(first + (k+1)%num)];
				...
				if (!pv0.edgeOnFace && !pv1.edgeOnFace) {		// existing vertex, duplicate
					...
					if (mOutVertNr[(physx::PxU32)pv1.id] < 0) {
						mOutVertNr[(physx::PxU32)pv1.id] = (physx::PxI32)out.vertices.size();
					...
					id = mOutVertNr[(physx::PxU32)pv1.id];
					faceId = -1;
				}
				else if (pv0.edgeOnFace && pv1.edgeOnFace) {	// new, non shared
					id = (physx::PxI32)out.vertices.size();
					out.vertices.pushBack((pv1.pos));
					out.normals.pushBack((pv1.normal));
					out.tangents.pushBack((pv1.tangent));
					out.texCoords.pushBack((float)pv1.u);
					...
					int sharedId = -1;
					int numPolys = mNumOutPolys[(physx::PxU32)edgeNeighbor];

					// new neighbor exists, search shared vertex
					if (numPolys > 0) {		
						int firstPoly = mFirstOutPoly[(physx::PxU32)edgeNeighbor];
						for (int l = 0; l < numPolys; l++) {
							int newPolyNr = firstPoly + l;
							int first = out.polyStarts[(physx::PxU32)newPolyNr];
							int last  = out.polyStarts[(physx::PxU32)newPolyNr+1];
							for (int m = first; m < last; m++) {
								int testId = out.polyIndices[(physx::PxU32)m];
								if (out.vertices[(physx::PxU32)testId] == (pv1.pos)) {
									sharedId = testId;
									break;
								}
							}
					...
					if (sharedId >= 0 && out.normals[(physx::PxU32)sharedId] == (pv1.normal)) 		// share it
						id = sharedId;
					else {
						id = (physx::PxI32)out.vertices.size();		// create new
						out.vertices.pushBack((pv1.pos));
						out.normals.pushBack((pv1.normal));
						out.tangents.pushBack((pv1.tangent));
						out.texCoords.pushBack((float)pv1.u);
			...
			out.polyStarts.pushBack((physx::PxI32)out.polyIndices.size());	// close poly
			...
			for (int k = 0; k < num; k++) {
				...
				if (pv1.id >= (int)mFaceVertNrs.size())
					mFaceVertNrs.resize((physx::PxU32)pv1.id + 1, -1);
				
				int &vertNr = mFaceVertNrs[(physx::PxU32)pv1.id];
				if (vertNr < 0) {
					FaceVert newSv;
					newSv.init();
					vertNr = (physx::PxI32)mFaceVerts.size();
					mFaceVerts.pushBack(newSv);
				}
				FaceVert &sv = mFaceVerts[(physx::PxU32)vertNr];
				sv.id = pv1.id;
			}
			...
			for (int k = 0; k < num; k++) {
				...
				FaceVert &sv = mFaceVerts[(physx::PxU32)mFaceVertNrs[(physx::PxU32)pv1.id]];
				if (pv0.edgeOnFace) {
					sv.nextFaceNr = pv0.edgeNeighbor;
					sv.neighborPos = out.polyStarts[out.polyStarts.size()-2] + (k+num-1)%num;
					sv.neighborPolyNr = (physx::PxI32)out.polyStarts.size()-2;
					sv.nextVert = mFaceVertNrs[(physx::PxU32)pv0.id];
				}
				if (pv1.edgeOnFace) {
	...
	for (int i = 0; i < (int)out.polyStarts.size()-1; i++) {
		int first = out.polyStarts[(physx::PxU32)i];
		int num = out.polyStarts[(physx::PxU32)i+1] - first;
		for (int j = 0; j < num; j++) {
			int oldNr = out.polyNeighbors[physx::PxU32(first + j)];
			if (oldNr < 0)
				continue;

			int numNew = mNumOutPolys[(physx::PxU32)oldNr];
			if (!check(numNew > 0, "fix internal neighbors, no new polygond created")) 
				return false;

			if (numNew == 1) {		// simple mapping
				out.polyNeighbors[physx::PxU32(first + j)] = mFirstOutPoly[(physx::PxU32)oldNr];
			}
			else {		// find correct new poly
				PxVec3 v1 = out.vertices[(physx::PxU32)out.polyIndices[physx::PxU32(first + (j+1)%num)]];
				float minD2 = PX_MAX_F32;
				int minNr = -1;

				for (int k = 0; k < numNew; k++) {
					int newNr = mFirstOutPoly[(physx::PxU32)oldNr] + k;
					int firstNew = out.polyStarts[(physx::PxU32)newNr];
					int lastNew = out.polyStarts[(physx::PxU32)newNr+1];
					for (int l = firstNew; l < lastNew; l++) {
						PxVec3 v0 = out.vertices[(physx::PxU32)out.polyIndices[(physx::PxU32)l]];
				...
				out.polyNeighbors[physx::PxU32(first + j)] = minNr;
			}
		}
	}
...
bool MeshClipper::createFacePolys()
{
	...
	Mesh &out = mOutMeshes[0];
	int firstFacePoly = (physx::PxI32)out.polyStarts.size()-1;


	// create face polys
	for (physx::PxU32 i = 0; i < mFaceVerts.size(); i++) {
		...
		mFaceTypes[(physx::PxU32)faceNr] = 1;	// has visual mesh

		PxVec3 faceNormal = clipPlanes[(physx::PxU32)faceNr].n;
		PxVec3 t0,t1;
		// tangents
		...
		const Convex::Face &face = clipConvexFaces[(physx::PxU32)faceNr];
		int thisPolyNr = (physx::PxI32)out.polyStarts.size()-1;

		int vertNr = (physx::PxI32)i; 
		int safetyCnt = 0;
		...
		while (true) {
			...
			out.polyIndices.pushBack((physx::PxI32)out.vertices.size());
			PxVec3 temp = out.vertices[(physx::PxU32)sv.id];
			out.vertices.pushBack(temp);
			out.normals.pushBack(faceNormal);
			out.tangents.pushBack(t0);
			out.texCoords.pushBack(out.vertices[(physx::PxU32)sv.id].dot(t0) * mTexScale);
			out.texCoords.pushBack(out.vertices[(physx::PxU32)sv.id].dot(t1) * mTexScale);

			if (sv.nextFaceNr == faceNr) {		// continues on this face
				sv.visited = true;
				out.polyNeighbors.pushBack(sv.neighborPolyNr);
				out.polyNeighbors[(physx::PxU32)sv.neighborPos] = thisPolyNr;
				vertNr = sv.nextVert;
				if (vertNr == (physx::PxI32)i)
					break;			// start reached
				else
					continue;
			}
			...
			int edgeNr = 0;
			while (edgeNr < face.numIndices && mClipFaceNeighbors[physx::PxU32(face.firstIndex + edgeNr)] != sv.nextFaceNr)
				edgeNr++;
			...
			for (int j = 0; j <= face.numIndices; j++) {
				int adjFaceNr = mClipFaceNeighbors[physx::PxU32(face.firstIndex + edgeNr)];
				out.polyNeighbors.pushBack(-1-adjFaceNr);
		
				PxVec3 edgeDir = clipPlanes[(physx::PxU32)faceNr].n.cross(clipPlanes[(physx::PxU32)adjFaceNr].n);
				float s0 = out.vertices[out.vertices.size()-1].dot(edgeDir);
				float minS = PX_MAX_F32;
				int minK = -1;
				for (physx::PxU32 k = 0; k < mFaceVerts.size(); k++) {		// todo: create edge face vert list to speed up
					FaceVert &vk = mFaceVerts[k];
//					if (vk.id < 0) continue;
					if (vk.nextFaceNr != faceNr || vk.prevFaceNr != adjFaceNr)
						continue;
					float si = out.vertices[(physx::PxU32)vk.id].dot(edgeDir);
					if (si > s0 && si < minS) {
						minS = si;
						minK = (physx::PxI32)k;
					}
				...
				PxVec3 pos = clipConvexVertices[(physx::PxU32)clipConvexIndices[physx::PxU32(face.firstIndex + (edgeNr+1)%face.numIndices)]];
				...
				out.polyIndices.pushBack((physx::PxI32)out.vertices.size()-1);
				if (numCorners > 0) {
					if (mFaceTypes[(physx::PxU32)mClipFaceNeighbors[physx::PxU32(face.firstIndex + edgeNr)]] == 0)
						mFaceTypes[(physx::PxU32)mClipFaceNeighbors[physx::PxU32(face.firstIndex + edgeNr)]] = 2;	// inside visual mesh
				}
		...
		out.polyStarts.pushBack((physx::PxI32)out.polyIndices.size());	// close face poly
	}
	...
	for (physx::PxU32 i = 0; i < mFaceTypes.size(); i++) {
		if (mFaceTypes[i] != 2)
			continue;
		mFaceTypes[i] = 0;
		mFaceQueue.resize(1, (physx::PxI32)i);
		while (mFaceQueue.size() > 0) {
			physx::PxU32 faceNr = (physx::PxU32)mFaceQueue[mFaceQueue.size()-1];
			mFaceQueue.popBack();
			...
			for (int j = 0; j < face.numIndices; j++) {
				int adjFaceNr = mClipFaceNeighbors[physx::PxU32(face.firstIndex + j)];
				if (mFaceTypes[(physx::PxU32)adjFaceNr] == 0)
	...
	for (physx::PxU32 i = 0; i < clipConvexFaces.size(); i++) {
		if (mFaceTypes[i] != 2)
			continue;
		...
		for (int j = 0; j < (int)face.numIndices; j++) {
			PxVec3 pos = clipConvexVertices[(physx::PxU32)clipConvexIndices[physx::PxU32(face.firstIndex + j)]];
			out.vertices.pushBack(pos);
			out.normals.pushBack(faceNormal);
			out.tangents.pushBack(t0);
			out.texCoords.pushBack(pos.dot(t0) * mTexScale);
			...
			out.polyIndices.pushBack((physx::PxI32)out.vertices.size()-1);
			out.polyNeighbors.pushBack(-1-mClipFaceNeighbors[physx::PxU32(face.firstIndex+j)]);
		}
		mFaceOfPoly.pushBack((physx::PxI32)i);
		out.polyStarts.pushBack((physx::PxI32)out.polyIndices.size());
	}

	// fix face polygon neighbors
	for (physx::PxU32 i = (physx::PxU32)firstFacePoly; i < out.polyStarts.size()-1; i++) {
		int first = out.polyStarts[i];
		int num = out.polyStarts[i+1] - first;
		for (int j = 0; j < num; j++) {
			int adjFaceNr = out.polyNeighbors[physx::PxU32(first + j)];
			if (adjFaceNr >= 0)
				continue;
			adjFaceNr = -1 - adjFaceNr;
			PxVec3 &p0 = out.vertices[(physx::PxU32)out.polyIndices[physx::PxU32(first + (j+1)%num)]];
			float minD2 = PX_MAX_F32;
			int minAdjPolyNr = -1;

			for (physx::PxU32 k = 0; k < mFaceOfPoly.size(); k++) {
				if (mFaceOfPoly[k] != adjFaceNr)
					continue;
				int adjPolyNr = firstFacePoly + (physx::PxI32)k;
				int adjFirst = out.polyStarts[(physx::PxU32)adjPolyNr];
				int adjLast = out.polyStarts[(physx::PxU32)adjPolyNr+1];
				for (int l = adjFirst; l < adjLast; l++) {
					PxVec3 &p1 = out.vertices[(physx::PxU32)out.polyIndices[(physx::PxU32)l]];
			...
			out.polyNeighbors[physx::PxU32(first + j)] = minAdjPolyNr;
		}
	}
	return true;
}
					   ...
					   bool &empty, bool &completelyInside)
{
	...
	int numPolys = (physx::PxI32)meshPolyStarts.size()-1;
	...
	mFirstOutPoly.resize((physx::PxU32)numPolys, -1);
	mNumOutPolys.clear();
	mNumOutPolys.resize((physx::PxU32)numPolys, 0);
...
bool MeshClipper::splitIslands()
{
	...
	physx::PxU32 numPolys = mOutMeshes[0].polyStarts.size()-1;
	mColors.clear();
	...
	for (physx::PxU32 i = 0; i < numPolys; i++) {
		...
		while (!mQueue.empty()) {
			physx::PxU32 polyNr = (physx::PxU32)mQueue[mQueue.size()-1];
			mQueue.popBack();
			if (mColors[polyNr] >= 0)
				continue;
			...
			for (int j = first; j < last; j++) {
				int neighborNr = mOutMeshes[0].polyNeighbors[(physx::PxU32)j];
				if (mColors[(physx::PxU32)neighborNr] >= 0)
					continue;
	...
	// create child meshes
	mOutMeshes.resize((physx::PxU32)numIslands+1);
	for (physx::PxU32 i = 0; i < (physx::PxU32)numIslands; i++) {
	...
	for (physx::PxU32 i = 0; i < numPolys; i++) {
		...
		Mesh &child = mOutMeshes[(physx::PxU32)meshNr+1];
		int first = parent.polyStarts[i];
		int last = parent.polyStarts[i+1];
		for (int j = first; j < last; j++) {
			physx::PxU32 id = (physx::PxU32)parent.polyIndices[(physx::PxU32)j];
			if (mNewVertNr[id] < 0) {
				mNewVertNr[id] = (physx::PxI32)child.vertices.size();
			...
			child.polyNeighbors.pushBack(parent.polyNeighbors[(physx::PxU32)j]);	// cannot adjust neighbor number yet
		}
		mNewPolyNr[i] = (physx::PxI32)child.polyStarts.size()-1;
		child.polyStarts.pushBack((physx::PxI32)child.polyIndices.size());
	}

	// adjust neighbor numbers
	//int num = 0;
	for (int i = 0; i < numIslands; i++) {
		Mesh &child = mOutMeshes[1+(physx::PxU32)i];
		for (int j = 0; j < (int)child.polyStarts.size()-1; j++) {
			int first = child.polyStarts[(physx::PxU32)j];
			int last = child.polyStarts[(physx::PxU32)j+1];
			for (int k = first; k < last; k++) {
				child.polyNeighbors[(physx::PxU32)k] = mNewPolyNr[(physx::PxU32)child.polyNeighbors[(physx::PxU32)k]];
			}
		}
	}
	return true;
...
bool MeshClipper::testNeighbors(int meshNr)
{
	if (meshNr >= (int)mOutMeshes.size())
		return false;
	Mesh &mesh = mOutMeshes[(physx::PxU32)meshNr];

	for (int i = 0; i < (int)mesh.polyStarts.size()-1; i++) {
		int first = mesh.polyStarts[(physx::PxU32)i];
		int num = mesh.polyStarts[(physx::PxU32)i+1] - first;
		for (int j = 0; j < num; j++) {
			physx::PxU32 i0 = (physx::PxU32)mesh.polyIndices[physx::PxU32(first+j)];
			physx::PxU32 i1 = (physx::PxU32)mesh.polyIndices[physx::PxU32(first + (j+1)%num)];
			PxVec3 p0 = mesh.vertices[i0];
			PxVec3 p1 = mesh.vertices[i1];

			int adj = mesh.polyNeighbors[physx::PxU32(first + j)];
			if (adj < 0) 
				return false;

			int adjFirst = mesh.polyStarts[(physx::PxU32)adj];
			int adjNum = mesh.polyStarts[(physx::PxU32)adj+1] - adjFirst;
			bool found = false;
			for (int k = 0; k < adjNum; k++) {
				if (mesh.polyNeighbors[physx::PxU32(adjFirst+k)] == i) {
					physx::PxU32 j0 = (physx::PxU32)mesh.polyIndices[physx::PxU32(adjFirst+k)];
					physx::PxU32 j1 = (physx::PxU32)mesh.polyIndices[physx::PxU32(adjFirst + (k+1)%adjNum)];
...
bool MeshClipper::loadDump()
{
	...
	bool isManifold = c->setExplicitVisMeshFromPolygons(
		(physx::PxI32)vertices.size(), &vertices[0], &normals[0], NULL, NULL,	// todo, save as well
		(physx::PxI32)polyStarts.size()-1, &polyStarts[0],
		(physx::PxI32)polyIndices.size(), &polyIndices[0]);
