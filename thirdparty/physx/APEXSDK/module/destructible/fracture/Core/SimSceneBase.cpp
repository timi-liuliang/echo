...
SimScene::~SimScene()
{
	...
	{
		PX_DELETE(mActors[(physx::PxU32)i]);
	}
...
void SimScene::clear()
{
	for(physx::PxU32 i = 0 ; i < mActors.size(); i++)
	{
		mActors[i]->clear();
	}
	deleteCompounds();
...
{
	for (physx::PxU32 i = 0; i < delCompoundList.size(); i++) 
	{
...
bool SimScene::rayCast(const PxVec3 &orig, const PxVec3 &dir, float &dist, int &actorNr , int &compoundNr, int &convexNr, PxVec3 &normal) const
{
	...
	for (int i = 0; i < (int)mActors.size(); i++)
	{
		...
		int conNr;
		PxVec3 n;
		if (mActors[(physx::PxU32)i]->rayCast(orig,dir,d,comNr,conNr,n))
		{
			if( d < dist )
			{
				dist = d;
...
bool SimScene::pickStart(const PxVec3 &orig, const PxVec3 &dir)
{
	...
	mPickActor = mActors[(physx::PxU32)actorNr]->mCompounds[(physx::PxU32)compoundNr]->getPxActor();
...
bool SimScene::patternFracture(const PxVec3 &orig, const PxVec3 &dir, const PxMat33 patternTransform, float impactRadius, float radialImpulse, float directionalImpulse)
{
	...
	return mActors[(physx::PxU32)actorNr]->patternFracture(orig+dist*dir,normal,compoundNr,patternTransform,impactRadius,radialImpulse,directionalImpulse);
}
...
void SimScene::processFractureEvents(bool& valid,bool* addFireDustP)
{
	...
	for (physx::PxU32 i = 0; i < mFractureEvents.size(); i++) {
...
//			playSound("explosion", 1);
		...
		for (physx::PxU32 j = 0; j < compounds.size(); j++) {
...
//			a->setContactReportFlags(Px_NOTIFY_ON_TOUCH_FORCE_THRESHOLD);
		...
		{
			mActors[(physx::PxU32)actorNr]->mCompounds[(physx::PxU32)compoundNr] = compounds[0];
			first++;
		}
		...
		for (int j = first; j < (int)compounds.size(); j++)
			mActors[(physx::PxU32)actorNr]->mCompounds.pushBack(compounds[(physx::PxU32)j]);
...
void SimScene::onContact(const PxContactPairHeader& pairHeader, const PxContactPair* pairs, PxU32 nbPairs)
{
	...
	for (int i = 0; i < (int)nbPairs; i++) {
		...
		physx::PxU32 num = pair.contactCount;
		if (num == 0)
			continue;
...
void SimScene::onTrigger(PxTriggerPair* pairs, PxU32 count)
{
	// Pass along to application's callback, if defined
	if (mAppNotify != NULL)
	{
...
void SimScene::onConstraintBreak(PxConstraintInfo* constraints, PxU32 count)
{
	// Pass along to application's callback, if defined
	if (mAppNotify != NULL)
	{
