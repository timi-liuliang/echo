...
void Compound::createFromMesh(const Mesh *mesh, const PxTransform &pose, const PxVec3 &vel, const PxVec3 &omega, int submeshNr, const PxVec3& scale)
{
	...
	if (submeshNr < 0)
		mConvexes[0]->setExplicitVisMeshFromTriangles((physx::PxI32)verts.size(), &verts[0], &normals[0], &texcoords[0], (physx::PxI32)indices.size(), &indices[0], &trans, &scale);
	else {
		const Mesh::SubMesh &sm = mesh->getSubMeshes()[(physx::PxU32)submeshNr];
		mConvexes[0]->setExplicitVisMeshFromTriangles((physx::PxI32)verts.size(), &verts[0], &normals[0], &texcoords[0], sm.numIndices, &indices[(physx::PxU32)sm.firstIndex], &trans, &scale);
	}
}
...
bool Compound::isAttached()
{
	...
	for (physx::PxU32 i = 0; i < mConvexes.size(); i++) {
		...
		for (physx::PxU32 j = 0; j < mAttachmentBounds.size(); j++) {
...
//	localHit = -pose.t;	// foo
	...
	int debugPointsStart = (physx::PxI32)debugPoints.size();

	physx::PxU32 convexNr = 0;
	for (physx::PxU32 i = 0; i < compoundSizes.size(); i++) {
		physx::PxU32 num = (physx::PxU32)compoundSizes[i];
		...
		addDust(newConvexes,debugPoints,(physx::PxI32&)convexNr,(physx::PxI32&)num);
		...
		if (m->createFromConvexes(&newConvexes[convexNr], (physx::PxI32)num, posei, v, av, false)) {
...
void Compound::attach(const shdfnd::Array<PxBounds3> &bounds)
{
	...
	for (physx::PxU32 i = 0; i < bounds.size(); i++) {
...
void Compound::createPointSamples(float area) {	
	...
	for (physx::PxU32 i = 0; i < mConvexes.size(); i++) {
		...
		for (physx::PxU32 k = 0; k < faces.size(); k++) {
			...
			*/
			...
			tmpPoints.clear();
			for (int q = 0; q < f.numIndices; q++) {
				tmpPoints.pushBack(verts[(physx::PxU32)indices[physx::PxU32(f.firstIndex+q)]]);
			}
			appendUniformSamplesOfConvexPolygon(&tmpPoints[0], (physx::PxI32)tmpPoints.size(), area, pointSamples);
