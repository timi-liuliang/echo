...
#define CREATE_HISTOGRAMS(type, buffer)														\
	/* Clear counters/histograms */															\
																							\
	memset(mHistogram, 0, 256*4*sizeof(PxU32));												\
																							\
	/* Prepare to count */																	\
	PxU8* p = (PxU8*)input;																\
	PxU8* pe = &p[nb*4];																	\
	PxU32* h0= &mHistogram[0];		/* Histogram for first pass (LSB)	*/					\
	PxU32* h1= &mHistogram[256];	/* Histogram for second pass		*/					\
	PxU32* h2= &mHistogram[512];	/* Histogram for third pass			*/					\
	PxU32* h3= &mHistogram[768];	/* Histogram for last pass (MSB)	*/					\
																							\
...
#define CHECK_PASS_VALIDITY(pass)															\
	...
	PxU32* CurCount = &mHistogram[pass<<8];												\
																							\
	...
	/* for words and O(n) for bytes. Running time for floats depends on actual values... */	\
																							\
	/* Get first byte */																	\
	PxU8 UniqueVal = *(((PxU8*)input)+pass);												\
																							\
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool RadixSort::Resize(PxU32 nb)
{
	if (mRanksSize >= nb)
		return true;
	...
	// Get some fresh one
	mRanks  = (PxU32*) PX_ALLOC(sizeof(PxU32)*nb,PX_DEBUG_EXP("RT_FRACTURE"));
	mRanks2 = (PxU32*) PX_ALLOC(sizeof(PxU32)*nb,PX_DEBUG_EXP("RT_FRACTURE"));
...
inline void RadixSort::CheckResize(PxU32 nb)
{
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadixSort& RadixSort::Sort(const PxU32* input, PxU32 nb, bool signedvalues)
{
	// Checkings
	...
	if(!signedvalues)	{ CREATE_HISTOGRAMS(PxU32, input);	}
	...
	for(PxU32 j=0;j<4;j++)
	{
		...
		if(PerformPass)
		{
			...
			if(j!=3 || !signedvalues)
			{
				...
				// Create offsets
				mOffset[0] = 0;
				for(PxU32 i=1;i<256;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];
			}
			else
			{
				...
				for(PxU32 i=1;i<128;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];	// 1 to 128 for positive numbers

				// Fixing the wrong place for negative values
				mOffset[128] = 0;
				for(PxU32 i=129;i<256;i++)			mOffset[i] = mOffset[i-1] + CurCount[i-1];
			}

			// Perform Radix Sort
			PxU8* InputBytes	= (PxU8*)input;
			InputBytes += j;
			if(INVALID_RANKS)
			{
				for(PxU32 i=0;i<nb;i++)	mRanks2[mOffset[InputBytes[i<<2]]++] = i;
				VALIDATE_RANKS;
			}
			else
			{
				PxU32* Indices		= mRanks;
				PxU32* IndicesEnd	= &mRanks[nb];
	...
	for(PxU32 j=0;j<4;j++)
	{
		...
		else
		{
			...
			if(PerformPass)
			{
				...
				mOffset[255] = 0;
				for(PxU32 i=0;i<127;i++)		mOffset[254-i] = mOffset[255-i] + CurCount[255-i];	// Fixing the wrong order for negative values
				for(PxU32 i=128;i<256;i++)	mOffset[i] += CurCount[i];							// Fixing the wrong place for negative values
				...
				if(INVALID_RANKS)
				{
					...
					{
						PxU32 Radix = input[i]>>24;							// Radix byte, same as above. AND is useless here (PxU32).
				...
				else
				{
					for(PxU32 i=0;i<nb;i++)
					{
						PxU32 Radix = input[mRanks[i]]>>24;							// Radix byte, same as above. AND is useless here (PxU32).
						// ### cmp to be killed. Not good. Later.
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
PxU32 RadixSort::GetUsedRam() const
{
	PxU32 UsedRam = sizeof(RadixSort);

	UsedRam += 2*CURRENT_SIZE*sizeof(PxU32);	// 2 lists of indices
	return UsedRam;
}
