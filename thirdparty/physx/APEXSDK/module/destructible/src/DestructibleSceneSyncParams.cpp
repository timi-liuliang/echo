...
DamageEvent & SyncParams::interpret(const UserDamageEvent & userDamageEvent_)
{
	...
	damageEventImplInstance.chunkIndexInAsset	= (physx::PxI32)userDamageEvent.chunkIndex;
...
template<typename Callback, typename Header, typename Unit> const Header * SyncParams::processReadBuffer(Callback & callback) const
{
	...
	callback.onPreProcessReadDone((physx::PxU32)headerCount);
...
bool SyncParams::interceptLoad(const DamageEvent & damageEvent, const DestructibleActor & syncActor) const
{
	...
	const physx::PxI32 chunkArrayCount = (physx::PxI32)syncActor.getAsset()->getChunkCount();
	...
	const physx::PxU16 chunkDepth = PxU16((NxModuleDestructibleConst::INVALID_CHUNK_INDEX == static_cast<physx::PxI32>(damageEvent.chunkIndexInAsset)) ? 0 : static_cast<const DestructibleAssetParameters*>(syncActor.getAsset()->getAssetNxParameterized())->chunks.buf[damageEvent.chunkIndexInAsset].depth);
	return
...
bool SyncParams::interceptLoad(const FractureEvent & fractureEvent, const DestructibleActor & syncActor) const
{
	...
	const physx::PxU32 chunkArrayCount = syncActor.getAsset()->getChunkCount();
	PX_UNUSED(chunkArrayCount);
	PX_ASSERT(chunkArrayCount > 0);
