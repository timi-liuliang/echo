...
CudaModuleScene::CudaModuleScene(NiApexScene& scene, NxModule& module, const char* modulePrefix)
	: mNiApexScene(scene)
{
	...
	mCudaModules.resize((physx::PxU32)numRegisteredModules);
...
{
	{
		physx::PxScopedCudaLock _lock_(*getGpuDispatcher()->getCudaContextManager());

		ApexCudaObjManager::releaseAll();

		for (physx::PxU32 i = 0 ; i < mCudaModules.size() ; i++)
		{
...
void CudaModuleScene::onAfterLaunchApexCudaFunc(const ApexCudaFunc& func, CUstream stream)
{
	...
	if (mNiApexScene.getCudaKernelCheckEnabled())
#endif
	{
...
{
	mCudaModules[(physx::PxU32)modIndex].init(moduleTable[(physx::PxU32)modIndex]);
	return &mCudaModules[(physx::PxU32)modIndex];
}
