...
#ifndef PXC_ARTICULATION_SIMD_FNS_H
#define PXC_ARTICULATION_SIMD_FNS_H
...
class PxcArticulationFnsSimdBase
{
public:
	...
	static PX_FORCE_INLINE Vec3V axisDot(const PxcSIMDSpatial S[3], const PxcSIMDSpatial &v)
	{
	...
	static PX_FORCE_INLINE PxcSIMDSpatial axisMultiply(const PxcSIMDSpatial S[3], Vec3V v)
	{
		return PxcSIMDSpatial(V3ScaleAdd(S[0].linear, V3GetX(v), V3ScaleAdd(S[1].linear, V3GetY(v), V3Scale(S[2].linear, V3GetZ(v)))),
							  V3ScaleAdd(S[0].angular, V3GetX(v), V3ScaleAdd(S[1].angular, V3GetY(v), V3Scale(S[2].angular, V3GetZ(v)))));
	}


	static PX_FORCE_INLINE PxcSIMDSpatial subtract(const PxcSIMDSpatial &a, const PxcSIMDSpatial &b)
	{
		return PxcSIMDSpatial(V3Sub(a.linear, b.linear), V3Sub(a.angular, b.angular));
	}

	static PX_FORCE_INLINE PxcSIMDSpatial add(const PxcSIMDSpatial &a, const PxcSIMDSpatial &b)
	{
		return PxcSIMDSpatial(V3Add(a.linear, b.linear), V3Add(a.angular, b.angular));
	}


	static PX_FORCE_INLINE PxcSIMDSpatial multiply(const PxcFsInertia &I, const PxcSIMDSpatial &S)
	{
		return PxcSIMDSpatial(V3Add(M33MulV3(I.ll,S.linear),	   M33MulV3(I.la,S.angular)),
							  V3Add(M33TrnspsMulV3(I.la,S.linear), M33MulV3(I.aa,S.angular)));
	}


	static PX_FORCE_INLINE PxcSIMDSpatial translateMotion(const Vec3V& p, const PxcSIMDSpatial& v)
	{
		return PxcSIMDSpatial(V3Add(v.linear, V3Cross(p, v.angular)), v.angular);
	}

	// translate a force resolved at position p to the origin

	static PX_FORCE_INLINE PxcSIMDSpatial translateForce(const Vec3V& p, const PxcSIMDSpatial& v)
	{
		return PxcSIMDSpatial(v.linear, V3Add(v.angular, V3Cross(p, v.linear)));
	}
	...
	static PX_FORCE_INLINE PxcFsInertia invertInertia(const PxcFsInertia &I)
	{
		...
		return PxcFsInertia(LL, LA, AA);
	}

	static PX_FORCE_INLINE Mat33V computeSIS(const PxcFsInertia &I, const PxcSIMDSpatial S[3], PxcSIMDSpatial IS[3])
	{
	...
	static PX_FORCE_INLINE PxcFsInertia multiplySubtract(const PxcFsInertia &I, const Mat33V &D, const PxcSIMDSpatial IS[3], PxcSIMDSpatial DSI[3])
	{
...
#undef SUBTRACT_DYAD
		...
		return PxcFsInertia(Mat33V(ll0, ll1, ll2), 
							Mat33V(la0, la1, la2),
							Mat33V(aa0, aa1, aa2));
	} 


	static PX_FORCE_INLINE PxcFsInertia multiplySubtract(const PxcFsInertia &I, const PxcSIMDSpatial S[3])
	{
		// cut'n'paste, how I love ya, how I love ya
...
#undef SUBTRACT_DYAD
	...
	static PX_FORCE_INLINE PxcFsInertia translateInertia(Vec3V a, const PxcFsInertia &input)
	{
		Vec3V b = V3Neg(a);
...
template<class Base>
class PxcArticulationFnsSimd : public Base
{
	static PX_FORCE_INLINE void axisMultiplyLowerTriangular(PxcSIMDSpatial ES[3], const Mat33V&E, const PxcSIMDSpatial S[3])
	{
		const Vec3V l0 = S[0].linear,  l1 = S[1].linear,  l2 = S[2].linear;
		const Vec3V a0 = S[0].angular, a1 = S[1].angular, a2 = S[2].angular;
		ES[0] = PxcSIMDSpatial(V3Scale(l0, V3GetX(E.col0)),
							   V3Scale(a0, V3GetX(E.col0)));
		ES[1] = PxcSIMDSpatial(V3ScaleAdd(l0, V3GetX(E.col1), V3Scale(l1, V3GetY(E.col1))),
							   V3ScaleAdd(a0, V3GetX(E.col1), V3Scale(a1, V3GetY(E.col1))));
		ES[2] = PxcSIMDSpatial(V3ScaleAdd(l0, V3GetX(E.col2), V3ScaleAdd(l1, V3GetY(E.col2), V3Scale(l2, V3GetZ(E.col2)))),
...
public:
	...
	static PX_INLINE PxcSIMDSpatial propagateImpulse(const PxcFsRow& row, 
	...
	static PX_INLINE PxcSIMDSpatial propagateVelocity(const PxcFsRow& row, 
													  const PxcFsJointVectors& jv,
													  const Vec3V& SZ, 
													  ...
													  const PxcFsRowAux& aux)
	{
		...
		PxcSIMDSpatial w = Base::translateMotion(V3Neg(jv.parentOffset), v);
		Vec3V DSZ = M33MulV3(row.D, SZ);

		Vec3V n = V3Add(Base::axisDot(row.DSI, w), DSZ);
		return w - PxcSIMDSpatial(V3Cross(jv.jointOffset, n), n);
	}
	...
	static PX_FORCE_INLINE Mat33V computeDriveInertia(const PxcFsInertia &I0, 
													  ...
													  const PxcSIMDSpatial S[3])
	{
		POD_U_LIKE(PxcSIMDSpatial, 3, 16) IS, ISD, dummy;		
		Mat33V D = Base::computeSIS(I0, S, IS);
		Mat33V DInv = Base::invertSym33(D);

		PxcFsInertia tmp = Base::addInertia(I0, I1);
		tmp = Base::multiplySubtract(tmp, DInv, IS, ISD);
		PxcFsInertia J = Base::invertInertia(tmp);
