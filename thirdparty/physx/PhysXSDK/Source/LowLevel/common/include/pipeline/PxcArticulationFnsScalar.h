...
#ifndef PXC_ARTICULATION_SCALAR_FNS_H
#define PXC_ARTICULATION_SCALAR_FNS_H
...
class PxcArticulationDiagnostics
{
public:
...
class PxcArticulationFnsScalar
{
public:
...
//		return SpInertia(i.mLL, i.mLA + llst, i.mAA + sla + sla.getTranspose() + S * llst);
									   ...
									   const Cm::SpatialVector S[3])
	{
		...
		PX_ASSERT(PxcArticulationDiagnostics::isSymmetric(load) && PxcArticulationDiagnostics::isPositiveDefinite(load));
		return load;
	}

	static PX_INLINE Cm::SpatialVector propagateImpulse(const PxcFsRow& row, 
	...
	static PX_INLINE Cm::SpatialVector propagateVelocity(const PxcFsRow& row, 
														 const PxcFsJointVectors& jv,
														 const PxVec3& SZ, 
	...
	{
		return (PxVec3&)m.col0*v.x + (PxVec3&)m.col1 * v.y + (PxVec3&)m.col2 * v.z;
	}

	static PX_FORCE_INLINE PxVec3 multiplyTranspose(const Mat33V& m, const PxVec3& v)
	{
		return PxVec3(v.dot((PxVec3&)m.col0), v.dot((PxVec3&)m.col1), v.dot((PxVec3&)m.col2));
	}

	static Cm::SpatialVector multiply(const PxcFsInertia& m, const Cm::SpatialVector& v)
	{
	...
	static PX_FORCE_INLINE Cm::SpatialVector getRootDeltaV(const PxcFsData& matrix, const Cm::SpatialVector& Z)
	{
