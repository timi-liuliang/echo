...
#ifndef PXC_HEIGHT_FIELD_AABB_TEST_H
#define PXC_HEIGHT_FIELD_AABB_TEST_H
...
*/
class PxcHeightFieldAabbTest
{
public:

	PxcHeightFieldAabbTest(const PxBounds3& localBounds, const Gu::HeightFieldUtil& hfUtil) :
		mHfUtil(hfUtil),
		mIsEmpty(false)
	{
		...
		if ((minimum.x > (mHfUtil.getHeightField().getNbRowsFast()-1)) || 
			...
			(minimum.z > (mHfUtil.getHeightField().getNbColumnsFast()-1)) ||
			(maximum.x < 0) ||
			(maximum.z < 0))
		{
			mIsEmpty = true;
		...
		// need to be swapped
		mSwapVertIdx12 = ((mHfUtil.getHeightField().getThicknessFast() > 0.0f) != Ps::differentSign(hfGeom.rowScale, hfGeom.columnScale));
	}
	...
	public:
		...
		Iterator& operator++()
		{
			...
			while (mOffset < endOffset)
			{
				...
				{
					mRow++;
					mOffset += (mTest.mHfUtil.getHeightField().getNbColumnsFast() - (mTest.mMaxColumn - mTest.mMinColumn));
	...
	private:
		...
		Iterator(PxU32 row, PxU32 column, const PxcHeightFieldAabbTest& test) :
			mRow(row),
			mColumn(column),
			mTest(test)
		{
		...
		PxU32 mTri;
		PxU32 mOffset;
		const PxcHeightFieldAabbTest& mTest;

		friend class PxcHeightFieldAabbTest;
	};
...
private:
	...
	PxcHeightFieldAabbTest& operator=(const PxcHeightFieldAabbTest&);
	...
	{
		return mHfUtil.getHeightField().getTriangleMaterial((offset << 1) + triangleIndex) == PxHeightFieldMaterial::eHOLE;
	}
	...
	bool intersectsSegment(PxU32 offset) const
	{
		...
		return (!((mMaxy < h0 && mMaxy < h1 && mMaxy < h2 && mMaxy < h3) || (mMiny > h0 && mMiny > h1 && mMiny > h2 && mMiny > h3)));
	}
	...
	void getTriangleVertices(PxVec3* triangleVertices, const Iterator& iterator) const
	{
		...
		mHfUtil.getHeightField().getTriangleVertices(triangleIndex, 
													 iterator.mRow,
													 iterator.mColumn, 
													 triangleVertices[0], 
													 triangleVertices[vertIdx1], 
													 triangleVertices[vertIdx2]);
...
#endif // PXC_HEIGHT_FIELD_AABB_TEST_H
