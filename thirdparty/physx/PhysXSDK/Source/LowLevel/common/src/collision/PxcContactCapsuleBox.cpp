...
static bool PxcTestAxis(const PxVec3& axis, const Gu::Segment& segment, PxReal radius, const Gu::Box& box, PxReal& depth)
{
	// Project capsule
...
static bool PxcCapsuleOBBOverlap3(const Gu::Segment& segment, PxReal radius, const Gu::Box& box, PxReal* t=NULL, PxVec3* pp=NULL)
{
	PxVec3 Sep(PxReal(0));
...
static void PxcGenerateVFContacts(	ContactBuffer& contactBuffer,
									//
									...
									const PxReal contactDistance)
{
	...
	for(PxU32 i=0;i<2;i++)
	{
		...
		PxReal tnear, tfar;
		int Res = Gu::intersectRayAABB(Min, Max, tmp, tmp2, tnear, tfar);
...
static void PxcGenerateEEContacts(	ContactBuffer& contactBuffer,
									...
									const PxVec3& normal)
{
	const PxU8* PX_RESTRICT Indices = Gu::getBoxEdges();
...
//		PxReal dist;
//		PxVec3 ip;
//		if(Gu::intersectEdgeEdge(p1, p2, -normal, segment.p0, segment.p1, dist, ip))
...
static void PxcGenerateEEContacts2(	ContactBuffer& contactBuffer,
									//
									const Gu::Segment& segment,
									const PxReal radius,
									//
...
//		PxReal dist;
//		PxVec3 ip;
//		bool contact = Gu::intersectEdgeEdge(p1, p2, normal, segment.p0, segment.p1, dist, ip);
...
bool PxcContactCapsuleBox(CONTACT_METHOD_ARGS)
{
	...
	// Box data
	const Gu::Box worldBox(transform1.p, shapeBox.halfExtents, PxMat33(transform1.q));

	// Collision detection
	PxReal t;
	PxVec3 onBox;
	const PxReal squareDist = Gu::distanceSegmentBoxSquared(worldSegment.p0, worldSegment.p1, worldBox.center, worldBox.extents, worldBox.rot, &t, &onBox);
	...
	if(squareDist != 0.0f)
	{
		...
		if(normalLen > 0.0f)
		{
			...
			PxcGenerateVFContacts(contactBuffer, worldSegment, shapeCapsule.radius, worldBox, normal, contactDistance);
			...
			PxcGenerateEEContacts2(contactBuffer, worldSegment, shapeCapsule.radius, worldBox, normal, contactDistance);
	...
	else
	{
		...
		PxVec3 sepAxis;
		PxReal depth;
		if(!PxcCapsuleOBBOverlap3(worldSegment, shapeCapsule.radius, worldBox, &depth, &sepAxis)) return false;

		// PT: generate VF contacts for segment's vertices vs box
		PxcGenerateVFContacts(contactBuffer, worldSegment, shapeCapsule.radius, worldBox, sepAxis, contactDistance);
		...
		// PT: else generate slower EE contacts
		PxcGenerateEEContacts(contactBuffer, worldSegment, shapeCapsule.radius, worldBox, sepAxis);
