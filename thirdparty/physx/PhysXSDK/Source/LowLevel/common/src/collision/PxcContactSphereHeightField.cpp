...
#ifdef __SPU__
unsigned char HeightFieldBuffer[sizeof(Gu::HeightField)+16] __attribute__((aligned(16)));
#endif
...
//		const PxReal sampleHeight = hfShape.getHeightAtShapePoint(sphereInHfShape.x, sphereInHfShape.z);
		...
		if (hf.isDeltaHeightInsideExtent(deltaHeight, eps))
		{
			...
			if (faceIndex != 0xffffffff)
			{
				...
				if (hf.getThicknessFast() <= 0)
					contactBuffer.contact(transform0.p,  hfShapeUp,  deltaHeight-radius, PXC_CONTACT_NO_FACE_INDEX, faceIndex);
				else
					contactBuffer.contact(transform0.p, -hfShapeUp, -deltaHeight-radius, PXC_CONTACT_NO_FACE_INDEX, faceIndex);
	...
	const PxReal inflatedRadius = radius + contactDistance;
...
#define HFU Gu::HeightFieldUtil
				...
				for(PxU32 pi = 0; pi < npcp; pi++)
				{
					...
					if (hf.isDeltaHeightOppositeExtent(d.y)) // See if we are 'above' the heightfield
					{
						...
						} else if(featureType == HFU::eVERTEX)
						{
							...
							{
								if (contactBuffer.contacts[iPrevEdgeContact].internalFaceIndex0 != HFU::eEDGE)
						...
						// temporarily use the internalFaceIndex0 slot in the contact buffer for featureType
	 					contactBuffer.contact(
							p, n, dMag - radius, featureType, HFU::getFeatureIndex(closestFeatures[pi]));	
					}
				}
			}
		}
	...
	if (!isCapsule) // keep the face labels as internalFaceIndex0 if this function is called from inside capsule/HF function
		for (PxU32 k = 0; k < numFaceContacts; k++)
			contactBuffer.contacts[k].internalFaceIndex0 = PXC_CONTACT_NO_FACE_INDEX;

	for (PxU32 k = numFaceContacts; k < contactBuffer.count; k++)
	{
		PX_ASSERT(contactBuffer.contacts[k].internalFaceIndex0 != HFU::eFACE);
		PX_ASSERT(contactBuffer.contacts[k].internalFaceIndex0 <= HFU::eVERTEX);
		PxU32 featureIndex = contactBuffer.contacts[k].internalFaceIndex1;
		if (contactBuffer.contacts[k].internalFaceIndex0 == HFU::eEDGE)
			contactBuffer.contacts[k].internalFaceIndex1 = hfUtil.getEdgeFaceIndex(featureIndex);
		else if (contactBuffer.contacts[k].internalFaceIndex0 == HFU::eVERTEX)
		{
...
{
	return PxcContactSphereHeightFieldShared(shape0, shape1, transform0, transform1, contactDistance, npCache, contactBuffer, false);
}

bool PxcContactSphereHeightFieldUnified(CONTACT_METHOD_ARGS)
{
	return Gu::contactSphereHeightField(shape0, shape1, transform0, transform1, contactDistance, (Gu::Cache&)npCache, contactBuffer);
}
}
