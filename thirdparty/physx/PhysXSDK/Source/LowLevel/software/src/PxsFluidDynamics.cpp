...
#include "PxsFluidDynamicHelper.h"
...
PX_FORCE_INLINE void PxsFluidDynamics::updateParticlesBruteForceHalo(PxsSphUpdateType updateType, PxVec3* forceBuf, PxsFluidParticle* particles,
																	 ...
																	 PxsFluidDynamicsTempBuffers& tempBuffers)
{
	for(PxU32 i=0; i < 26; i++)
	{
		...
		for(PxU32 j=0; j < numHaloRegions; j++)
		{
			...
			if (mergedIndexCount + haloRegions.numParticles[idx] > PXS_FLUID_SUBPACKET_PARTICLE_LIMIT_FORCE_DENSITY)
			{
				updateParticleGroupPair(forceBufA, forceBuf, particlesA, particles,
					tempBuffers.orderedIndicesSubpacket, packetSections.numParticles[i],
...
PxsFluidDynamics::SectionToHaloTable PxsFluidDynamics::sSectionToHaloTable[26] =
	{
...
PxsFluidDynamics::OrderedIndexTable PxsFluidDynamics::sOrderedIndexTable;

PxsFluidDynamics::OrderedIndexTable::OrderedIndexTable()
{
	for (PxU32 i = 0; i < PXS_FLUID_SUBPACKET_PARTICLE_LIMIT_FORCE_DENSITY; ++i)
		indices[i] = i;
}

class PxsFluidDynamicsSphTask : public Cm::Task
{
public:
	PxsFluidDynamicsSphTask(PxsFluidDynamics& context, PxU32 taskDataIndex) :
		mDynamicsContext(context),
	...
	{
		return "PxsFluidDynamics.sph";
	}

private:
	PxsFluidDynamicsSphTask& operator=(const PxsFluidDynamicsSphTask&);
	PxsFluidDynamics& mDynamicsContext;
	PxU32 mTaskDataIndex;
};

PxsFluidDynamics::PxsFluidDynamics(PxsParticleSystemSim& particleSystem) : 
	mParticleSystem(particleSystem),
	mTempReorderedParticles(NULL),
	mTempParticleForceBuf(NULL),
	mMergeDensityTask(this, "PxsFluidDynamics.mergeDensity"),
	mMergeForceTask(this, "PxsFluidDynamics.mergeForce"),
	mNumTempBuffers(0)
{
}

PxsFluidDynamics::~PxsFluidDynamics()
{
}

//-------------------------------------------------------------------------------------------------------------------//

void PxsFluidDynamics::init(bool /*isSph*/)
{
#ifdef PX_PS3
...
void PxsFluidDynamics::mergeForce(PxBaseTask* /*continuation*/)
{
	PxU32 numParticles = mParticleSystem.mNumPacketParticlesIndices;
	PxsFluidParticle* particles = mParticleSystem.mParticleState->getParticleBuffer();
	...
	{
		PxU32 particleIndex = particleIndices[i];
		PxsFluidParticle& particle = mTempReorderedParticles[i];
...
void PxsFluidDynamics::updateSubpacketPairHalo(PxVec3* __restrict forceBufA,
											   PxsFluidParticle* __restrict particlesSpA,
											   PxU32 numParticlesSpA, 
											   PxsParticleCell* __restrict particleCellsSpA,
											   PxU32* __restrict particleIndicesSpA, 
											   bool& isLocalHashSpAValid, 
											   PxU32 numCellHashBucketsSpA,											 
											   PxsFluidParticle* __restrict particlesSpB, 
											   PxU32 numParticlesSpB, 
											   PxsParticleCell* __restrict particleCellsSpB,
											   PxU32* __restrict particleIndicesSpB, 
											   const PxVec3& packetCorner, 
											   PxsSphUpdateType updateType, 
											   PxU16* __restrict hashKeyArray, 
											   PxsFluidDynamicsTempBuffers& tempBuffers)
{
	bool sectionEnablesBruteForce = (numParticlesSpA <= PXS_FLUID_BRUTE_FORCE_PARTICLE_THRESHOLD_HALO_VS_SECTION);
	bool haloRegionEnablesBruteForce = (numParticlesSpB <= PXS_FLUID_BRUTE_FORCE_PARTICLE_THRESHOLD_HALO_VS_SECTION);
	...
	if (sectionEnablesBruteForce && haloRegionEnablesBruteForce)
	{
		...
		updateParticleGroupPair(forceBufA, NULL, particlesSpA, particlesSpB,
			tempBuffers.orderedIndicesSubpacket, numParticlesSpA,
			tempBuffers.orderedIndicesSubpacket, numParticlesSpB,
			false, updateType == PXS_SPH_DENSITY,
			mParams, tempBuffers.simdPositionsSubpacket, tempBuffers.indexStream);	
	}
	else
	{
		if (!isLocalHashSpAValid)
		{
			// Get local cell hash for the current subpacket
			PxsFluidSpatialHash::buildLocalHash(particlesSpA, numParticlesSpA, particleCellsSpA,
				particleIndicesSpA, hashKeyArray, numCellHashBucketsSpA, mParams.cellSizeInv, packetCorner);
		...
		// Get local cell hash for other subpacket
		PxsFluidSpatialHash::buildLocalHash(particlesSpB, numParticlesSpB, particleCellsSpB,
			particleIndicesSpB, hashKeyArray, numCellHashBucketsSpB, mParams.cellSizeInv, packetCorner);
