...
*/
void PxsFluidSpatialHash::buildLocalHash(const PxsFluidParticle* particles, PxU32 numParticles, PxsParticleCell* cells,
										 PxU32* particleIndices, PxU16* hashKeyArray, PxU32 numHashBuckets, PxF32 cellSizeInv, const PxVec3& packetCorner)
{
	...
	PX_ASSERT(numHashBuckets > numParticles);	// Needs to be larger to have at least one empty hash bucket (required to detect invalid cells).
	...
	const PxsFluidParticle* prt0 = particles;
	const PxsFluidParticle* prt1 = (1 < numParticles) ? particles + 1 : &fakeParticle;
	const PxsFluidParticle* prt2 = (2 < numParticles) ? particles + 2 : &fakeParticle;
	const PxsFluidParticle* prt3 = (3 < numParticles) ? particles + 3 : &fakeParticle;
	...
	{
		const PxsFluidParticle* prt0_N = (p + 4 < numParticles) ? particles + p + 4 : &fakeParticle;
		const PxsFluidParticle* prt1_N = (p + 5 < numParticles) ? particles + p + 5 : &fakeParticle;
		const PxsFluidParticle* prt2_N = (p + 6 < numParticles) ? particles + p + 6 : &fakeParticle;
		const PxsFluidParticle* prt3_N = (p + 7 < numParticles) ? particles + p + 7 : &fakeParticle;
		...
		for (PxU32 i = 0; i < 4; ++i)
		{
			if (p + i < numParticles)
			{
				...
				const PxcGridCellVector cellCoord(PxI16(int32Vec3.x), PxI16(int32Vec3.y), PxI16(int32Vec3.z)); 
				PxU32 hashKey = getCellIndex(cellCoord, cells, numHashBuckets);
				PX_ASSERT(hashKey < PXS_PARTICLE_SYSTEM_HASH_KEY_LIMIT);
				PxsParticleCell* cell = &cells[hashKey];
...
*/
void PxsFluidSpatialHash::reorderParticleIndicesToCells(const PxsFluidParticle* /*particles*/,
														PxU32 numParticles, PxsParticleCell* cells,
