...
void collideWithCapsuleNonContinuous(PxsParticleCollData& collData, const PxVec3& q,
									 const PxReal& h, const PxReal& r,
...
void collideWithCapsuleTestSphere(PxsParticleCollData& collData, const PxVec3& p,
								  const PxVec3& q, const PxVec3& d,
								  const PxReal& h, const PxReal& r,
								  const PxReal& sphereH, const PxReal& discS,
								  const PxReal& aS, const PxReal& bS,
								  const PxReal& proxRadius)
{
	...
	else
	{
		...
		else if(t < collData.ccTime)
		{
			...
			collData.localSurfaceNormal.x -= sphereH;
			collData.localSurfaceNormal *= (1.0f / r);
			computeContinuousTargetPosition(collData.localSurfacePos, p, relativePOSITION, collData.localSurfaceNormal, collData.restOffset);
			collData.ccTime = t;  
...
//      
// ----------------------------------------------------------------
PX_FORCE_INLINE void collideWithCapsule(PxsParticleCollData& collData, const PxCapsuleGeometry& capsuleShapeData, PxReal proxRadius)
{
...
void collideWithCapsule(PxsParticleCollData* collShapeData, PxU32 numCollData, const Gu::GeometryUnion& capsuleShape, PxReal proxRadius)
{
