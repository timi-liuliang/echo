...
namespace physx
{
	...
	void collideWithConvexPlanesSIMD(PxsParticleCollDataV4& collDataV4, const PxPlane* convexPlanes, PxU32 numPlanes, const PxReal proxRadius)
	{
		...
		Vec4V localNewPos0 = V4LoadA((const PxF32*)&collDataV4.localNewPos[0]);
		Vec4V localOldPos0 = V4LoadA((const PxF32*)&collDataV4.localOldPos[0]);

		Vec4V localNewPos1 = V4LoadA((const PxF32*)&collDataV4.localNewPos[1]);
		Vec4V localOldPos1 = V4LoadA((const PxF32*)&collDataV4.localOldPos[1]);

		Vec4V localNewPos2 = V4LoadA((const PxF32*)&collDataV4.localNewPos[2]);
		Vec4V localOldPos2 = V4LoadA((const PxF32*)&collDataV4.localOldPos[2]);

		Vec4V localNewPos3 = V4LoadA((const PxF32*)&collDataV4.localNewPos[3]);
		Vec4V localOldPos3 = V4LoadA((const PxF32*)&collDataV4.localOldPos[3]);
		...
		VecU32V localFlags = VecU32VLoadXYZW(collDataV4.localFlags[0], collDataV4.localFlags[1], collDataV4.localFlags[2], collDataV4.localFlags[3]);
		...
		const VecU32V flagCC = VecU32VLoadXYZW(PXS_FLUID_COLL_FLAG_CC, PXS_FLUID_COLL_FLAG_CC, PXS_FLUID_COLL_FLAG_CC, PXS_FLUID_COLL_FLAG_CC);
		const BoolV noFlagCC = V4IsEqU32(V4U32and(flagCC, localFlags), u4Zero);

		//proximity
		const VecU32V flagLPROX = VecU32VLoadXYZW(PXS_FLUID_COLL_FLAG_L_PROX, PXS_FLUID_COLL_FLAG_L_PROX, PXS_FLUID_COLL_FLAG_L_PROX, PXS_FLUID_COLL_FLAG_L_PROX);
		const BoolV proximityV = BAnd(BAnd(BAnd(noFlagCC, V4IsGrtrOrEq(newPosClosestDist, V4Zero())), V4IsGrtr(proxRadiusV4, newPosClosestDist)), BNot(newPosOutMask));
		VecU32V stateFlag = V4U32Sel(proximityV, flagLPROX, u4Zero);

		//discrete
		const VecU32V flagLDC = VecU32VLoadXYZW(PXS_FLUID_COLL_FLAG_L_DC, PXS_FLUID_COLL_FLAG_L_DC, PXS_FLUID_COLL_FLAG_L_DC, PXS_FLUID_COLL_FLAG_L_DC);
		const BoolV DCV = BAnd(BAnd(noFlagCC, V4IsGrtrOrEq(newPosClosestDist, V4Zero())), V4IsGrtr(restOffsetV4, newPosClosestDist));
		stateFlag = V4U32or(stateFlag, V4U32Sel(DCV, flagLDC, u4Zero));	

		//cc
		const VecU32V flagLCC = VecU32VLoadXYZW(PXS_FLUID_COLL_FLAG_L_CC, PXS_FLUID_COLL_FLAG_L_CC, PXS_FLUID_COLL_FLAG_L_CC, PXS_FLUID_COLL_FLAG_L_CC);

		Vec4V oldCCTimeV = V4LoadA(collDataV4.ccTime); 
		const BoolV ccHappenedV = BAnd(BAnd(V4IsGrtrOrEq(latestEntry, V4Zero()), V4IsGrtr(oldCCTimeV,latestEntry)) , V4IsGrtrOrEq(soonestExit, latestEntry));
		...
		//store
		V4StoreA(localSurfacePos0, (PxF32*)&collDataV4.localSurfacePos[0]);
		V4StoreA(localSurfacePos1, (PxF32*)&collDataV4.localSurfacePos[1]);
		V4StoreA(localSurfacePos2, (PxF32*)&collDataV4.localSurfacePos[2]);
		V4StoreA(localSurfacePos3, (PxF32*)&collDataV4.localSurfacePos[3]);

		V4StoreA(localSurfaceNormal0, (PxF32*)&collDataV4.localSurfaceNormal[0]);
		V4StoreA(localSurfaceNormal1, (PxF32*)&collDataV4.localSurfaceNormal[1]);
		V4StoreA(localSurfaceNormal2, (PxF32*)&collDataV4.localSurfaceNormal[2]);
		V4StoreA(localSurfaceNormal3, (PxF32*)&collDataV4.localSurfaceNormal[3]);

		V4StoreA(ccTimeV, collDataV4.ccTime);

		V4U32StoreAligned(stateFlag, (VecU32V*)collDataV4.localFlags);
	}
}

void physx::collideWithConvexPlanes(PxsParticleCollData& collData, const PxPlane* convexPlanes, PxU32 numPlanes, const PxReal proxRadius)
{
	PX_ASSERT(convexPlanes);
	...
	if(isContained)
	{
		...
		collData.localSurfacePos = collData.localOldPos + plane[oldPosPlaneIndex].n * (collData.restOffset - oldPosClosestDist);
	}
	else
	{
		...
		{			
			...
			//collData.localSurfacePos = collData.localOldPos + (motion * latestEntry) + (continuousNormal * collData.restOffset);
			computeContinuousTargetPosition(collData.localSurfacePos, collData.localOldPos, motion * latestEntry, plane[rayPlaneIndex].n, collData.restOffset);
		...
		else if (!(collData.localFlags & PXS_FLUID_COLL_FLAG_CC))
		{
			...
			if (isProximity) // proximity
			{
				...
				collData.localSurfacePos = collData.localNewPos + plane[newPosPlaneIndex].n * (collData.restOffset - newPosClosestDist);
