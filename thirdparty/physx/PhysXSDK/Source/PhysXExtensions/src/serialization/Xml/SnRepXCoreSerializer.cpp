...
namespace physx { 
	typedef ProfileHashMap< const PxSerialObjectId, const PxArticulationLink* > TArticulationLinkLinkMap;
	...
	{
		FoundationWrapper wrapper( inAllocator.getAllocator() );
		...
		return ret ? createRepXObject(theShape) : PxRepXObject();
	}
	...
	void PxTriangleMeshRepXSerializer::objectToFileImpl( const PxTriangleMesh* mesh, PxCollection* /*inCollection*/, XmlWriter& inWriter, MemoryBuffer& inTempBuffer, PxRepXInstantiationArgs& inArgs )
	{
		...
		bool isU16 = mesh->getTriangleMeshFlags() & PxTriangleMeshFlag::eHAS_16BIT_TRIANGLE_INDICES ? true : false;
		...
		if(inArgs.cooker != NULL)
		{
			...
			writeBuffer( inWriter, inTempBuffer, 16, (PxU8*)theTempBuf.mBuffer, theTempBuf.mWriteOffset, "CookedData", writeDatatype<PxU8> );
		}

		delete []meshDesc.materialIndices.data;
	}

	PxRepXObject PxTriangleMeshRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* /*inCollection*/ )
	{
	...
	PxRepXObject PxHeightFieldRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		PxHeightField* retval = inArgs.physics.createHeightField( theDesc );
	...
	PxRepXObject PxAggregateRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		return ret ? createRepXObject(theAggregate) : PxRepXObject();		
	}
	...
	PxRepXObject PxClothFabricRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* /*inCollection*/)
	{
		...
		readStridedBufferProperty<PxClothFabricPhase>( inReader, "Phases", const_cast<void*&>((const void*&)desc.phases), strideIgnored, desc.nbPhases, inAllocator );

		PxU32 numRestvalues = 0;
		readStridedBufferProperty<PxF32>( inReader, "Restvalues", const_cast<void*&>((const void*&)desc.restvalues), strideIgnored, numRestvalues, inAllocator );

		readStridedBufferProperty<PxU32>( inReader, "Sets", const_cast<void*&>((const void*&)desc.sets), strideIgnored, desc.nbSets, inAllocator );

		PxU32 numIndices = 0;
		readStridedBufferProperty<PxU32>( inReader, "ParticleIndices", const_cast<void*&>((const void*&)desc.indices), strideIgnored, numIndices, inAllocator );

		readStridedBufferProperty<PxU32>( inReader, "TetherAnchors", const_cast<void*&>((const void*&)desc.tetherAnchors), strideIgnored, desc.nbTethers, inAllocator );
		readStridedBufferProperty<PxU32>( inReader, "TetherLengths", const_cast<void*&>((const void*&)desc.tetherLengths), strideIgnored, desc.nbTethers, inAllocator );
	...
	void PxClothRepXSerializer::objectToFileImpl( const PxCloth* data, PxCollection* inCollection, XmlWriter& inWriter, MemoryBuffer& inTempBuffer, PxRepXInstantiationArgs& /*inArgs*/)
	{
		...
		ProfileArray<PxU8> dataBuffer( inTempBuffer.mManager->getWrapper() );
	...
	PxRepXObject PxClothRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		void* virtualParticleWeights = NULL;
		...
		readStridedBufferProperty<PxClothCollisionSphere>( inReader, "CollisionSpheres", (void*&)spheres, strideIgnored, numSpheres, inAllocator );
		readStridedBufferProperty<PxU32>( inReader, "CollisionSpherePairs", (void*&)capsules, strideIgnored, numCapsules, inAllocator );
		readStridedBufferProperty<PxClothCollisionPlane>( inReader, "CollisionPlanes", (void*&)planes, strideIgnored, numPlanes, inAllocator );
		readStridedBufferProperty<PxU32>( inReader, "CollisionConvexMasks", (void*&)convexMasks, strideIgnored, numConvexMasks, inAllocator );
		readStridedBufferProperty<PxClothCollisionTriangle>( inReader, "CollisionTriangles", (void*&)triangles, strideIgnored, numTriangles, inAllocator );

		readFlagsProperty( inReader, inAllocator, "ClothFlags", PxEnumTraits<PxClothFlag::Enum>().NameConversion, flags );

		readStridedBufferProperty<PxClothParticle>( inReader, "ParticleAccelerations", (void*&)particleAccelerations, strideIgnored, numParticleAccelerations, inAllocator );
		readStridedBufferProperty<PxU32>( inReader, "SelfCollisionIndices", (void*&)selfCollisionIndices, strideIgnored, numSelfCollisionIndices, inAllocator );
		readStridedBufferProperty<PxClothParticle>( inReader, "MotionConstraints", (void*&)particleMotionConstraint, strideIgnored, numParticleMotionConstraint, inAllocator );
		readStridedBufferProperty<PxClothParticle>( inReader, "SeparationConstraints", (void*&)separationConstraints, strideIgnored, numSeparationConstraints, inAllocator );
		readStridedBufferProperty<PxClothParticle>( inReader, "RestPositions", (void*&)restPositions, strideIgnored, numRestPositions, inAllocator );
		...
		return ret ? createRepXObject(cloth) : PxRepXObject();
	}
	...
	PxRepXObject PxParticleRepXSerializer<TParticleType>::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		if (validParticleRange > 0)
		{
			...
			return ret ? createRepXObject(theParticle) : PxRepXObject();
		}
		else
		{
