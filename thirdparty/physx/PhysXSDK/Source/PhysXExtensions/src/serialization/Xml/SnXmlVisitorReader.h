...
namespace physx { namespace Sn {
	...
	inline TObjType* findReferencedObject( PxCollection& collection, PxSerialObjectId id)
	{
		...
		if (outObject == NULL)
		{
			Ps::getFoundation().error(PxErrorCode::eINVALID_PARAMETER, __FILE__, __LINE__, 
				...
				"Reference to %s with ID %d cannot be resolved. Make sure externalRefs collection is specified if required and "
				"check Xml file for completeness.",
				PxTypeInfo<TObjType>::name(),
				id);
		}
		return outObject;
	}
	...
	inline bool readAllProperties( PxRepXInstantiationArgs args, TReaderNameStack& names, ProfileArray<PxU32>& contexts, XmlReader& reader, TObjType* obj, XmlMemoryAllocator& alloc, PxCollection& collection, TInfoType& info )
	{
		bool hadError = false;
