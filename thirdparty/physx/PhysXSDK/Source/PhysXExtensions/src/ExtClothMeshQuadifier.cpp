...
namespace 
{
	...
	void copyIndices(const PxClothMeshDesc &desc, shdfnd::Array<PxU32> &triangles, shdfnd::Array<PxU32> &quads)
	{
		...
		PxStrideIterator<const T> tIt = PxMakeIterator((const T*)desc.triangles.data, desc.triangles.stride);
		for(PxU32 i=0; i<desc.triangles.count; ++i, ++tIt)
			for(PxU32 j=0; j<3; ++j)
				triangles[i*3+j] = tIt.ptr()[j];

		quads.resize(desc.quads.count*4);
		PxStrideIterator<const T> qIt = PxMakeIterator((const T*)desc.quads.data, desc.quads.stride);
		...
		for (PxU32 i = 0; i < uniqueEdges.size(); i++)
		{
			...
			if (uniqueEdge.vertex3 != 0xffffffff)
			{
				...
				for (PxU32 j = 0; j < 4; j++)
				{
					...
					PxReal denominator = e0.magnitude() * e1.magnitude();
					if (denominator != 0.0f)
					{
						PxF32 cosAngle = PxAbs(e0.dot(e1)) / denominator;
