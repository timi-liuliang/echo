...
namespace Gu
{
	...
	template<class Element, PxU32 Capacity, class Comparator = Ps::Less<Element> >

	class PriorityQueue : protected Comparator // inherit so that stateless comparators take no space
	{
	public:
		PriorityQueue(const Comparator& less = Comparator()) : Comparator(less), mHeapSize(0)
		{
		}
		...
		{
			PX_ASSERT((PxU32)mHeapSize < Capacity);
			PxU32 newIndex;
			PxU32 parentIndex = parent(mHeapSize);

			for (newIndex = mHeapSize; newIndex > 0 && compare(value, mData[parentIndex]); newIndex = parentIndex, parentIndex= parent(newIndex)) 
			{
				mData[ newIndex ] = mData[parentIndex];
			}
			mData[newIndex] = value; 
			mHeapSize++;
			PX_ASSERT(valid());
		}
		...
		PX_FORCE_INLINE Element pop()
		{
			...
			for (i = 0; (child = left(i)) < tempHs; i = child) 
			{
				...
				child += ((rightChild < tempHs) & compare((mData[rightChild]), (mData[child]))) ? 1 : 0;
		...
		bool valid() const
		{
			const Element& min = mData[0];
			for(PxU32 i=1; i<mHeapSize; ++i)
			{
				if(compare(mData[i], min))
					return false;
			}
	...
	public:		
		...
		bool expandPoint(EPASupportMapPair* pair, PxI32& numVerts, const FloatVArg lowerBound, const FloatVArg upperBound);
		bool expandSegment(EPASupportMapPair* pair, PxI32& numVerts, const FloatVArg lowerBound, const FloatVArg upperBound);
		...
		PriorityQueue<Facet*, MaxFacets, FacetDistanceComparator> heap;
	...
	static PX_FORCE_INLINE void doSupport(EPASupportMapPair* pair, const Ps::aos::Vec3VArg dir, Ps::aos::Vec3V& supportA, Ps::aos::Vec3V& supportB, Ps::aos::Vec3V& support)
	{
		pair->doSupport(dir, supportA, supportB, support);
	}
	...
	PxGJKStatus epaPenetration(const ConvexV& a, const ConvexV& b, EPASupportMapPair* pair, PxU8* PX_RESTRICT aInd, PxU8* PX_RESTRICT bInd, PxU8 _size, Ps::aos::Vec3V& contactA, Ps::aos::Vec3V& contactB, Ps::aos::Vec3V& normal, Ps::aos::FloatV& penetrationDepth, const bool takeCoreShape)
	{
		...
		while(BAllEq(bNotTerminated, bTrue))
		{
			...
			v = GJKCPairDoSimplex(Q, A, B, Q[index], size);
			sDist = V3Dot(v, v);
	...
	bool EPA::expandPoint(EPASupportMapPair* pair, PxI32& numVerts, const FloatVArg lowerBound, const FloatVArg upperBound)
	{
		const Vec3V x = V3UnitX();
		Vec3V q0;
		doSupport(pair, x, aBuf[1], bBuf[1], q0);
		return expandSegment(pair, numVerts, lowerBound, upperBound);
	}

	//ML: this function use the segement to create a triangle
	bool EPA::expandSegment(EPASupportMapPair* pair, PxI32& numVerts, const FloatVArg lowerBound, const FloatVArg upperBound)
	{
		...
		Vec3V q0;
		doSupport(pair, aux1, aBuf[0], bBuf[0], q0);
		...
		Vec3V q1;
		doSupport(pair, aux2, aBuf[1], bBuf[1], q1);
		
		const Vec3V aux3 = V3Normalize(QuatRotate(qua0, aux2));//(aux2 * qua0);
	
		Vec3V q2;
		doSupport(pair, aux3, aBuf[2], bBuf[2], q2);
	...
	static void calculateContactInformation(const Ps::aos::Vec3V* PX_RESTRICT aBuf, const Ps::aos::Vec3V* PX_RESTRICT bBuf, const Facet* facet, const ConvexV& a, const ConvexV& b, Vec3V& pa, Vec3V& pb, Vec3V& normal, FloatV& penDepth, const bool takeCoreShape)
	{
		FloatV zero = FZero();
		Vec3V _pa, _pb;
	...
	PxGJKStatus EPA::PenetrationDepth(const ConvexV& a, const ConvexV& b, EPASupportMapPair* pair, const Ps::aos::Vec3V* PX_RESTRICT /*Q*/, const Ps::aos::Vec3V* PX_RESTRICT A, const Ps::aos::Vec3V* PX_RESTRICT B, const PxI32 size, Ps::aos::Vec3V& pa, Ps::aos::Vec3V& pb, Ps::aos::Vec3V& normal, Ps::aos::FloatV& penDepth, const bool takeCoreShape)
	{
		...
		case 1:
			{
				...
				if(!expandPoint(pair, numVertsLocal, lower_bound, upper_bound))
					return EPA_FAIL;
				break;
			}
		case 2: 
			{
				...
				if(!expandSegment(pair, numVertsLocal, lower_bound, upper_bound))
					return EPA_FAIL;
				break;
			}
		case 3: 
		...
		do 
		{
			...
			if (!facet->isObsolete()) 
			{
				...
				pair->doSupport(V3Neg(planeNormal), tempa, tempb, q);
				...
				if(BAllEq(degeneratedCondition, bTrue))
				{
					calculateContactInformation(aBuf, bBuf, facet, a, b, pa, pb, normal, penDepth, takeCoreShape);
