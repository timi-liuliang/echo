...
bool sweepCCTCapsule_BoxGeom(GU_CAPSULE_SWEEP_FUNC_PARAMS)
{
	...
	if (lss.p0 == lss.p1)  // The capsule is actually a sphere
	{
		...
		Box box;	buildFrom1(box, pose.p, boxGeom.halfExtents, pose.q);
	...
	else
	{
		...
		if((hintFlags & PxHitFlag::ePOSITION) && sweepHit.distance!=0.0f)
		{
			// The sweep test doesn't compute the impact point automatically, so we have to do it here.
...
bool sweepCCTBox_SphereGeom(GU_BOX_SWEEP_FUNC_PARAMS)
{
...
bool sweepCCTBox_CapsuleGeom(GU_BOX_SWEEP_FUNC_PARAMS)
{
	...
	Box relBox(box.center - pose.p, box.extents, box.rot);

	PxVec3 n;
	const PxVec3 negDir = -unitDir;

	Capsule capsule;
	getCapsule(capsule, capsuleGeom, relPose);

	const PxTransform boxWorldPose = relBox.getTransform();

	if(!sweepCapsuleBox(capsule, boxWorldPose, relBox.extents, negDir, distance, sweepHit.position, sweepHit.distance, n, hintFlags))
		return false;
...
bool sweepCCTBox_BoxGeom(GU_BOX_SWEEP_FUNC_PARAMS)
{
	PX_ASSERT(geom.getType() == PxGeometryType::eBOX);
	PX_UNUSED(inflation);
	...
	const Box relBox(box.center - pose.p, box.extents, box.rot);
	Box staticBox;	buildFrom1(staticBox, PxVec3(0), boxGeom.halfExtents, pose.q);
...
bool sweepCCTBox_HeightFieldGeom(GU_BOX_SWEEP_FUNC_PARAMS)
{
	...
	struct LocalReport : EntityReport<PxU32>
	{
		virtual bool onEvent(PxU32 nb, PxU32* indices)
		{
			for(PxU32 i=0; i<nb; i++)
			{
				...
				const bool b = sweepBoxVsTriangles(1, &currentTriangle, mBox, mDir, mDist, sweepHit_, mHintFlags, mIsDoubleSided, NULL);
	...
	MPT_SET_CONTEXT("scbh", pose, PxMeshScale);
	hfUtil.overlapAABBTriangles(pose, bounds, PxHfQueryFlags::eWORLD_SPACE, &myReport);

	return myReport.mStatus;
}

bool Gu::sweepCCTBoxTriangles(	PxU32 nbTris, const PxTriangle* triangles, bool doubleSided,
								...
								PxVec3& _normal, float& _d, PxU32& _index, const PxU32* cachedIndex, const PxReal inflation, PxHitFlags hintFlags)
{
	...
	PxSweepHit sweepHit;
	const bool status = sweepBoxVsTriangles(nbTris, triangles, box, dir, length, sweepHit, hintFlags, doubleSided, cachedIndex);
	if(status)
	{
