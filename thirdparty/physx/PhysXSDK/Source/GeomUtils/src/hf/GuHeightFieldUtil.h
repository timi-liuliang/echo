...
namespace Gu
{
	...
	PX_PHYSX_COMMON_API void writeTrace(
		const char* tag, const PxTransform& xf, const PxMeshScale& scale,
		const Ps::Array<PxU32>& touchedFaces,
		bool newMesh, PxU32 meshId, const PxVec3* verts, PxU32 nbVerts, const void* inds, PxU32 nbInds, PxU32 has16bitIndices,
		const PxVec3* orig, const PxVec3* dir, const PxReal* maxT, PxU32 bothSides, const PxVec3* inflate, const Box* obb, const PxHeightFieldGeometry* hfg);
	#endif
	...
	private:
		...
		PX_CUDA_CALLABLE PX_FORCE_INLINE void initialize()
		{
            ...
            PX_ASSERT(sizeof(((PxHeightFieldSample *)0)->height) == 2);
...
/*			return ((!(x < 0))
		...
		public:
			...
			OverlapTraceSegment(const HeightFieldUtil& hfUtil,const Gu::HeightField& hf)
			  : mInitialized(false), mHfUtil(hfUtil), mHf(hf), mIndexBufferUsed(0), mFaces(NULL)
			{}
			...
			PX_INLINE bool init(
				const PxI32 ui, const PxI32 vi, const PxI32 nbVi, const PxI32 step_ui, const PxI32 step_vi, T* aCallback, Ps::Array<PxU32>* aFaces = NULL)
			{
				PX_UNUSED(aFaces);
				#if MIDPHASE_TRACE
				mFaces = aFaces;
...
#define ISHOLE1 (mHf.getMaterialIndex1(vertexIndex) == PxHeightFieldMaterial::eHOLE)				
				PxReal h0 = mHf.getHeight(vertexIndex);
				PxReal h1 = mHf.getHeight(vertexIndex + 1);
				PxReal h2 = mHf.getHeight(vertexIndex + mNumColumns);
				PxReal h3 = mHf.getHeight(vertexIndex + mNumColumns + 1);
...
#undef ISHOLE1
			...
			{
				if(mIndexBufferUsed == MAX_BUFFER_SIZE)
				{
					if(!reportOverlaps())
						return false;					
				}

				mIndexBuffer[mIndexBufferUsed] = triangleIndex;
				mIndexBufferUsed++;
				#if MIDPHASE_TRACE
				mFaces->pushBack(triangleIndex);
				#endif
				return true;
			}
			...
			bool reportOverlaps()
			{
				if (!mCallback->onEvent(mIndexBufferUsed,mIndexBuffer))
					return false;
				mIndexBufferUsed = 0;
				return true;
			}
		...
		private:
			static const PxU32 MAX_BUFFER_SIZE = 16;
			...
			PxU32					mIndexBuffer[MAX_BUFFER_SIZE];
			PxU32					mIndexBufferUsed;
			Ps::Array<PxU32>*		mFaces;
		};
		...
		#if MIDPHASE_TRACE
		void generateMesh(PxU32 nbCols, PxU32 nbRows, Ps::Array<PxVec3>& verts, Ps::Array<PxU32>& inds, PxU32& numVerts, PxU32& numInds) const;
		#endif

		template<class T, bool useUnderFaceCallback, bool backfaceCull, bool overlap>
		PX_INLINE void traceSegment(const PxVec3& aP0, const PxVec3& aP1, T* aCallback, const PxVec3* overlapObjectExtent = NULL) const
		{
			//pxPrintf("traceSegment\n");
			//pxPrintf("aP0 = %.12f %.12f %.12f; aP1 = %.12f %.12f %.12f\n", aP0.x, aP0.y, aP0.z, aP1.x, aP1.y, aP1.z);
			//pxPrintf("numCols=%d, numRows=%d, mOneOverColumnScale=%.12f, mOneOverRowScale=%.12f\n",
			//	mHeightField->getNbColumnsFast(), mHeightField->getNbRowsFast(), mOneOverColumnScale, mOneOverRowScale);
			PxBounds3 localBounds;
			computeLocalBounds(localBounds);
			// AP: needed to register a hit against a HF with 0 thickness, 3.4 only
			//localBounds.minimum.y -= GU_MIN_AABB_EXTENT; // see comments for GU_MIN_AABB_EXTENT

			if(overlap)
			{
				// extend the bounds by overlap object bounds, now we can get even negative values for u|v, this is correct, since we have to test 
				// the bounded ray which can overlap with the HF				
				localBounds.minimum = localBounds.minimum - *overlapObjectExtent;
				localBounds.maximum = localBounds.maximum + *overlapObjectExtent;
			}
			...
			if(overlap)
			{
				...
				overlapTraceSegment.prepare(aP0,aP1,*overlapObjectExtent,expandu,expandv);
			}

			#if MIDPHASE_TRACE
			Ps::Array<PxU32> faces;
			#endif
			...
			u0 = PxMin(PxMax(p0.x * mOneOverRowScale, 1e-7f - expandu), nbUcells + expandu); // multiplication rescales the u,v grid steps to 1
			v0 = PxMin(PxMax(p0.z * mOneOverColumnScale, 1e-7f - expandv), nbVcells + expandv);
			const PxReal h0 = p0.y; // we don't scale y

			// map p1 from (x, z, y) to (u1, v1, h1)
			u1 = PxMin(PxMax(p1.x * mOneOverRowScale, 1e-7f - expandu), nbUcells + expandu);
			v1 = PxMin(PxMax(p1.z * mOneOverColumnScale, 1e-7f - expandv), nbVcells + expandv);
			...
			//pxPrintf(
			//	"u0=%.12f, v0=%.12f, h0=%.12f, u1=%.12f, v1=%.12f, h1=%.12f, step_ui=%.2f, step_vi=%.2f\n",
			//	u0, v0, h0, u1, v1, h1, step_uif, step_vif);
			...
			do
			{
				...
				if(overlap)
				{
					if(!overlapTraceSegment.initialized())
					{
						...
						#if MIDPHASE_TRACE
						if(!overlapTraceSegment.init(ui,vi,nbVi,step_ui,step_vi,aCallback, &faces))
							goto onReturn;
						#else
				...
				//pxPrintf("here1 ui=%d vi=%d\n", ui, vi);
				//pxPrintf("h4=%.12f %.12f %.12f %.12f, minH=%.12f, maxH=%.12f, hPrev=%.12f, hNext=%.12f\n",
				//	h[0], h[1], h[2], h[3], minH, maxH, hLinePrev, hLineNext);
					...
					(useUnderFaceCallback && hLineCellRangeMax < maxH))
				{
					...
					{
						// AP: roll back to 3.3 legacy behavior, keep code around for 3.4
						//if (mHeightField->getThicknessFast() > 0.0f) // flip triangle winding if thickness is positive
						//	Ps::swap<const PxVec3*>(p00a, p11a);

						//pxPrintf("here3 cull=%d\n", backfaceCull);
						...
						//pxPrintf("uhv0=%.12f %.12f %.12f, duhv=%.12f %.12f %.12f\n", uhv0.x, uhv0.y, uhv0.z, duhv.x, duhv.y, duhv.z);
						//pxPrintf("p00a=%.12f %.12f %.12f, p01a=%.12f %.12f %.12f\n", p00a->x, p00a->y, p00a->z, p01a->x, p01a->y,  p01a->z);
						//pxPrintf("p10a=%.12f %.12f %.12f, p11a=%.12f %.12f %.12f\n", p10a->x, p10a->y, p10a->z, p11a->x, p11a->y,  p11a->z);
						if (Gu::intersectRayTriangle(uhv0, duhvNorm, *p10a, *p00a, *p11a, triT0, triU, triV, backfaceCull, enlargeEpsilon) &&
							triT0 >= -tEpsilon && triT0 <= duhvLen+tEpsilon && !ISHOLE0)
						{
							...
							triT0 = PX_MAX_REAL;
						if (Gu::intersectRayTriangle(uhv0, duhvNorm, *p01a, *p11a, *p00a, triT1, triU, triV, backfaceCull, enlargeEpsilon)
