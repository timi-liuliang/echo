...
#if defined(__BIG_ENDIAN__) || defined(_XBOX) || defined(__CELLOS_LV2__)
...
#define CREATE_HISTOGRAMS(type, buffer)														\
	...
	PxMemZero(mHistogram1024, 256*4*sizeof(PxU32));											\
																							\
	/* Prepare to count */																	\
	const PxU8* PX_RESTRICT p = (PxU8*)input;												\
	...
	{																						\
		...
		const type* PX_RESTRICT Running = (type*)buffer;									\
...
//		CHECK_PASS_VALIDITY(j);
		...
		if(CurCount)
		{
			...
			// Perform Radix Sort
			PxU8* PX_RESTRICT InputBytes	= (PxU8*)input;
...
RadixSort& RadixSort::Sort(const float* input2, PxU32 nb)
{
	...
	PxU32* PX_RESTRICT input = (PxU32*)input2;
