...
//			tnear = FLT_MAX;
	...
	// PT: Xbox path disabled since it misses the fix for DE8395. No big deal anymore
	// since this code is now only used once per call (not in the traversal).
/*#ifdef _XBOX
							...
							PxVec3& hit, PxVec3& normal, PxReal& d)
{
	...
	// Backface culling
	const bool doCulling = true;	// PT: TODO: is that really correct?
	if(doCulling && (triNormal.dot(motion)) >= 0.0f)	// ">=" is important !
		return false;
	...
	// Box vertices VS triangle
	{
		...
		const PxVec3* VN = (const PxVec3*)getBoxVertexNormals();
