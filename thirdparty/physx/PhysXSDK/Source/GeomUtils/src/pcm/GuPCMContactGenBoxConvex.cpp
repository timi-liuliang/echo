...
namespace Gu
{
	...
	void buildPartialHull(const Gu::PolygonalData& polyData, SupportLocal* map, Gu::SeparatingAxes& validAxes, const Ps::aos::Vec3VArg planeP, const Ps::aos::Vec3VArg planeDir)
	{
		using namespace Ps::aos;
		...
		for(PxU32 i=0; i<polyData.mNbPolygons; ++i)
		{
			...
			Vec3V v0 = M33MulV3(map->vertex2Shape, V3LoadU(polyData.mVerts[inds[polygon.mNbVerts - 1]]));
			...
			for (PxU32 iStart = 0; iStart < polygon.mNbVerts; iStart++)
			{
				...
				const Vec3V v1 = M33MulV3(map->vertex2Shape, V3LoadU(polyData.mVerts[inds[iStart]]));
	...
	static bool testEdgeNormal(const Gu::PolygonalData& polyData0, const Gu::PolygonalData& polyData1, SupportLocal* map0, SupportLocal* map1, const Ps::aos::PsMatTransformV& transform0To1, const Ps::aos::PsMatTransformV& transform1To0, const Ps::aos::FloatVArg contactDist,
	...
	void generatedContacts(Gu::PolygonalData& polyData0, Gu::PolygonalData& polyData1,const Gu::HullPolygonData& referencePolygon, const Gu::HullPolygonData& incidentPolygon,  
		Gu::SupportLocal* map0, Gu::SupportLocal* map1, const Ps::aos::PsMatTransformV& transform0To1, Gu::PersistentContact* manifoldContacts, 
...
EdgeTest:
			if(doEdgeTest)
			{
				...
				if(!testEdgeNormal(polyData0, polyData1, map0, map1, transform0To1V, transform1To0V, contactDist, minOverlap, minNormal, EDGE0, status))
					return false;
			...
			if(status == POLYDATA0)
			{
				...
				const Gu::HullPolygonData& referencePolygon = polyData0.mPolygons[feature0];
				const Vec3V n = transform0To1V.rotate(minNormal);
				const Gu::HullPolygonData& incidentPolygon = polyData1.mPolygons[getPolygonIndex(polyData1, map1, n)];
				
				generatedContacts(polyData0, polyData1, referencePolygon,  incidentPolygon, map0, map1, transform0To1V, manifoldContacts, numContacts, contactDist);
				if(numContacts != origContactCount)
				{
					const Vec3V nn = V3Neg(n);
					//flip the contacts
					for(PxU32 i=origContactCount; i<numContacts; ++i)
					{
	...
	bool computeMTD(Gu::PolygonalData& polyData0, Gu::PolygonalData& polyData1,  SupportLocal* map0, SupportLocal* map1, Ps::aos::FloatV& penDepth, Ps::aos::Vec3V& normal)
	{
		...
		{
			PX_ASSERT(status == POLYDATA0 || status == EDGE0);
