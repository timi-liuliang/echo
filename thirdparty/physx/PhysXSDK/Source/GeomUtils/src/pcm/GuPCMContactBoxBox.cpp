	...
	case 3: //ub0
		{
			...
			if(FAllGrtrOrEq(zero, sign[3]))	
			{
				...
				newTransformV.p = V3ScaleAdd(axis10, eb0, transform1.p); //transform0.base3 - extents0.x*axis00;
			}
			else		
			{
				mtd = V3Neg(axis10);
				...
				newTransformV.p =V3NegScaleSub(axis10, eb0, transform1.p);//transform0.base3 + extents0.x*axis00;
			}
	...
	case 4: //ub1;
		{
			...
			else		
			{
				...
				newTransformV.p = V3NegScaleSub(axis11, eb1, transform1.p); //transform0.base3 + extents0.x*axis00;
	...
	if(bLostContacts || manifold.invalidate_BoxConvex(curRTrans, minMargin))	
	{
		...
		if(doBoxBoxGenerateContacts(boxExtents0, boxExtents1, transfV0, transfV1, contactDist, manifoldContacts, numContacts)) 
		{
			...
			else
			{
				...
				PxGJKStatus status = Gu::GJKRelativePenetration(box0, box1, aToB, contactDist, closestA, closestB, normal, penDep,
