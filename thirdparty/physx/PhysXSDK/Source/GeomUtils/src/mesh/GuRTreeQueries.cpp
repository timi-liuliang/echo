...
namespace
{
	const VecU32V signMask = VecU32VLoadXYZW((PxU32(1)<<31), (PxU32(1)<<31), (PxU32(1)<<31), (PxU32(1)<<31));
	const Vec4V epsFloat4 = V4Load(1e-9f);
	const Vec4V zeroes = V4Zero();
	const Vec4V twos = V4Load(2.0f);
	const Vec4V epsInflateFloat4 = Vec4VLoadXYZW(1e-7f, 1e-7f, 1e-7f, 1e-7f);
}
	...
	const PxVec3* fattenAABBs, PxF32 maxT) const
{
	...
	PxU32 stackPtr = 0;
	for (PxI32 j = PxI32(mNumRootPages-1); j >= 0; j --)
		stack[stackPtr++] = (mFlags & IS_DYNAMIC) ? pagePtrTo32Bits(mPages) : j*sizeof(RTreePage);
