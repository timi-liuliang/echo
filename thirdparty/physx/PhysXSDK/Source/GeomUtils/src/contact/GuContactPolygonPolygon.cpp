...
#ifdef _XBOX
	const int ild0 = int(PX_IR(localPlane0.d) ^ 0x80000000);
	const int ild1 = int(PX_IR(localPlane1.d) ^ 0x80000000);
#endif
...
//				PX_ASSERT(dd <= 0.0f);		// PT: dn is always positive, so dd is always negative
	...
	PxVec3* PX_RESTRICT verts1in0 = (PxVec3*)stackMemory;
	for(PxU32 i=0; i<numVerts1; i++)
	{
