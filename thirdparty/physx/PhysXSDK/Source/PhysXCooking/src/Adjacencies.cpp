...
bool Adjacencies::Walk(WALK_CALLBACK callback, void* user_data)
{
	...
	bool* Markers = (bool*)PX_ALLOC_TEMP(sizeof(bool)*mNbFaces, PX_DEBUG_EXP("bool"));
	PxMemZero(Markers, mNbFaces*sizeof(bool));
			...
			PxU32 Crossed[] = { 0, 1, 2 };
			{
				...
				for(PxU32 i=0;i<3;i++)
				{
					for(PxU32 j=0;j<2;j++)
					{
						...
						{
							Ps::swap(d[j], d[j+1]);
							Ps::swap(Links[j], Links[j+1]);
							Ps::swap(Indices[j], Indices[j+1]);
							Ps::swap(Edges[j], Edges[j+1]);
							Ps::swap(Crossed[j], Crossed[j+1]);
						}
					}
				}
			}
...
#endif
	{
		...
		while(nb_edges--)
		{
			...
			if(Ref0==LastRef0 && Ref1==LastRef1)
			{
				...
				if(Count==3)
				{
					Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, __FILE__, __LINE__, "Adjacencies::CreateDatabase: can't work on non-manifold meshes. Try to use the PxConvexFlag::eINFLATE_CONVEX flag. (see PxToolkit::createConvexMeshSafe)");
					return false;
				}
			}
			else
