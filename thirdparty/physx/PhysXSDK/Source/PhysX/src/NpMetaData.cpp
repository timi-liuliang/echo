...
// We don't want to put the meta-data stuff in the header, since it would drag the allocator in the SPU code.
...
//	PX_DEF_BIN_METADATA_ITEM(stream,		NpMaterial, InvasiveSetHook,	mTableHook,	0)
}
...
// 208 => 224 => 208 => 192 bytes
	...
	PX_DEF_BIN_METADATA_ITEM(stream,		NpShape, NpInternalShapeFlags,	mInternalShapeFlags,0)

#ifdef EXPLICIT_PADDING_METADATA
	PX_DEF_BIN_METADATA_ITEMS_AUTO(stream,	NpShape, PxU32,					mPadding,			PxMetaDataFlag::ePADDING)
#endif
...
// 48 bytes
	...
	{ PxMetaDataEntry tmp = {"char", "mPadding", 1 + (PxU32)PX_OFFSET_OF(NpConnectorArray, mBufferUsed), 3, 3, 0, PxMetaDataFlag::ePADDING, 0}; PX_STORE_METADATA(stream, tmp); }
...
void NpArticulationLinkArray::getBinaryMetaData(PxOutputStream& stream)
{
	...
	{ PxMetaDataEntry tmp = {"char", "mPadding", 1 + (PxU32)PX_OFFSET_OF(NpArticulationLinkArray, mBufferUsed), 3, 3, 0, PxMetaDataFlag::ePADDING, 0}; PX_STORE_METADATA(stream, tmp); }
...
static void getBinaryMetaData_PxvParticleSystemParameter(PxOutputStream& stream)
{	
	PX_DEF_BIN_METADATA_CLASS(stream,	PxvParticleSystemParameter)

	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, restParticleDistance, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, kernelRadiusMultiplier, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, viscosity, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, surfaceTension, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, fadeInTime, 0)	
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxU32,	flags, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxU32,	packetSizeMultiplierLog2, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, restitution, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, dynamicFriction, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, staticFriction, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, restDensity, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, damping, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, stiffness, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, maxMotionDistance, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, restOffset, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxReal, contactOffset, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxPlane,projectionPlane, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxU16,	particleReadDataFlags, 0)
	PX_DEF_BIN_METADATA_ITEM(stream,	PxvParticleSystemParameter, PxU32,	noiseCounter, 0)	
}
...
#if PX_USE_PARTICLE_SYSTEM_API
	getBinaryMetaData_PxvParticleSystemParameter(stream);
#endif

	MaterialIndicesStruct::getBinaryMetaData(stream);
	Gu::GeometryUnion::getBinaryMetaData(stream);
	Gu::ConvexMesh::getBinaryMetaData(stream);		// NP_CONVEX_MESH
	Gu::TriangleMesh::getBinaryMetaData(stream);		// NP_TRIANGLE_MESH
	Gu::HeightField::getBinaryMetaData(stream);		// NP_HEIGHTFIELD
