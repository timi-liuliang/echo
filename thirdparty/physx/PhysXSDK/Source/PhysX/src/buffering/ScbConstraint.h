...
private:
	...
	PX_FORCE_INLINE	const Buf*		getBufferedData()	const	{ return (const Buf*)getStream();	}
	PX_FORCE_INLINE	Buf*			getBufferedData()			{ return (Buf*)getStream();		}
};
...
{
	return isBuffered(BF_FLAGS) ? getBufferedData()->flags & (~PxConstraintFlag::eBROKEN | mBrokenFlag)
								: mConstraint.getFlags() & (~PxConstraintFlag::eBROKEN | mBrokenFlag);
}
...
{
	PX_ASSERT(!getScbScene()->isPhysicsRunning());
