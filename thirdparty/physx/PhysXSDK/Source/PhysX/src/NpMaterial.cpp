...
{
	PX_CHECK_AND_RETURN(PxIsFinite(x), "NpMaterial::setDynamicFriction: invalid float");
...
{
	PX_CHECK_AND_RETURN(PxIsFinite(x), "NpMaterial::setStaticFriction: invalid float");
...
{
	PX_CHECK_AND_RETURN(PxIsFinite(x), "NpMaterial::setRestitution: invalid float");
	PX_CHECK_MSG(((x >= 0.0f) && (x <= 1.0f)), "NpMaterial::setRestitution: Restitution value has to be in [0,1]!");
	if ((x < 0.0f) || (x > 1.0f))
	{
		PxClamp(x, 0.0f, 1.0f);
		Ps::getFoundation().error(PxErrorCode::eINVALID_PARAMETER, __FILE__, __LINE__, "NpMaterial::setRestitution: Invalid value %f was clamped to [0,1]!", x);
	}
