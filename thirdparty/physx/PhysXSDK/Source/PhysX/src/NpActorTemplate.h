...
// PT: this one is doubly slow, since "getScene" is virtual and "getScbSceneSLOW" has another v-call
template<class APIClass>
...
void NpActorTemplate<APIClass>::setName(const char* debugName)
{
	...
	GRB_EVENT(getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorSetName, static_cast<PxActor *>(this), debugName);
	...
	if(scbScene 
		&& scbScene->getSceneVisualDebugger().isConnected(true)
		&& scbScene->getSceneVisualDebugger().isInstanceValid(this))
			scbScene->getSceneVisualDebugger().updatePvdProperties(&scbActor);
#endif
}
...
void NpActorTemplate<APIClass>::setDominanceGroup(PxDominanceGroup dominanceGroup)
{
	...
	GRB_EVENT(getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorSetDominanceGroup, static_cast<PxActor *>(this), dominanceGroup);
}
...
PX_FORCE_INLINE void NpActorTemplate<APIClass>::setActorFlagInternal(PxActorFlag::Enum flag, bool value)
{
	...
	GRB_EVENT(getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorSetFlags, static_cast<PxActor *>(this), (PxU32)a.getActorFlags());
}
...
PX_FORCE_INLINE void NpActorTemplate<APIClass>::setActorFlagsInternal(PxActorFlags inFlags)
{
	...
	GRB_EVENT(getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorSetFlags, static_cast<PxActor *>(this), (PxU32)inFlags);
}
