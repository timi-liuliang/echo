...
//~PX_SERIALIZATION
	...
	void		addLaunchTaskDependentGpu(class PxBaseTask& dependent);
...
namespace
{
	...
	bool checkRestOffset(PxsParticleData& standaloneData, PxF32 restOffset)
	{
...
void NpParticleBaseTemplate<APIClass, LeafClass>::setRestOffset(PxReal r)
{
	...
	{	
		PxsParticleData* standaloneData = getScbParticleSystem().getScParticleSystem().obtainStandaloneData();
...
template<class APIClass, class LeafClass>
PxBaseTask* NpParticleBaseTemplate<APIClass, LeafClass>::getLaunchTaskGpu() const
{
	NP_READ_CHECK(NpActor::getOwnerScene(*this));
...
template<class APIClass, class LeafClass>
void NpParticleBaseTemplate<APIClass, LeafClass>::addLaunchTaskDependentGpu(class PxBaseTask& dependent)
{
	NP_WRITE_CHECK(NpActor::getOwnerScene(*this));
