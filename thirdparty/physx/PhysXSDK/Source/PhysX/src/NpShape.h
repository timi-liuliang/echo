...
public:
// PX_SERIALIZATION
												NpShape(PxBaseFlags baseFlags) : PxShape(baseFlags), mShape(PxEmpty), mInternalShapeFlags(PxEmpty)	{}
...
//~PX_SERIALIZATION
	...
	virtual			PxU32						getMaterials(PxMaterial** userBuffer, PxU32 bufferSize)	const;
...
private:
					...
					Scb::Shape					mShape;
					const char*					mName;
					NpInternalShapeFlags		mInternalShapeFlags;			// maybe we could pack this in with the ref count, or in Scb?
					char						mPadBytes[3];
};
