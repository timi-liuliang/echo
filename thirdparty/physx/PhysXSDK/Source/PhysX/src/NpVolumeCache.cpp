...
PX_FORCE_INLINE void makeHitSafe(HitType& hit) // hit can contain NaNs (due to uninitialized mem allocation) which cannot be copied
{
	...
	{
		PxLocationHit& hit1 = (PxLocationHit&)hit;
	...
	if (HitTypeSupport<HitType>::IsRaycast == 1 && !(((PxRaycastHit&)hit).flags & PxHitFlag::eUV) )
	{
		((PxRaycastHit&)hit).u = 0.0f;
		((PxRaycastHit&)hit).v = 0.0f;
	}
}
