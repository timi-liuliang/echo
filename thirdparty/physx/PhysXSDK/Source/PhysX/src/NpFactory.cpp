...
void NpFactory::addCollection(const Cm::Collection& collection)
{
	...
	const Pair<PxBase* const, PxSerialObjectId>* entries = collection.internalGetObjects();
...
{	
	...
	{
		const size_t offset = size_t(&(reinterpret_cast<NpParticleFluid*>(0)->getScbParticleSystem()));
		NpParticleFluid* np = reinterpret_cast<NpParticleFluid*>(reinterpret_cast<char*>(&scb)-offset);
		void* ud = np->userData;
		if(np->getBaseFlags() & PxBaseFlag::eOWNS_MEMORY)
