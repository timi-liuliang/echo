...
HandleManager::HandleManager() : mCurrentNbObjects(0), mNbFreeIndices(0)
{
	...
	mObjects		= (void**)PX_ALLOC(sizeof(void*)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));
	mOutToIn		= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));
	mInToOut		= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));
	mStamps			= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));
...
Handle HandleManager::Add(void* object)
{
	...
	else
	{
		...
		if(mCurrentNbObjects==mMaxNbObjects)
		{
			...
			void** NewList		= (void**)PX_ALLOC(sizeof(void*)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));		// New physical list
			PxU16* NewOTI		= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));		// New remapping table
			PxU16* NewITO		= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));		// New remapping table
			PxU16* NewStamps	= (PxU16*)PX_ALLOC(sizeof(PxU16)*mMaxNbObjects, PX_DEBUG_EXP("HandleManager"));		// New stamps
...
static PX_FORCE_INLINE void* encodeInternalHandle(PxU32 index, PxGeometryType::Enum type)
{
	...
	return (void*)((size_t(index)<<16)|(size_t(type)+1));
}
...
const PxObstacle* ObstacleContext::raycastSingle(PxRaycastHit& hit, const PxVec3& origin, const PxVec3& unitDir, const PxReal distance, ObstacleHandle& obstacleHandle) const
{
	...
	const PxHitFlags hintFlags = PxHitFlag::eDISTANCE;
