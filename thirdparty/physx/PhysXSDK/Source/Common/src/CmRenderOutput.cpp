...
namespace Cm
{
	...
	RenderOutput& RenderOutput::outputCapsule(PxF32 radius, PxF32 halfHeight, const Cm::Matrix34& absPose)
	{
		RenderOutput& out = *this;

		const PxMat44 m44 = ::convertToPxMat44(absPose);
		...
		out << m44;
		out.outputSegment(	m44.transform(PxVec3(-halfHeight,  radius, 0)),
							m44.transform(PxVec3( halfHeight,  radius, 0)));
		out.outputSegment(	m44.transform(PxVec3(-halfHeight, -radius, 0)),
							m44.transform(PxVec3( halfHeight, -radius, 0)));
		out.outputSegment(	m44.transform(PxVec3(-halfHeight,  0, radius)),
							m44.transform(PxVec3( halfHeight,  0, radius)));
		out.outputSegment(	m44.transform(PxVec3(-halfHeight, 0, -radius)),
							m44.transform(PxVec3( halfHeight, 0, -radius)));
