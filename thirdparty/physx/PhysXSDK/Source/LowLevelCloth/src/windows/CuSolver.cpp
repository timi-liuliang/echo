...
: CuContextLock(factory),
  ...
  mProfiler(profiler), 
  mSimulateEventId(mProfiler ? mProfiler->getEventIdForName("cloth::CuSolver::simulate") : (uint16_t)-1),
	...
	if(mClothDataDirty)
	{
		...
		if (mFactory.mNumThreadsPerBlock != numThreadsPerBlock)
		{
			checkSuccess( cuFuncSetBlockShape(mKernelFunction, 
				int(mFactory.mNumThreadsPerBlock = numThreadsPerBlock), 1, 1 ) );
		}
...
void cloth::CuSolver::executeKernel()
{
	...
	// launch kernel
	CUresult result = cuLaunchKernel(mKernelFunction, mCloths.size(), 1, 1, 
...
void cloth::CuSolver::interCollision()
{
	...
	for(uint32_t i=0, n = mCloths.size(); i < n; ++i)
	{
		...
		mInterCollisionInstances.pushBack(SwInterCollisionData(
			particles, particles + cloth.mNumParticles, numIndices, indices, 
			cloth.mTargetMotion, cloth.mParticleBoundsCenter,
			cloth.mParticleBoundsHalfExtent, elasticity, cloth.mUserData));
