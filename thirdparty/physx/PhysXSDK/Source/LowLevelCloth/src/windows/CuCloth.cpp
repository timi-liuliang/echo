...
bool cloth::CuCloth::updateClothData(CuClothData& clothData)
{
	...
	{
		uint32_t numSelfCollisionIndices = mSelfCollisionIndices.empty() ? 
			mNumParticles : uint32_t(mSelfCollisionIndices.size());
...
uint32_t cloth::CuCloth::getSharedMemorySize() const
{
	...
	return phaseConfigSize + sizeof(float) * (continuousCollisionSize 
		+ PxMax(selfCollisionSize, discreteCollisionSize));
}
...
void cloth::CuCloth::syncDeviceParticles()
{
	...
	{
		CuContextLock contextLock(mFactory);
		const PxVec4* src = mParticlesHostCopy.begin();
		mFactory.copyFromHost(src, src + 2*mNumParticles, mParticles.begin().get());
		mDeviceParticlesDirty = false;
	}
}
...
void ClothImpl<CuCloth>::setPhaseConfig( Range<const PhaseConfig> configs )
{
	...
	mCloth.setPhaseConfig( Range<const PhaseConfig>(
		transformedConfigs.begin(), transformedConfigs.end()) );
...
namespace
{
	uint32_t calculateNumReplays(const Vector<Vec4u>::Type& triplets, 
		const Vector<uint32_t>::Type setSizes)
	{
		uint32_t result = 0;
