...
void cloth::checkSuccessImpl( CUresult err, const char *file, const int line )
{
	if( err != CUDA_SUCCESS)
	{
		...
		switch(err)
		{
		#define ADD_CASE(X) case X: code = #X; break
		...
		default: ADD_CASE(CUDA_ERROR_UNKNOWN);
		#undef ADD_CASE
		}
		shdfnd::getFoundation().error(PxErrorCode::eINTERNAL_ERROR, 
			file, line, "CUDA error: %s", code);
	}
}
...
namespace
{
	...
	uint32_t getMaxThreadsPerBlock(const PxCudaContextManager& contextManager)
	{
...
cloth::CuFactory::CuFactory(PxCudaContextManager* contextManager)
: Factory(CUDA), 
...
cloth::Fabric* cloth::CuFactory::createFabric( uint32_t numParticles, 
...
{
	return new CuFabric( *this, numParticles, phases, sets, 
...
cloth::Solver* cloth::CuFactory::createSolver(physx::PxProfileZone* profiler, physx::PxTaskManager*)
{
	CuSolver* solver = new CuSolver(*this, profiler);
...
void cloth::CuFactory::copyFromHost( const void* srcIt, const void* srcEnd, void* dstIt ) const
{
	CuContextLock contextLock(*this);

	checkSuccess( cuMemcpyHtoD(CUdeviceptr(dstIt), srcIt, size_t(intptr_t(srcEnd)-intptr_t(srcIt))) );
}

void cloth::CuFactory::copyToHost( const void* srcIt, const void* srcEnd, void* dstIt, CUstream stream ) const
{
	CuContextLock contextLock(*this);

	checkSuccess( cuMemcpyDtoHAsync(dstIt, CUdeviceptr(srcIt), size_t(intptr_t(srcEnd)-intptr_t(srcIt)), stream)  );
}

void cloth::CuFactory::copyFromHost( const void* srcIt, const void* srcEnd, void* dstIt, CUstream stream ) const
{
	CuContextLock contextLock(*this);

	checkSuccess( cuMemcpyHtoDAsync(CUdeviceptr(dstIt), srcIt, size_t(intptr_t(srcEnd)-intptr_t(srcIt)), stream) );
}

void cloth::CuFactory::extractFabricData(const Fabric& fabric, 
...
void cloth::CuFactory::extractCollisionData(const Cloth& cloth, Range<PxVec4> spheres, 
	...
	Range<uint32_t> convexes, Range<PxVec3> triangles) const
{
	...
	if(!cuCloth.mCapsuleIndices.empty() && !capsules.empty())
		memcpy(capsules.begin(), &cuCloth.mCapsuleIndices.front(),
			cuCloth.mCapsuleIndices.size()*sizeof(IndexPair));
	...
	if(!cuCloth.mConvexMasks.empty() && !convexes.empty())
		memcpy(convexes.begin(), &cuCloth.mConvexMasks.front(),
			cuCloth.mConvexMasks.size()*sizeof(uint32_t));
...
void cloth::CuFactory::extractMotionConstraints(const Cloth& cloth, Range<PxVec4> destConstraints) const
{
	...
	else
	{
		...
		CuDeviceVector<PxVec4> const& srcConstraints = !cuCloth.mMotionConstraints.mTarget.empty() 
			? cuCloth.mMotionConstraints.mTarget : cuCloth.mMotionConstraints.mStart;

		PX_ASSERT(destConstraints.size() == srcConstraints.size());

		copyToHost(srcConstraints.begin().get(), 
			srcConstraints.end().get(), destConstraints.begin());
	}
}
...
void cloth::CuFactory::extractSeparationConstraints(const Cloth& cloth, Range<PxVec4> destConstraints) const
{
	...
	else
	{
		...
		CuDeviceVector<PxVec4> const& srcConstraints = !cuCloth.mSeparationConstraints.mTarget.empty() 
			? cuCloth.mSeparationConstraints.mTarget : cuCloth.mSeparationConstraints.mStart;
...
void cloth::CuFactory::extractVirtualParticles(const Cloth& cloth, 
	Range<uint32_t[4]> destIndices, Range<PxVec3> destWeights) const
{
	PX_ASSERT(&cloth.getFactory() == this);
	...
	if (destWeights.size() > 0)
	{
		...
		copyToHost(cuCloth.mVirtualParticleWeights.begin().get(),
			cuCloth.mVirtualParticleWeights.end().get(), &hostWeights.front());
	...
	if (destIndices.size() > 0)
	{
		...
		copyToHost(cuCloth.mVirtualParticleIndices.begin().get(),
			cuCloth.mVirtualParticleIndices.end().get(), &hostIndices.front());
...
void cloth::CuFactory::extractSelfCollisionIndices( 
	const Cloth& cloth, Range<uint32_t> destIndices ) const
{
	const CuCloth& cuCloth = static_cast<const CuClothImpl&>(cloth).mCloth;
	PX_ASSERT(destIndices.size() == cuCloth.mSelfCollisionIndices.size());
	copyToHost(cuCloth.mSelfCollisionIndices.begin().get(), 
		cuCloth.mSelfCollisionIndices.end().get(), destIndices.begin());
}

void cloth::CuFactory::extractRestPositions( 
	const Cloth& cloth, Range<PxVec4> destRestPositions ) const
{
	const CuCloth& cuCloth = static_cast<const CuClothImpl&>(cloth).mCloth;
	PX_ASSERT(destRestPositions.size() == cuCloth.mRestPositions.size());
	copyToHost(cuCloth.mRestPositions.begin().get(), 
		cuCloth.mRestPositions.end().get(), destRestPositions.begin());
}
