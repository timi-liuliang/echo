...
struct SwInterCollisionData
{
	...
	SwInterCollisionData(PxVec4* particles, 
		PxVec4* prevParticles, 
		uint32_t numParticles,
		uint32_t* indices,
		const PxTransform& globalPose,
		const PxVec3& boundsCenter,
		const PxVec3& boundsHalfExtents,
		float impulseScale,
		void* userData) : 
...
private:
	...
	void collideParticles(const uint32_t* keys, uint32_t firstColumnSize, 
		const uint32_t* sortedIndices, uint32_t numParticles, uint32_t collisionDistance);
