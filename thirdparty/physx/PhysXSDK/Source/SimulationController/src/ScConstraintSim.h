...
namespace Sc
{
	...
	public:
		...
		PX_INLINE		void					setConstantsLL(void* addr);
		PX_INLINE		const void*				getConstantsLL() const;

						void					postFlagChange(PxConstraintFlags oldFlags, PxConstraintFlags newFlags);

		PX_FORCE_INLINE	const PxsConstraint*	getLowLevelConstraint()	const	{ return &mLowLevelConstraint; }
		PX_FORCE_INLINE	PxsConstraint*			getLowLevelConstraint()			{ return &mLowLevelConstraint; }
	...
	public:
						...
						void					projectPose(BodySim* childBody);
	...
	private:
						PxsConstraintWriteback	mSolverOutput;
						PxsConstraint			mLowLevelConstraint;
						Scene&					mScene;
...
PX_INLINE void Sc::ConstraintSim::setConstantsLL(void* addr)
{
