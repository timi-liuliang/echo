...
namespace
{
	...
	void gatherMeshVertices(const PxTriangleMesh& mesh, Ps::Array<PxVec3>& vertices)
	{
...
void Sc::ClothSim::updateRigidBodyPositions()
{
	...
	for (PxU32 i = 0; i<mNumMeshes; ++i, ++shapeIt)
	{
		...
		if(geometry.triangleMesh->getTriangleMeshFlags() & PxTriangleMeshFlag::eHAS_16BIT_TRIANGLE_INDICES)
			gatherMeshVertices<PxU16>(*geometry.triangleMesh, curTriangles);
		else
			gatherMeshVertices<PxU32>(*geometry.triangleMesh, curTriangles);
