...
void Sc::ParticleSystemSim::addInteraction(const ParticlePacketShape& particleShape, const ShapeSim& shape, const PxU32 ccdPass)
{
	...
	mLLSim->addInteractionV(*particleShape.getLowLevelParticleShape(), (PxvShapeHandle)&shapeCore, (PxvBodyHandle)&rigidCore, isDynamic, (ccdPass > 0));
	mInteractionCount++; 
}
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(getScene(),ParticleSim,startStep);
...
{
	CM_PROFILE_ZONE(getScene().getEventProfiler(),Cm::ProfileEventId::ParticleSim::GetshapesUpdateProcessing() );
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(getScene(),ParticleSim,endStep );
