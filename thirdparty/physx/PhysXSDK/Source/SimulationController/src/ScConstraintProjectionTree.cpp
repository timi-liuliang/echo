...
void Sc::ConstraintProjectionTree::rankConstraint(ConstraintSim& c, BodyRank& br, PxU32& dominanceTracking)
{
...
void Sc::ConstraintProjectionTree::buildProjectionTrees(ConstraintGroupNode& root)
{
	...
	while (node0)	//for all nodes in group
	{
		...
		if (!node0->body->isKinematic())
		{
			...
			Cm::Range<Interaction*const> interactions = node0->body->getActorInteractions();
			for(; !interactions.empty(); interactions.popFront())
			{
				...
				if (interaction->getType() == PX_INTERACTION_TYPE_CONSTRAINTSHADER)
				{
					...
					rankConstraint(*c, br, dominanceTracking);
				}
			}
	...
	if (bodyRankArray.size())  // all of the bodies might have been switched to kinematic in which case there will be no ranked body
	{
		...
		ConstraintGroupNode** nodeQueue = (ConstraintGroupNode**)PX_ALLOC_TEMP(sizeof(ConstraintGroupNode*)*bodyRankArray.size(), PX_DEBUG_EXP("ProjectionNodeQueue"));
		if (nodeQueue)
		{
