...
namespace Sc
{
	...
	*/
	...
	public:
		...
		PX_FORCE_INLINE static void fillInContactReportExtraData(PxContactPairPose*, PxU32 index, const RigidSim&, bool isCCDPass);
		PX_FORCE_INLINE void fillInContactReportExtraData(PxU8* stream, PxU32 extraDataFlags, const RigidSim&, const RigidSim&, PxU32 ccdPass, PxU32 pairIndex, PxU32 sizeOffset);
...
PX_FORCE_INLINE void Sc::ContactStreamManager::fillInContactReportExtraData(PxContactPairVelocity* cpVel, PxU32 index, const RigidSim& rs, bool isCCDPass)
{
	if (rs.getActorType() != PxActorType::eRIGID_STATIC)
	{
		...
		else
		{
			...
			const Cm::SpatialVector& vel = bs.getLowLevelBody().getAccelerationV();
...
PX_FORCE_INLINE void Sc::ContactStreamManager::fillInContactReportExtraData(PxContactPairPose* cpPose, PxU32 index, const RigidSim& rs, bool isCCDPass)
{
	if (rs.getActorType() != PxActorType::eRIGID_STATIC)
	{
		const BodySim& bs = static_cast<const BodySim&>(rs);
...
PX_FORCE_INLINE void Sc::ContactStreamManager::fillInContactReportExtraData(PxU8* stream, PxU32 extraDataFlags, const RigidSim& rs0, const RigidSim& rs1, PxU32 ccdPass, PxU32 pairIndex, PxU32 sizeOffset)
{
	...
	if (extraDataFlags & PxPairFlag::eCONTACT_EVENT_POSE)
	{
		...
		ContactStreamManager::fillInContactReportExtraData(cpPose, 0, rs0, isCCDPass);
		ContactStreamManager::fillInContactReportExtraData(cpPose, 1, rs1, isCCDPass);
