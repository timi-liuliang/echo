...
struct RayAABBTest
{
	...
	{
		PxVec3 ext = maxDist >= PX_MAX_F32 ? PxVec3(unitDir_.x == 0 ? origin_.x : PxSign(unitDir_.x)*PX_MAX_F32,
	...
	{
		Vec3V ext = V3ScaleAdd(mDir, FLoad(distance), mOrigin);
	...
	PX_FORCE_INLINE PxU32 check(const Vec3V center, const Vec3V extents) const
	{
		...
		const BoolV andABCMasks = BAnd(BAnd(maskA, maskB), maskC);
				...
				return PxU32(_mm_movemask_ps(andABCMasks)==15);
			#else		
				return BAllEq(andABCMasks, BTTTT());
			#endif
		#else
			return BAllEq(andABCMasks, BTTTT());
		#endif
...
struct SphereAABBTest
{
	...
	{	
		...
		return (Ps::IntBool)BAllEq(FIsGrtrOrEq(mRadius2, V3Dot(d, d)), BTTTT());
	}
...
#define CAPSULE_AS_SEGMENT
#ifdef CAPSULE_AS_SEGMENT
...
{
	PX_FORCE_INLINE CapsuleAABBTest(const PxTransform& t, const PxCapsuleGeometry& c, const PxReal radiusInflation)
		: RayAABBTest(t.p-t.q.getBasisVector0()*c.halfHeight, t.q.getBasisVector0(), 2*c.halfHeight, PxVec3(c.radius*radiusInflation))
	{}
...
{
	CapsuleAABBTest(const PxTransform& t, const PxCapsuleGeometry& c)
	: mOrigin(V3LoadU(t.p))
	, mDir(V3LoadU(t.q.getBasisVector0()*c.halfHeight))
  	, mDirYZX(V3PermYZX(mDir))
	, mAbsDir(V3Abs(mDir))
	, mAbsDirYZX(V3Abs(mDirYZX))
	{	
		Vec3V radius = V3Load(c.radius);
		mExtents = V3Add(mAbsDir, radius);

		// magnitudes of the edge cross vectors
		Vec3V rs2 = V3MulAdd(mDir, mDir, V3Mul(mDirYZX, mDirYZX));
		Vec3V rs = V3SetX(FSqrt(V3GetX(rs2)), 
			       V3SetY(FSqrt(V3GetY(rs2)), 
				   V3SetZ(FSqrt(V3GetZ(rs2)), V3Zero()))); // TODO: V3Sqrt

		mScaledRadius = V3Mul(radius, rs);
	}

	Ps::IntBool operator()(const Vec3VArg center, const Vec3VArg extents) const			
	{	
		// coordinate axes
		const Vec3V offset = V3Sub(mOrigin, center);
		const BoolV maskA = V3IsGrtr(V3Abs(offset), V3Add(extents, mExtents));

		// edge cross axes
		const Vec3V offsetYZX = V3PermYZX(offset);
		const Vec3V extentsYZX = V3PermYZX(extents);
	
		const Vec3V f = V3NegMulSub(mDirYZX, offset, V3Mul(mDir, offsetYZX));
		const Vec3V g = V3MulAdd(extents, mAbsDirYZX, V3MulAdd(extentsYZX, mAbsDir, mScaledRadius));
		const BoolV maskC = V3IsGrtr(V3Abs(f), g);

		return BAllEq(BOr(maskC, maskA), BFFFF());
	}

private:
	const Vec3V mOrigin, mDir, mDirYZX, mAbsDir, mAbsDirYZX;
	Vec3V mExtents, mScaledRadius;
};
#endif
...
struct OBBAABBTests
{
	OBBAABBTests(const PxTransform&t, const PxBoxGeometry&b, PxReal inflation)										
	{
		const Vec3V eps = V3Load(1e-6f);

		mT = V3LoadU(t.p);
		mExtents = V3LoadU(b.halfExtents*inflation);	

		// storing the transpose matrices yields a simpler SIMD test
		mRT = Mat33V_From_PxMat33(PxMat33(t.q).getTranspose());	
	...
	{	
		...
		return (Ps::IntBool)BAllEq(BOr(ba, BOr(bb,bc)), BFFFF());
	}
