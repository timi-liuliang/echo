...
{
    CrossProductDataModel::CrossProductDataModel()
    {
        m_inputDataTypes = 
        {
            {"any", "A"},
    ...
    QJsonObject CrossProductDataModel::save() const
    {
        QJsonObject modelJson = NodeDataModel::save();
        return modelJson;
    }

    void CrossProductDataModel::restore(QJsonObject const &p)
    {
    }

    unsigned int CrossProductDataModel::nPorts(PortType portType) const
    {
    ...
    NodeDataType CrossProductDataModel::dataType(PortType portType, PortIndex portIndex) const
    {
    ...
    std::shared_ptr<NodeData> CrossProductDataModel::outData(PortIndex portIndex)
    {
        return m_outputs[portIndex];
    }

    void CrossProductDataModel::setInData(std::shared_ptr<NodeData> nodeData, PortIndex portIndex)
    {
        m_inputs[portIndex] = std::dynamic_pointer_cast<ShaderData>(nodeData);
        if (m_inputs[0] && m_inputs[1])
        {
            m_outputs[0] = OperationRules::instance().NewCrossProductOutput( DataAny::getInternalData(m_inputs[0])->type().id, DataAny::getInternalData(m_inputs[1])->type().id, this);
            m_outputs[0]->setVariableName(getVariableName());
        }
        else
        {
    ...
    bool CrossProductDataModel::generateCode(ShaderCompiler& compiler)
    {
        if (m_inputs[0] && m_inputs[1])
        {
            compiler.addCode(Echo::StringUtil::Format("\t%s %s = cross(%s, %s);\n",
