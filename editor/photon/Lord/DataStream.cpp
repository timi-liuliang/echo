...
namespace Lightmass
{
		...
		const string& delim)
	{
		if (delim.empty())
		{
			exception("Error: No delimiter provided ------ FileStreamDataStream::readLine");
		}
		if (delim.size() > 1)
		{
			exception("WARNING: FileStreamDataStream::readLine ------ using only first delimeter");
		}
		// Deal with both Unix & Windows LFs
		bool trimCR = false;
		if (delim.at(0) == '\n') 
		...
		else if (mpInStream->fail())
		{
			...
			{
				exception("Error: Streaming error occurred ------- FileStreamDataStream::readLine");
			}
		}
		else 
		{
