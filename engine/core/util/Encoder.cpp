...
#define COMPBYTE(x, y) ((unsigned char)(x) << 8 | (unsigned char)(y))
	...
	const unsigned short *_initTrad2Simp_utf16()
	{
		unsigned short *gbk2utf16 = ECHO_ALLOC_T(unsigned short, 0x10000);
		unsigned short n;
		unsigned short d;
		unsigned short c = 0;
		n = _gbk2utf16_2_size;
	...
	void _NormalizeUTF8(char *text, unsigned options)
	{
		const char *pTransTable =
			(options & _ENCODING_LOWER) ? _pUpper2Lower : (
			(options & _ENCODING_UPPER) ? _pLower2Upper :
			_pPlain);
		const unsigned short *pTransTable_utf16 =
			(options & _ENCODING_SIMPLIFIED) ? _pTrad2Simp_utf16 : _pPlain_utf16;
	...
	void _NormalizeGBK(char *text, unsigned options)
	{
		const char *pTransTable =
			(options & _ENCODING_LOWER) ? _pUpper2Lower : (
			(options & _ENCODING_UPPER) ? _pLower2Upper :
		...
		const unsigned short *pTransTable_gbk =
			(options & _ENCODING_SIMPLIFIED) ? _pTrad2Simp_gbk : _pPlain_gbk;
		unsigned i_from = 0, i_to = 0, flag = 0;

		for (; text[i_from]; i_from ++)
		{
			if (flag && (options & _ENCODING_HALF))
			{
	...
	void _GBK_TO_UTF8(const char *from, unsigned int from_len, char **to, unsigned int *to_len)
	{
		char *result = *to;
		unsigned i_from = 0;
		unsigned i_to = 0;
		...
		for (i_from = 0; i_from < from_len; i_from ++)
		{
			...
			{
				flag = 0;
				unsigned short tmp = _pGbk2Utf16[COMPBYTE(from[i_from - 1], from[i_from]) & ~0x8000];
	...
	void _UTF8_TO_GBK(const char *from, unsigned int from_len, char **to, unsigned int *to_len)
	{
		char *result = *to;
		unsigned i_from = 0;
		unsigned i_to = 0;
