	...
	const int bufferSizeInByte)
{
	...
	string128 single_name;
	const int parent_index = _get_parent_table(function, single_name);
	...
	{
		_stack_setfield(L, -2, single_name.c_str());
		_stack_pop(L, 1);  // ensure stack blance
	}
...
void Lua::set_readonly(const char* const name)
{
	...
	string128 single_name;
	const int parentIndex = _get_parent_table	(name, single_name);				// stack + 1 or 0
	lua_pushvalue		(L, new_table_index);		// stack + 1
	if (parentIndex > 0)
		lua_setfield(L, -2, single_name.c_str());	// stack - 1
	else
		lua_setglobal(L, single_name.c_str());		// stack - 1
...
//{
//	return 0 != lua_isinteger(L, index);
//}
...
{
	return 0 != lua_isuserdata(L, index);
}
