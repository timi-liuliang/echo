...
//inline void		_stack_pushvalue		(lua_State* L, unsigned int v)	{ _stack_pushnumber(L, v);						}
...
//template<> inline unsigned int			_stack_tovalue<unsigned int>(lua_State* L, const int index) { return static_cast<unsigned int>(_stack_tonumber(L, index)); }
...
//	_stack_gettable(L, 1); //push self[_CHILL_LUA_CPP_PTR_NAME_] to the current stack
