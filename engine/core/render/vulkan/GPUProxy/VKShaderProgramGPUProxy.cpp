...
namespace LORD
{
	...
	{
		for (uint i = 0; i < RenderInput::VS_MAX; ++i)
		{
	...
	void GLES2ShaderProgramGPUProxy::bindUniforms(ShaderParamType type, int location, int count, Byte* value)
	{
		...
		case SPT_TEXTURE:	OGLESDebug(glUniform1i(location, *(uint*)value));							break;
	...
	void GLES2ShaderProgramGPUProxy::attachShader(uint shader)
	{
		OGLESDebug(glAttachShader(m_hProgram, shader));
	}

	void GLES2ShaderProgramGPUProxy::detachShader(uint shader)
	{
