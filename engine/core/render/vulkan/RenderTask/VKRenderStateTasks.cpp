...
namespace LORD
{
		...
		, m_frontFace(frontFace)
	{
		if (currDesc)
		{
			m_currDesc = LordNew(RasterizerState::RasterizerDesc);
			*m_currDesc = *currDesc;
		}
	}
		...
		, m_prev_desc(nullptr)
	{
		if (curr_desc)
		{
			m_curr_desc = LordNew(SamplerState::SamplerDesc);
			*m_curr_desc = *curr_desc;
		}

		if (prev_desc)
		{
			m_prev_desc = LordNew(SamplerState::SamplerDesc);
