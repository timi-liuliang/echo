#include "shader_node_radian_to_degree.h"
...
namespace Echo
{
    ShaderNodeRadianToDegree::ShaderNodeRadianToDegree()
        : ShaderNode()
    {
        m_inputDataTypes = 
        {
	...
	void ShaderNodeRadianToDegree::bindMethods()
	{

	}

    void ShaderNodeRadianToDegree::setInData(std::shared_ptr<NodeData> nodeData, QtNodes::PortIndex portIndex)
    {
    ...
    bool ShaderNodeRadianToDegree::generateCode(Echo::ShaderCompiler& compiler)
    {
        if (m_inputs[0])
        {
            compiler.addCode(Echo::StringUtil::Format("\t%s %s = degrees(%s);\n",
