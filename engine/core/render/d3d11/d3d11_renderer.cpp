...
#include "d3d11_render_proxy.h"
#include "d3d11_compute_proxy.h"
...
namespace Echo
{
    ...
    bool D3D11Renderer::initialize(const Settings& settings)
    {
	...
	GPUBuffer* D3D11Renderer::createVertexBuffer(Dword usage, const Buffer& buff)
	{
		return EchoNew(VKBuffer(GPUBuffer::GPUBufferType::GBT_VERTEX, usage, buff));
	}

	GPUBuffer* D3D11Renderer::createIndexBuffer(Dword usage, const Buffer& buff)
	{
		return EchoNew(VKBuffer(GPUBuffer::GPUBufferType::GBT_INDEX, usage, buff));
	}

    Texture* D3D11Renderer::createTexture2D(const String& name)
    {
        return EchoNew(VKTexture2D(name));
    }

	TextureRenderTarget2D* D3D11Renderer::createTextureRender(const String& name)
	{ 
		return EchoNew(VKTextureRender(name)); 
	}

	void D3D11Renderer::setTexture(ui32 index, Texture* texture, bool needUpdate)
	{
		m_currentTextures[index] = texture;
	}

	VKTexture* D3D11Renderer::getTexture(ui32 index)
	{
		return ECHO_DOWN_CAST<VKTexture*>(m_currentTextures[index]);
	}

    RenderProxy* D3D11Renderer::createRenderProxy()
    {
	...
	ComputeProxy* D3D11Renderer::createComputeProxy()
	{
		static ui32 id = 0; id++;
	...
	void D3D11Renderer::getDepthRange(Vector2& vec)
	{
		vec.x = 0.0f;
		vec.y = 1.0f;
	}
	...
	void D3D11Renderer::convertMatView(Matrix4& mat)
	{
		mat.m01 = -mat.m01;
		mat.m11 = -mat.m11;
		mat.m21 = -mat.m21;
	...
	void D3D11Renderer::unproject(Vector3& worldPos, const Vector3& screenPos, const Matrix4& matVP, Viewport* viewport)
	{
	...
	FrameBufferWindow* D3D11Renderer::createFrameBufferWindow()
	{
		static VKFramebufferWindow* frameBufferWindow = EchoNew(VKFramebufferWindow);
		return frameBufferWindow;
	}

    RasterizerState* D3D11Renderer::createRasterizerState()
    {
        return EchoNew(VKRasterizerState);
    }

    DepthStencilState* D3D11Renderer::createDepthStencilState()
    {
        return EchoNew(VKDepthStencilState);
    }

    BlendState* D3D11Renderer::createBlendState()
    {
        return EchoNew(VKBlendState);
    }

    MultisampleState* D3D11Renderer::createMultisampleState()
    {
        return EchoNew(VKMultisampleState);
    }

    SamplerState* D3D11Renderer::createSamplerState()
    {
        return EchoNew(VKSamplerState);
    }

	void D3D11Renderer::createVkInstance()
	{
	...
	i32 D3D11Renderer::calcVkPhysicalDeviceScore(const VkPhysicalDevice& device)
	{
	...
	void D3D11Renderer::pickVkPhysicalDevice()
	{
		ui32 deviceCount = 0;
	...
	void D3D11Renderer::enumerateQueueFamalies()
	{
	...
	const ui32 D3D11Renderer::getGraphicsQueueFamilyIndex()
	{
    ...
    const ui32 D3D11Renderer::getPresentQueueFamilyIndex(VkSurfaceKHR vkSurface)
    {
        for (size_t i = 0; i < m_vkQueueFamilies.size(); i++)
        {
	...
	void D3D11Renderer::createVkLogicalDevice()
	{
		ui32 extensionCount = 0;
	...
	void D3D11Renderer::createVkValidation()
	{
		m_validation.create();
	}

	void D3D11Renderer::createVkCommandPool()
	{
	...
	void D3D11Renderer::createVkDescriptorPool()
	{
	...
	ui32 D3D11Renderer::findVkMemoryType(ui32 typeBits, VkMemoryPropertyFlags properties)
	{
	...
	VkCommandBuffer D3D11Renderer::createVkCommandBuffer()
	{
	...
	void D3D11Renderer::initRayTracer()
	{
		m_rayTracer = EchoNew(VKRayTracer);
		m_rayTracer->init(m_vkPhysicalDevice);
	}

	void D3D11Renderer::flushVkCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue, bool free)
	{
		if (commandBuffer)
		{
	...
	void D3D11Renderer::submitSingleTimeCommands(const std::function<void(VkCommandBuffer)>& action)
	{
    ...
    void D3D11Renderer::onSize(int width, int height)
    {
		m_screenWidth = width;
    ...
    void D3D11Renderer::draw(RenderProxy* renderable, FrameBufferPtr& frameBuffer)
    {
