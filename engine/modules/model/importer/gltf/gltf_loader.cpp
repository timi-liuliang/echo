...
namespace Gltf
{
	...
	MaterialInfo  MaterialInfo::DEFAULT = MaterialInfo();
	...
	bool Loader::load(const ResourcePath& path)
	{
		...
		if (memReader.getSize())
		{
			...
			if (!loadAsset(j))
			{
				EchoLogError("gltf parse asset failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadScenes(j))
			{
				EchoLogError("gltf parse scenes failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadNodes(j))
			{
				EchoLogError("gltf parse nodes failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadBuffers(j))
			{
				EchoLogError("gltf parse buffers failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadBufferViews(j))
			{
				EchoLogError("gltf parse bufferViews failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadAccessors(j))
			{
				EchoLogError("gltf parse accessors failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadMaterials(j))
			{
				EchoLogError("gltf parse materials failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadImages(j))
			{
				EchoLogError("gltf parse images failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadSamplers(j))
			{
				EchoLogError("gltf parse samples failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadTextures(j))
			{
				EchoLogError("gltf parse textures failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadMeshes(j))
			{
				EchoLogError("gltf parse meshes failed when load resource [%s].", path.getPath().c_str());
				return false;
			}

			if (!loadSkins(j))
			{
				EchoLogError("gltf parse skins failed when load resource [%s]", path.getPath().c_str());
				return false;
			}

			if (!loadAnimations(j))
			{
				EchoLogError("gltf parse animations failed when load resource [%s]", path.getPath().c_str());
				return false;
			}
	...
	bool Loader::loadAsset(nlohmann::json& json)
	{
	...
	bool Loader::loadScenes(nlohmann::json& json)
	{
	...
	bool Loader::loadNodes(nlohmann::json& json)
	{
	...
	bool Loader::loadBuffers(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < buffers.size(); i++)
		{
			...
			if (buffer.find("uri") != buffer.end())
			{
				...
				if (StringUtil::StartWith(bufferUri, "data:"))
				{
					...
					m_buffers[i].m_uriType = BufferInfo::UriType::Data;
				}
				else if (StringUtil::StartWith(bufferUri, "blob:"))
				{
					m_buffers[i].m_uri = bufferUri;
					m_buffers[i].m_uriType = BufferInfo::UriType::Blob;
				}
				else
				{
	...
	bool Loader::loadBufferData(BufferInfo& buffer)
	{
		if (buffer.m_uri.empty() && buffer.m_byteLength > 0)
			return false;

		// create MemoryReader based on Uri type
		if (buffer.m_uriType == BufferInfo::UriType::Data)
		{
			// extract data
	...
	bool Loader::loadAccessors(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < accessors.size(); ++i)
		{
			...
			m_accessors[i].m_componentType = static_cast<AccessorInfo::ComponentType>(accessor["componentType"].get<ui16>());
			...
			if (type == "SCALAR")		m_accessors[i].m_type = AccessorInfo::Type::Scalar;
			else if (type == "VEC2")	m_accessors[i].m_type = AccessorInfo::Type::Vec2;
			else if (type == "VEC3")	m_accessors[i].m_type = AccessorInfo::Type::Vec3;
			else if (type == "VEC4")	m_accessors[i].m_type = AccessorInfo::Type::Vec4;
			else if (type == "MAT2")	m_accessors[i].m_type = AccessorInfo::Type::Mat2;
			else if (type == "MAT3")	m_accessors[i].m_type = AccessorInfo::Type::Mat3;
			else if (type == "MAT4")	m_accessors[i].m_type = AccessorInfo::Type::Mat4;
			else						return false;
	...
	bool Loader::loadBufferViews(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < bufferViews.size(); i++)
		{
			...
			if (parseJsonValueUI32(target, bufferView, "target", false))
				m_bufferViews[i].m_target = static_cast<BufferViewInfo::TargetType>(target);
			else
				return false;
		}
	...
	bool Loader::loadAnimations(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < animations.size(); i++)
		{
			...
			for (ui32 j = 0; j < channels.size(); j++)
			{
				...
				AnimChannel& animChannel = m_animations[i].m_channels[j];
				...
				{
					if (StringUtil::Equal(path, "Rotation", false))			animChannel.m_path = AnimChannel::Path::Rotation;
					else if (StringUtil::Equal(path, "Scale", false))		animChannel.m_path = AnimChannel::Path::Scale;
					else if (StringUtil::Equal(path, "Translation", false))	animChannel.m_path = AnimChannel::Path::Translation;
					else if (StringUtil::Equal(path, "Weigths", false))		animChannel.m_path = AnimChannel::Path::Weights;
			...
			for (ui32 j = 0; j < samplers.size(); j++)
			{
				...
				AnimSampler& animSampler = m_animations[i].m_samplers[j];
	...
	static AnimProperty::Type MapAnimPropertyType(AccessorInfo::Type type)
	{
		switch (type)
		{
		case AccessorInfo::Type::Scalar:	return AnimProperty::Type::Float;
		case AccessorInfo::Type::Vec3:		return AnimProperty::Type::Vector3;
		case AccessorInfo::Type::Vec4:		return AnimProperty::Type::Quaternion;
	...
	bool Loader::buildAnimationData()
	{
		for (AnimInfo& anim : m_animations)
		{
			//AnimClip* animClip = EchoNew(AnimClip);
			//animClip->m_name = anim.m_name;

			for (AnimChannel& channel : anim.m_channels)
			{
				AnimSampler& sampler = anim.m_samplers[channel.m_sampler];
				{
					...
					// properties
					AccessorInfo& timeAccess = m_accessors[sampler.m_input];
					AccessorInfo& keyAccess = m_accessors[sampler.m_output];
	...
	bool Loader::loadMeshes(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < meshes.size(); i++)
		{
			...
			for (ui32 j = 0; j < primitives.size(); j++)
			{
				...
				if (primitive.find("mode") != primitive.end())
				{
					...
					m_meshes[i].m_primitives[j].m_mode = Primitive::Mode(primitive["mode"].get<ui8>());
				}
	...
	bool Loader::buildPrimitiveData(int meshIdx, int primitiveIdx)
	{
		...
		{
			AccessorInfo& access = m_accessors[primitive.m_indices];
			indicesDataVoid = getAccessData<Byte*>(access);
			indicesCount = access.m_count;

			// stride
			if (access.m_componentType == AccessorInfo::UnsignedByte)
				indicesStride = 1;
			else if (access.m_componentType == AccessorInfo::UnsignedShort)
				indicesStride = 2;
			else if (access.m_componentType == AccessorInfo::UnsignedInt)
				indicesStride = 4;
			else
			{
		...
		{
			AccessorInfo& access = m_accessors[it.second];
		...
		for (auto& it : primitive.m_attributes)
		{
			...
			AccessorInfo& access = m_accessors[attributeIdx];
			vertexData.set(vertFormat, access.m_count);
			if (it.first == "POSITION")
			{
				if (access.m_type == AccessorInfo::Vec3 && access.m_componentType == AccessorInfo::ComponentType::Float)
				{
			...
			{
				if (access.m_type == AccessorInfo::Vec2 && access.m_componentType == AccessorInfo::ComponentType::Float)
				{
			...
			{
				if (access.m_type == AccessorInfo::Vec4 && access.m_componentType == AccessorInfo::ComponentType::Float)
				{
			...
			{
				if (access.m_type == AccessorInfo::Vec4 && access.m_componentType == AccessorInfo::ComponentType::UnsignedShort)
				{
					ui8 joint[4];
	...
	bool Loader::loadSkins(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < skins.size(); i++)
		{
			...
			{
				AccessorInfo& access = m_accessors[skin.m_inverseBindMatrices];
				if (access.m_type == AccessorInfo::Type::Mat4)
				{
	...
	bool Loader::buildMaterial(int meshIdx, int primitiveIdx)
	{
		...
		primitive.m_shader = nullptr;// GltfMaterial::getPbrMetalicRoughnessContent();
	...
	bool Loader::loadMaterials(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < materials.size(); i++)
		{
			...
			if (material.find("alphaMode") != material.end())
			{
				...
				if (type == "OPAQUE")		m_materials[i].m_alphaMode = MaterialInfo::AlphaMode::Opaque;
				else if (type == "MASK")	m_materials[i].m_alphaMode = MaterialInfo::AlphaMode::Mask;
				else if (type == "BLEND")	m_materials[i].m_alphaMode = MaterialInfo::AlphaMode::Blend;
				else                        return false;
			}
	...
	bool Loader::loadTextureInfo(MaterialInfo::Texture& texture, nlohmann::json& json)
	{
		if (!json.is_object())
			return false;
	...
	bool Loader::loadImages(nlohmann::json& json)
	{
	...
	bool Loader::loadSamplers(nlohmann::json& json)
	{
		...
		for (ui32 i = 0; i < samplers.size(); i++)
		{
			...
			if (parseJsonValueI32(iValue, sampler, "magFilter", false))
				m_samplers[i].m_magFilter = SamplerInfo::MagFilter(iValue);
			else
				return false;

			// minFilter
			if (parseJsonValueI32(iValue, sampler, "minFilter", false))
				m_samplers[i].m_minFilter = SamplerInfo::MinFilter(iValue);
			else
				return false;

			// wrapS
			if (parseJsonValueI32(iValue, sampler, "wrapS", false))
				m_samplers[i].m_wrapS = SamplerInfo::WrappingMode(iValue);
			else
				return false;

			// wrapT
			if (parseJsonValueI32(iValue, sampler, "wrapT", false))
				m_samplers[i].m_wrapT = SamplerInfo::WrappingMode(iValue);
			else
				return false;
	...
	bool Loader::loadTextures(nlohmann::json& json)
	{
	...
	i32  Loader::getNodeIdxByMeshIdx(i32 meshIdx)
	{
