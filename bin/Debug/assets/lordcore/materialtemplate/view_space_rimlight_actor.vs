...
void main ()
{
	xlv_TEXCOORD0 = inTexCoord;

	vec4 transform = TransformByBone(vec4(inPosition.xyz, 1.0));

	worldNormal = normalize(mat3(matW) * RotationByBone(inNormal));
	viewNormal = normalize(mat3(matWV) * RotationByBone(inNormal));
	worldPos = (matW * transform).xyz;
...
#ifdef RIME_LIGHT
	rimelight_viewDir = normalize(vec3(0.0) - (matWV * transform).xyz);
#endif
